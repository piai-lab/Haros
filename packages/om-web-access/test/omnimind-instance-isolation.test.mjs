...
function makeHarness(configService, curatorPresenter) {
	...
	const entries = [];
	makeOmniMindWebAccessExtension({ configService, curatorPresenter })({
...
function makePresenter() {
	...
	return {
		requests,
		settlements,
		async snapshot() { return { locale: "en", theme: "dark" }; },
		async present(request) {
			requests.push(request);
			return { kind: "presented", tabId: `tab-${request.surfaceId}` };
		},
		async settle(input) { settlements.push(input); },
	};
}
...
test("call terminal, Run abort, session_tree, and shutdown clean only their lifecycle scope", { timeout: 20_000 }, async (t) => {
	const originalFetch = globalThis.fetch;
	t.after(() => { globalThis.fetch = originalFetch; });
	globalThis.fetch = async (url, init) => {
	...
	const root = await mkdtemp(join(tmpdir(), "omnimind-web-lifecycle-"));
	...
	const first = harness.tool("web_search").execute(
		"call-first",
		{ query: "first", provider: "exa" },
		undefined,
		undefined,
		context,
	);
	const second = harness.tool("web_search").execute(
		"call-second",
		{ query: "second", provider: "exa" },
		undefined,
		undefined,
		context,
	);
	...
	const runOne = harness.tool("web_search").execute(
		"run-call-one",
	...
	const runTwo = harness.tool("web_search").execute(
		"run-call-two",
	...
	const independent = harness.tool("web_search").execute(
		"independent-call",
	...
	const secondaryPending = secondary.tool("web_search").execute(
		"secondary-call",
	...
	await waitFor(() => secondaryPresenter.requests.length === 1, "secondary Session Curator was not presented");
...
test("recoverable presentation stays pending while fatal presentation settles with a typed error", { timeout: 10_000 }, async (t) => {
	const originalFetch = globalThis.fetch;
	t.after(() => { globalThis.fetch = originalFetch; });
	globalThis.fetch = async (url, init) => {
		if (String(url).startsWith("https://api.exa.ai/answer")) {
	...
	const root = await mkdtemp(join(tmpdir(), "omnimind-web-presentation-"));
	...
	const updates = [];
	const recoverablePending = recoverableHarness.tool("web_search").execute(
		"recoverable-call",
	...
	const fatal = await fatalHarness.tool("web_search").execute(
		"fatal-call",
