...
| `workflow` | `auto-summary` (OmniMind profile default; with presentation off, generate a summary without opening the curator or waiting for review), `summary-review` (upstream interactive default when Pi UI is present; pause for source review), or `none` (return raw results) |
