...
It is composed only into the bundled HarnessOS Agent. It is not a Host capability, HostGateway backend, npm release, or supported stock Pi package, and it never reads or synchronizes stock Pi's `.pi` state. The exact source adoption is recorded in [`source-adoptions.json`](../../source-adoptions.json); packaged and release evidence are intentionally not duplicated in this package README.
