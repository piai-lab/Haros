## HarnessOS distribution note

This directory is the private workspace package `@harnessos/oa-web-access`, adopted from Nico Bailon's MIT-licensed `pi-web-access@0.25.0` at exact commit `08e347f4fe6bea807882c2363527118cce6eb539`. The upstream structure, legal text, mature Provider/fetch/Curator implementation, and author tests remain here; HarnessOS's bounded P1–P6 lifecycle differences are tracked in [`research/pi-web-access-intake.md`](../../research/pi-web-access-intake.md).

It is composed only into the bundled HarnessOS Agent. It is not a Host capability, AgentGateway backend, npm release, or supported stock Pi package, and it never reads or synchronizes stock Pi's `.pi` state. The exact source adoption is recorded in [`source-adoptions.json`](../../source-adoptions.json); packaged and release evidence are intentionally not duplicated in this package README.

The bundled HarnessOS profile defaults to `workflow: "auto-summary"` and `autoOpenBrowser: false`. In this profile `autoOpenBrowser` is an independent, nonblocking Right Dock process display for `auto-summary` and `none`; it never changes their result workflow or creates waiting-for-user. The retained upstream/default export still follows the author's UI-dependent workflow and browser-opening semantics described below.
...
web_search({ query: "...", provider: "broad" })
...
| `provider` | Configured provider when omitted or set to `auto`; `broad` uses up to three actually available services with configured routes first and free/session paths filling remaining slots; `all` searches every eligible provider except Parallel MCP, DuckDuckGo, Kimi, AnySearch, Valyu, xAI, Bright Data, SerpBase, and Serper simultaneously; otherwise a named service or non-empty named-service array is strict and explicit |
| `includeContent` | Fetch full page content from sources in background |
| `workflow` | `auto-summary` (HarnessOS profile default; with presentation off, generate a summary without opening the curator or waiting for review), `summary-review` (upstream interactive default when Pi UI is present; pause for source review), or `none` (return raw results) |
...
```
web_search(query)
  → auto: ordered route, first success stops
  → broad: up to 3 actually available services, partial success returns
  → all: every eligible service in parallel
...
Results get injected into the conversation when you approve the summary or click "Send selected results without summary". On timeout, the curator auto-submits and falls back to a deterministic summary if no approved draft is present. In the HarnessOS profile, `auto-summary` does not wait for approval: the same completed summary is returned to the Agent and, when the observer is open, shown read-only after the result cards and Add Search row. Explicit `summary-review` keeps generation, editing, regeneration, preview, and approval in that same inline tail position.
...
Set `geminiAuth` to `"adc"` together with `geminiProject` and `geminiLocation` to use Google Application Default Credentials through Vertex AI. ADC supports grounded search and inline generation, but not Gemini Files API video upload. An explicit Gemini API key or compatible base URL continues to take precedence when configured.
...
`provider` and `searchProvider` also accept `"duckduckgo"`; it remains explicit-only.

`provider` and `searchProvider` also accept `"broad"`. It is the bounded multi-source route: explicit advanced routing comes first, then configured auto-eligible providers, then free/session/zero-config paths fill remaining slots; candidates are de-duplicated, checked for actual availability, and capped at three. Explicit-only providers join broad only when the user placed them in advanced routing or an explicit provider array. Multi-query fan-out is bounded independently: auto/named/single-provider routes run at most three queries concurrently, broad and two-to-three-provider arrays run at most two, and all plus larger arrays run queries serially while retaining provider-internal concurrency. Results and Artifact indexes always keep input query order.

Summary generation receives every query answer plus source titles and URLs, not fetched page bodies, and never owns the source directory. Both model summaries and deterministic fallback output get one complete, stable, de-duplicated source directory appended mechanically. Stored answers, snippets, titles, and URLs remain available by query index through `get_search_content`; fetched page bodies keep their existing Artifact TTL, Session ownership, and cleanup lifecycle.

### Broad providers

Set `provider: "broad"` for important, open-ended, disputed, or multi-perspective questions that need more than the first successful route without contacting every configured service. It selects no more than three actually available providers, preserves successful answers separately, reports local provider failures, and returns as soon as the selected fan-out settles. One success is enough; when every selected provider fails, the error carries the complete typed broad-route diagnostics. Fewer than three available providers naturally means a two-provider or single-provider search—broad never upgrades itself to `all`.
...
| `search-result.ts` | Canonical source ordering, complete source directory, Agent result, and Artifact read projection |
