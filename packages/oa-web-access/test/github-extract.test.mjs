...
test("malformed GitHub identifiers cannot delete outside the clone cache", async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({ githubClone: { clonePath: join(root, "cache") } }),
    "utf8",
  );
...
test(
  "clearCloneCache removes only its verified clone entry",
  ...
  async () => {
    ...
    const digest = createHash("sha256")
      .update(JSON.stringify(["owner", "repo", null]))
      .digest("hex");
...
test(
  "clone cleanup unlinks a direct-child symlink without deleting its target",
  { skip: process.platform === "win32" },
  async () => {
...
test(
  "GitHub clones disable interactive credential prompts",
  { skip: process.platform === "win32" },
  async () => {
    ...
    await writeFakeExecutable(
      ...
      `
		...
		writeFileSync(${JSON.stringify(ghEnvFile)}, JSON.stringify({
			providerSecret: process.env.HARNESSOS_TEST_PROVIDER_SECRET,
    ...
    const child = spawnSync(process.execPath, ["--input-type=module"], {
      ...
      env: {
        ...process.env,
        HARNESSOS_TEST_PROVIDER_SECRET: "must-not-cross-child-boundary",
...
test(
  "GitHub clone timeout force-kills the SIGTERM-resistant process group",
  { skip: process.platform === "win32" },
  async () => {
    ...
    try {
      assert.equal(
        ...
        true,
        `clone process ${rootPid} survived SIGKILL fallback`,
      );
      assert.equal(
        await waitForProcessExit(helperPid),
        true,
        `clone helper ${helperPid} survived SIGKILL fallback`,
      );
    } finally {
