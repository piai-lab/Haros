...
test("Settings projection uses auto-summary when workflow is absent", async () => {
  const agentDir = await mkdtemp(join(tmpdir(), "harnessos-web-settings-default-workflow-"));
  await writeFile(
  ...
  const projection = projectWebSearchSettings(
    createWebSearchConfigService(agentDir).readSnapshot(),
  );
...
test("Settings projection returns literal expressions and credential-blind runtime descriptors", async () => {
  const agentDir = await mkdtemp(join(tmpdir(), "harnessos-web-settings-projection-"));
  await writeFile(
    join(agentDir, "web-search.json"),
    JSON.stringify({
      schemaVersion: 1,
  ...
  assert.equal(
    tavily.fields.find(({ configKey }) => configKey === "tavilyApiKey").value,
    "$TAVILY_API_KEY",
  );
  assert.equal(
    exa.fields.find(({ configKey }) => configKey === "exaApiKey").value,
    "!security find-generic-password -w -s synthetic",
  );
...
test("Provider descriptors own structural possibility including Gemini browser cookies", async () => {
  const agentDir = await mkdtemp(join(tmpdir(), "harnessos-web-settings-structural-"));
  await writeFile(
    join(agentDir, "web-search.json"),
    JSON.stringify({
      schemaVersion: 1,
...
test("Provider configuration state requires every descriptor-owned prerequisite", async () => {
  const agentDir = await mkdtemp(join(tmpdir(), "harnessos-web-settings-prerequisites-"));
...
test("Provider configuration projection preserves alternative credential and session paths", async () => {
  await withoutProviderEnvironment(
    ...
    async () => {
      const agentDir = await mkdtemp(join(tmpdir(), "harnessos-web-settings-alternatives-"));
      const configPath = join(agentDir, "web-search.json");
      ...
      assert.deepEqual(
        [
          state(projection, "openai").configurationState,
          state(projection, "xai").configurationState,
        ],
...
test("Provider Settings grouping follows descriptor connection roles", async () => {
  const agentDir = await mkdtemp(join(tmpdir(), "harnessos-web-settings-groups-"));
...
test("Settings capability status follows the selected route's structural prerequisites", async () => {
  await withoutProviderEnvironment(
    ...
    async () => {
      const agentDir = await mkdtemp(join(tmpdir(), "harnessos-web-settings-route-status-"));
      const configPath = join(agentDir, "web-search.json");
      const project = async (config) => {
        await writeFile(
          configPath,
          JSON.stringify({ schemaVersion: 1, workflow: "auto-summary", ...config }) + "\n",
          { mode: 0o600 },
        );
        return projectWebSearchSettings(createWebSearchConfigService(agentDir).readSnapshot());
      };
      ...
      assert.equal(
        (await project({ provider: "tavily", tavilyBaseUrl: "https://example.invalid" }))
          .capabilityStatus,
        "needs-configuration",
      );
      assert.equal(
        (await project({ provider: "tavily", tavilyApiKey: "synthetic" })).capabilityStatus,
        "possible",
      );
      ...
      assert.equal(
        (await project({ provider: ["tavily"] })).capabilityStatus,
        "needs-configuration",
      );
      assert.equal((await project({ provider: "all" })).capabilityStatus, "possible");
      assert.equal(
        (await project({ provider: "auto", webSearch: { enabled: false } })).capabilityStatus,
        "file-disabled",
      );
...
test("Settings projects the same file-level tool policy used by registration", async () => {
  const agentDir = await mkdtemp(join(tmpdir(), "harnessos-web-settings-tools-"));
  const configPath = join(agentDir, "web-search.json");
  await writeFile(
    configPath,
    JSON.stringify({
...
test("closed Settings mutation preserves unknown file-owned fields", async () => {
  const agentDir = await mkdtemp(join(tmpdir(), "harnessos-web-settings-mutation-"));
  const configPath = join(agentDir, "web-search.json");
  await writeFile(
    configPath,
    JSON.stringify({
...
test("Provider draft test uses unsaved candidate through the formal runtime without writing", async () => {
  const agentDir = await mkdtemp(join(tmpdir(), "harnessos-web-settings-test-"));
  const service = createWebSearchConfigService(agentDir);
...
test("Provider draft test projects quota and network failures without exposing diagnostics", async () => {
  const agentDir = await mkdtemp(join(tmpdir(), "harnessos-web-settings-test-errors-"));
  const service = createWebSearchConfigService(agentDir);
  service.ensureDefault();
  const originalFetch = globalThis.fetch;
  try {
