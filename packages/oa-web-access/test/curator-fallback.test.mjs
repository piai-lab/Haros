...
test("curator fallback helper is visible to the browser-open catch block", () => {
	...
	const tryOffset = indexSrc
		.slice(declarationIndex)
		.search(/try\s*\{\s*pc\.phase = "curating";/);
	const tryIndex = tryOffset < 0 ? -1 : declarationIndex + tryOffset;
	const catchCallOffset = indexSrc.slice(tryIndex).search(
		/sendCuratorFallbackUpdate\(\s*"Search curator is running, but the browser did not open automatically\."\s*,?\s*\)/,
	);
	const catchCallIndex = catchCallOffset < 0 ? -1 : tryIndex + catchCallOffset;
