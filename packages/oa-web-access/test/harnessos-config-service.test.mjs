...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), "harnessos-web-config-"));
...
test("damaged and future schemas preserve original bytes and fail closed", async () => {
  ...
  assert.throws(
    ...
    (error) => error instanceof WebSearchConfigError && error.kind === "damaged-json",
  );
  ...
  assert.throws(
    () => service.readSnapshot(),
    (error) => error instanceof WebSearchConfigError && error.kind === "future-schema",
  );
