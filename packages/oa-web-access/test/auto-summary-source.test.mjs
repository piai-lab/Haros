...
test("web_search accepts auto-summary workflow in schema and config resolution", () => {
  ...
  assert.match(indexSrc, /if \(profile === "oa"\) return "auto-summary"/);
  assert.match(indexSrc, /return hasUI \? "summary-review" : "none"/);
  assert.match(
    indexSrc,
    /arg === "none" \|\| arg === "summary-review" \|\| arg === "auto-summary"/,
  );
});
...
test("auto-summary skips curator and reuses summary model fallback plumbing", () => {
  ...
  assert.match(
    indexSrc,
    /await generateSummaryDraft\(\s*searchResults,\s*summaryContext,\s*signal,\s*summaryModelChoices\.defaultSummaryModel \?\? undefined,\s*undefined,\s*undefined,\s*getSummaryGenerationDeadlineMs\(\),\s*\)/,
  );
