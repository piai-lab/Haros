...
function buildProviderLabelResolver(): string {
  return getSearchProviderPresentation()
    .map(
      (provider) =>
        `    if (provider === ${JSON.stringify(provider.id)}) return ${JSON.stringify(provider.curatorLabel)};`,
    )
    .join("\n");
}
...
function buildProviderButtons(
  ...
  return providers
    ...
    .map((p) => {
      ...
      const descriptor = getSearchProviderPresentation().find(
        (provider) => provider.id === p.value,
      );
      const icon =
        descriptor?.icon.kind === "local-asset"
...
export function generateCuratorPage(
  ...
  const providerPresentation = Object.fromEntries(
    providerProjection.map((provider) => [
      provider.id,
      {
        label: provider.curatorLabel,
        iconSrc:
  ...
  const inlineScript = SCRIPT.replace("__INLINE_DATA__", () => inlineData)
