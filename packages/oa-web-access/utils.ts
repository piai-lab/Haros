...
const API_REQUEST_BODY_HEADERS = [
  "Content-Encoding",
  "Content-Language",
  "Content-Location",
  "Content-Type",
...
const LOCAL_CURATOR_NETWORK_DEFAULTS: CuratorNetworkConfig = {
  enabled: false,
  host: "localhost",
  bind: "127.0.0.1",
};
...
export function resolveCuratorNetworkConfig(): CuratorNetworkConfig {
  if (currentWebAccessContext()?.profile === "oa") {
...
async function fetchViaCurlOnce(url: URL, init: RequestInit, proxyUrl: string): Promise<Response> {
  ...
  if (status === 0) {
    throw new Error(
      `Proxy fetch to ${url.toString()} via ${redactProxyUrl(proxyUrl)} returned no HTTP status`,
    );
  }
