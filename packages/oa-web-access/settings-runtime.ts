...
import { createWebAccessInstanceContext, runWithWebAccessContext } from "./runtime-context.ts";
...
const knownConfigKeys = new Set([
  ...
  "workflow",
  "autoOpenBrowser",
  ...getSearchProviderPresentation().flatMap(({ fields }) =>
    fields.map(({ configKey }) => configKey),
  ),
]);
...
function validatedDraftPatch(draft: WebSearchSettingsDraft): {
  ...
  for (const field of draft.fields) {
    if (
      !knownConfigKeys.has(field.configKey) ||
      field.configKey === "provider" ||
      field.configKey === "workflow" ||
      field.configKey === "autoOpenBrowser"
    ) {
      throw new Error(`Unknown Web search setting: ${field.configKey}`);
    }
    if (seen.has(field.configKey))
      throw new Error(`Duplicate Web search setting: ${field.configKey}`);
