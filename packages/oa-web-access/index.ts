...
export const OA_WEB_ACCESS_PLAN_TOOL_NAMES = Object.freeze(Object.values(OA_WEB_ACCESS_TOOL_NAMES));
const DEFAULT_TOOL_NAMES: ToolNames = { ...OA_WEB_ACCESS_TOOL_NAMES };
...
function registerWebAccessExtension(pi: ExtensionAPI) {
  const harnessosProfile = currentWebAccessContext()?.profile === "oa";
  const availability = harnessosProfile
    ? new WebSearchSessionAvailability(pi, currentWebSearchConfigService(), isOAWebAccessTool)
    : undefined;
  const initConfig = loadConfigForExtensionInit();
  installGlobalProxyFetch();
  const toolNames = harnessosProfile ? DEFAULT_TOOL_NAMES : resolveToolNames(initConfig);
  ...
  const searchErrorMessage = (error: unknown): string => {
    ...
    if (!harnessosProfile) return raw;
  ...
  async function openCuratorBrowser(
    ...
    try {
            ...
            if (reason === "timeout") {
              ...
              pc.finish(
                buildCurationCancelledReturn(reason, {
                  ...
                  ...(harnessosProfile ? {} : { curatorUrl: pc.curatorUrl }),
                  browserOpenError: pc.browserOpenError,
                }),
              );
            }
      ...
      if (harnessosProfile && presenter && pc.surfaceId) {
        let result: Awaited<ReturnType<CuratorPresenter["present"]>>;
        try {
          result = await presenter.present({
            toolCallId: callId,
      ...
      if (harnessosProfile && pendingCurates.get(callId) === pc) {
        pc.finish({
          content: [
            {
              type: "text",
              ...
              if (curator && !cancelled) {
                ...
                if (pc.browserOpenError) {
                  pc.onUpdate?.({
                    ...
                    details: {
                      phase: harnessosProfile ? "curating" : "curator-fallback",
                      progress: 1,
                      ...(harnessosProfile && pc.surfaceId
                        ? { engineWebSurface: { surfaceId: pc.surfaceId, status: "pending" } }
                        : { curatorUrl: pc.curatorUrl }),
  ...
  if (!harnessosProfile && isCommandEnabled(initConfig, "websearch"))
  ...
  if (!harnessosProfile && isCommandEnabled(initConfig, "curator"))
  ...
  if (!harnessosProfile && isCommandEnabled(initConfig, "google-account"))
  ...
  if (!harnessosProfile && isCommandEnabled(initConfig, "search"))
...
export interface OAWebAccessExtensionOptions {
...
export const OA_WEB_ACCESS_EXTENSION_NAME = "harnessos-web-access";
export const OA_WEB_ACCESS_EXTENSION_PATH = `<inline:${OA_WEB_ACCESS_EXTENSION_NAME}>`;

export function isOAWebAccessTool(tool: ToolInfo | undefined): boolean {
  return (
    tool?.sourceInfo.path === OA_WEB_ACCESS_EXTENSION_PATH &&
...
export interface OAWebAccessRegistrationInspection {
...
export function inspectOAWebAccessRegistration(
  tools: readonly ToolInfo[],
): OAWebAccessRegistrationInspection {
  const names = Object.values(DEFAULT_TOOL_NAMES);
  const deliveredToolNames = names.filter((name) =>
    isOAWebAccessTool(tools.find((tool) => tool.name === name)),
  );
...
export function makeOAWebAccessExtension(
  options: OAWebAccessExtensionOptions,
...
export function makeOAWebAccessInlineExtension(
  options: OAWebAccessExtensionOptions,
): InlineExtension {
  return {
    name: OA_WEB_ACCESS_EXTENSION_NAME,
    hidden: true,
    factory: makeOAWebAccessExtension(options),
  };
}
