...
This directory is HarnessOS's subtractive, modified Ask User runtime fork. The
package itself remains Host-neutral and side-effect free; HarnessOS Server owns
...
The subsequent HarnessOS product profile keeps the same upstream controller
...
multi-choice custom input coexists with presets. HarnessOS deliberately deletes
...
mistaking the donor questionnaire contract for an HarnessOS Product contract.
...
The replacement `src/product.ts` and `src/tool.ts` are explicit HarnessOS product
