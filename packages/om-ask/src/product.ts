...
export function validateAskUserToolInput(input: AskUserToolInput): AskUserToolInput {
  ...
  if (byteLength(input) > ASK_USER_MAX_UTF8_BYTES) {
    throw new AskUserProductValidationError(
      "ask_user request exceeds the 1 MiB UTF-8 safety guard.",
    );
  }
  ...
  for (const question of input.questions) {
    ...
    if (!nonBlank(question.id) || !nonBlank(question.prompt)) {
      throw new AskUserProductValidationError(
        "Every ask_user question requires a non-empty id and prompt.",
      );
    }
    ...
    if (question.type === "text") {
      if (question.suggestion !== undefined && !nonBlank(question.suggestion.text)) {
        throw new AskUserProductValidationError(
          `Text question "${question.id}" has an empty suggestion.`,
        );
      }
      continue;
    }
    if (
      ...
      throw new AskUserProductValidationError(
        `Choice question "${question.id}" requires authored options.`,
      );
    }
    ...
    for (const option of question.options) {
      ...
      if (!nonBlank(option.value) || !nonBlank(option.label)) {
        throw new AskUserProductValidationError(
          `Choice question "${question.id}" has an empty option value or label.`,
        );
      }
      if (option.value === ASK_USER_RESERVED_CUSTOM_VALUE) {
        throw new AskUserProductValidationError(
          `Option value "${ASK_USER_RESERVED_CUSTOM_VALUE}" is reserved by OmniMind.`,
        );
      }
      if (values.has(option.value) || labels.has(option.label)) {
        throw new AskUserProductValidationError(
          `Choice question "${question.id}" has duplicate option values or labels.`,
        );
      }
  ...
  if (nodes > ASK_USER_MAX_NODES) {
    throw new AskUserProductValidationError(
      "ask_user request exceeds the 10,000-node safety guard.",
    );
  }
  return input;
}

export function validateAskUserResult(
  request: AskUserToolInput,
  result: AskUserResult,
): AskUserResult {
  if (result.version !== ASK_USER_CONTRACT_VERSION || !nonBlank(result.requestId)) {
    throw new AskUserProductValidationError(
      "ask_user interaction returned an invalid result envelope.",
    );
  }
  if (result.status !== "answered") {
    if (result.answers !== undefined) {
      throw new AskUserProductValidationError(
        `${result.status} ask_user result cannot carry answers.`,
      );
    }
    return result;
  }
  if (!Array.isArray(result.answers) || result.answers.length !== request.questions.length) {
    throw new AskUserProductValidationError(
      "answered ask_user result must contain exactly one answer per question.",
    );
  }
  ...
  for (const answer of result.answers) {
    ...
    if (!question || seen.has(answer.questionId)) {
      throw new AskUserProductValidationError(
        "ask_user result contains an unknown or duplicate question id.",
      );
    }
    seen.add(answer.questionId);
    if (!Array.isArray(answer.selectedValues)) {
      throw new AskUserProductValidationError(
        `Answer "${answer.questionId}" has invalid selectedValues.`,
      );
    }
    if (answer.customText !== undefined && !nonBlank(answer.customText)) {
      throw new AskUserProductValidationError(
    ...
    if (new Set(answer.selectedValues).size !== answer.selectedValues.length) {
      throw new AskUserProductValidationError(
        `Answer "${answer.questionId}" has duplicate selected values.`,
      );
    }
    if (answer.selectedValues.some((value) => !allowed.has(value))) {
      throw new AskUserProductValidationError(
        `Answer "${answer.questionId}" has an unknown selected value.`,
      );
    }
    if (!question.multi && answer.selectedValues.length > 1) {
      throw new AskUserProductValidationError(
        `Single-choice answer "${answer.questionId}" has multiple selected values.`,
      );
    }
    ...
    if (answer.selectedValues.length === 0 && answer.customText === undefined) {
      throw new AskUserProductValidationError(
        `Answer "${answer.questionId}" does not contain an answer.`,
      );
    }
  }
  if (byteLength(result) > ASK_USER_MAX_UTF8_BYTES) {
    throw new AskUserProductValidationError(
      "ask_user result exceeds the 1 MiB UTF-8 safety guard.",
    );
  }
  return result;
}
