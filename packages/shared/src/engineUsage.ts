...
export function engineUsageLabel(engine: string | null | undefined): string {
  const meta = lookupMeta(engine);
  return meta ? `${meta.displayName} usage` : "Usage";
}

export function engineUsageDisplayName(engine: string | null | undefined): string {
  return lookupMeta(engine)?.displayName ?? "Engine";
}

export function engineUsageLearnMoreHref(engine: string | null | undefined): string | null {
...
export function engineUsageNeedsAuthDetail(engine: string | null | undefined): string {
  const meta = lookupMeta(engine);
  if (!meta) {
