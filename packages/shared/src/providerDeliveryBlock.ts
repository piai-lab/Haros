...
// Exports: ENGINE_DELIVERY_BLOCK_SUMMARY, formatProviderDeliveryBlockDetail, isProviderDeliveryBlockDetail
...
export function formatProviderDeliveryBlockDetail(blockerDetail: string): string {
  return `${ENGINE_DELIVERY_BLOCK_SUMMARY}: ${blockerDetail}`;
}
...
export function isProviderDeliveryBlockDetail(detail: string | null | undefined): boolean {
  return typeof detail === "string" && detail.trimStart().startsWith(ENGINE_DELIVERY_BLOCK_SUMMARY);
}
