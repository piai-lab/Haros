...
export type BrowserCookieImportResult =
  ...
  | {
      ...
      code:
        ...
        | "transfer_failed"
        | "persistence_failed";
      mayHaveImported?: boolean;
      platform: "macos" | "windows" | "linux";
      stage?: "acquisition" | "parse" | "decrypt" | "decode" | "query" | "discovery";
    };

export type BrowserCookieImportStatus = {
  operationId: string;
  phase: "checking" | "waiting" | "importing" | "persisting" | "stopping";
};

export interface BrowserVaultMethods {
  snapshot(): Promise<BrowserVaultSnapshot>;
  listLogins(): Promise<BrowserVaultLogin[]>;
  retryCapture(): Promise<void>;
  configure(settings: BrowserVaultSettings): Promise<void>;
  ...
  cookieImportStatus(): Promise<BrowserCookieImportStatus | null>;
  cancelCookieImport(operationId: string): Promise<void>;
  onChanged(listener: (change: { logins: boolean }) => void): () => void;
}

/** Safe, stable IPC failure codes; raw upstream exceptions never cross this boundary. */
export const BROWSER_VAULT_ERROR_CODES = [
  "wrong_password",
  "locked",
  "os_unavailable",
  "storage_failed",
...
export type BrowserVaultErrorCode = (typeof BROWSER_VAULT_ERROR_CODES)[number];
export class BrowserVaultError extends Error {
  constructor(readonly code: BrowserVaultErrorCode) {
    super(`HAROS_VAULT:${code}`);
  }
}
export function browserVaultErrorCode(error: unknown): BrowserVaultErrorCode {
  const message = error instanceof Error ? error.message : "";
  return (
    BROWSER_VAULT_ERROR_CODES.find((code) => message.endsWith(`HAROS_VAULT:${code}`)) ??
    "unavailable"
  );
}
