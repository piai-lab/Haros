...
describe("Haros model-services contracts", () => {
  ...
  it("decodes the credential-blind list projection", () => {
    const decoded = Schema.decodeUnknownSync(HarosModelServicesListResult)({
      state: "ready",
      services: [descriptor],
      connectableServices: [],
      errorCode: null,
  ...
  it("only admits the explicit add-service intent and a bounded extension outcome", () => {
    ...
    expect(
      Schema.decodeUnknownSync(HarosModelServicesGetInput)({
        serviceId: "extension-service",
        intent: "add_service",
      }),
    ).toEqual({ serviceId: "extension-service", intent: "add_service" });
    expect(() =>
      Schema.decodeUnknownSync(HarosModelServicesListInput)({ intent: "settings_open" }),
    ).toThrow();
  ...
  it("decodes bounded public model facts without accepting private runtime fields", () => {
    const decoded = Schema.decodeUnknownSync(HarosModelServicesGetResult)({
      state: "ready",
      service: descriptor,
      models: [
        {
  ...
  it("admits only write-only environment header mutations and public metadata", () => {
    ...
    const projected = Schema.decodeUnknownSync(HarosModelServicesGetResult)({
      state: "ready",
      service: { ...descriptor, serviceId: "custom", providerId: "custom", origin: "models_json" },
      models: [],
      customConfig: {
  ...
  it("rejects path-shaped get inputs and invalid counts", () => {
    ...
    expect(Schema.decodeUnknownSync(HarosModelServicesGetInput)({ serviceId: "小米代理" })).toEqual(
      {
        serviceId: "小米代理",
      },
    );
    for (const serviceId of ["..", "provider/child", "provider\\child", "provider\u202esecret"]) {
      expect(() => Schema.decodeUnknownSync(HarosModelServicesGetInput)({ serviceId })).toThrow();
    }
    expect(() =>
  ...
  it("rejects path, URL, control, and bidi-shaped display labels", () => {
    for (const displayName of [
      ...
      expect(() =>
        Schema.decodeUnknownSync(HarosModelServiceDescriptor)({ ...descriptor, displayName }),
      ).toThrow();
    }
  });
  ...
  it("rejects contradictory projection and catalog error states", () => {
    expect(() =>
      Schema.decodeUnknownSync(HarosModelServicesListResult)({
        state: "error",
        services: [],
        connectableServices: [],
        errorCode: null,
  ...
  it("strips secret, endpoint, header, and path fields from encoded output", () => {
    const encoded = Schema.encodeSync(HarosModelServiceDescriptor)({
      ...descriptor,
      apiKey: "sk-secret",
  ...
  it("decodes typed auth prompts while keeping credentials out of results", () => {
    ...
    expect(
      Schema.decodeUnknownSync(HarosModelServiceBeginLoginInput)({
        serviceId: "deepseek",
        authType: "api_key",
      }),
    ...
    expect(
      Schema.decodeUnknownSync(HarosModelServicePollLoginInput)({
        requestId,
        afterEventCount: 1,
    ...
    expect(
      Schema.decodeUnknownSync(HarosModelServiceAnswerLoginInput)({
        requestId,
        promptId,
        value: "test-secret",
      }),
    ).toEqual({ requestId, promptId, value: "test-secret" });
    const result = Schema.encodeSync(HarosModelServiceAuthResult)({
      state: "complete",
      requestId,
      service: descriptor,
  ...
  it("admits only an explicit bounded saved-key reveal result", () => {
    expect(
      Schema.decodeUnknownSync(HarosModelServiceRevealApiKeyInput)({
        serviceId: "deepseek",
      }),
    ).toEqual({ serviceId: "deepseek" });
    expect(
      Schema.decodeUnknownSync(HarosModelServiceRevealApiKeyResult)({
        state: "ready",
