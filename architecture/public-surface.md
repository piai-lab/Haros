...
| Update discovery | 不是 public-site route；使用独立 signed channel/feed   | Desktop update check、download、install 与 rollback      | app ↔ update feed                    | app identity、platform、arch、current version、channel；不含用户内容或 credential                                       | 签名与 artifact identity、channel/feed、manifest、install/rollback 和平台 proof                                                                                  | 不检查猜测 feed；应用继续本地运行并提供诊断                                           | Release/update authority                   |
