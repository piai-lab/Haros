...
Thread 的 `default / plan / debug / converge / learn` 由 Product State 的单一 `interactionMode` 持久化，并由 Composer、PlanSidebar、Refine/Implement 与 SQLite 投影共同消费；五种模式互斥，不是独立 Agent，也不建立第二 Session、Plan store、Prompt Library 或偏好设置。Server 的同一 `ProviderExecutionStructure` 同时拥有 `supportedInteractionModes` 与 runtime-mode 结构真相，capability projection 叠加当前 health/auth/version，`sendTurn/steerTurn` 在最终 adapter dispatch 前再次 admission。缺省值是 `default`；不支持或尚未确认时 fail closed，不 silent fallback。Composer 只在 Plan 可执行时提供进入动作；已经保存的 Plan intent 遇到 Engine 切换或健康漂移时保持原值、禁止 Plan send并保留显式退出。切换 interaction mode 不替换 native Session，只影响切换后接纳的新 dispatch。

`debug`、`converge` 与 `learn` 是 Host-owned persistent prompt policies，不改变工具、文件、runtime mode 或 Host 权限，也不是执行安全边界。Debug 保留既有 prompt owner；ProviderCommandReactor 只在现有 dispatch 组合路径中为 Converge/Learn 投影固定正文，同一 turn 的 Tool loop 与 canonical Ask answer continuation不新增投影。正文不进入 canonical user message、Timeline、导出或反馈；Engine private Session 是否保留已发送内容沿用其原生行为。切回 `default` 或进入 `plan` 后不再投影这些 Host cognitive prompts，不为此创建 reset marker、Prompt Cache、last-mode state 或 adapter-specific lifecycle。

Converge 要求模型先调查可确认事实；在输出实质分析、建议、brief、计划或执行前，若仍有会实质改变答案、验收或路径的用户选择，必须先调用当前 Engine 暴露的 structured Ask User（OmniMind Agent 中 exact Tool 为 `ask_user`）并等待回答，不能用推测用户真实目标、普通文字选项、表格或模型自己的推荐替代用户决定。只有不存在这类选择时，事实查询和不会因合理解释差异而改变结果的只读说明才可直接回答。确认前不实施，且模型无权自行退出模式。仅当当前 Engine 没有 structured Ask User 或 canonical Tool 确实不可用时，模型才可在普通 Assistant 回复中给出同一问题的清晰 `A / B / C` 选项并允许自定义答案，随后立即结束回复等待下一条用户消息；不得先给实质答案。这只是普通对话，不创建、伪造、resolve 或降级 canonical User Input request，原 Ask Tool 的注册、lease 与 terminal lifecycle 仍按上文 fail closed。Learn 采用 answer-first、example-first、adult-respect 的持续教学，并按真实结构使用现有 Mermaid/ASCII/table 表达，不建立测验门、学习档案或掌握度状态。两种模式都由用户通过 Composer 明确退出；一次发送、回答或 Ask settlement 不会自动清除。
...
`@harnessos/oa-web-access`是bundled OmniMind Agent专用的Pi-native Extension，不是第七组Host capability，也不进入AgentGateway或跨Engine投影。它保留自己的source、maintenance、configuration、Provider routing、Tool、Curator与Session cleanup责任；Pi `ResourceLoader`、Tool Registry / active set仍唯一拥有registered/active与Session lifecycle。产品不安装、注册、测试或承诺该fork对stock Pi可用，stock Pi继续使用上游package及其原生生命周期。
...
Web Access的runtime Provider定义是服务stable ID、参与`auto/broad/all/explicit-only`、配置前提、连接角色与routing eligibility的唯一来源。`auto`沿canonical候选首个成功即停止；`broad`从高级`searchRouting`明确顺序、其他已配置且auto-eligible服务、免费/Session/零配置服务依次补位，按真实可用性去重后每个query最多并发三家；explicit-only服务只有被高级route或显式数组指定时参加；`all`继续并发全部eligible服务。`broad`至少一家成功即返回带逐Provider结果与局部失败的聚合，全部失败才terminal，候选不足时诚实退化且绝不升级为`all`。供Settings、Curator、Timeline和技术详情使用的presentation字段必须与这些exact定义同源并由Server做credential-blind投影，不能在Web、AgentGateway或另一个manifest中平行枚举服务；presentation字段缺失只触发中性UI fallback，绝不能改变路由、可用性或active set。新增、删除或重命名上游Provider时，同一fork revision必须同时闭合runtime定义、presentation覆盖、品牌资产准入和作者/OmniMind回归，不能让UI冻结在某个历史“26家”快照。
...
Web Access的typed搜索页面有两种产品语义：`summary-review`是pending tool call的短时交互面；`auto-summary/none`在用户显式开启自动展示时只是非阻塞观察面。二者第一次呈现都通过Browser owner内部typed seam创建独立ephemeral Tab，绝不能复用并导航用户当前Tab；只有review的pending handle进入Timeline waiting/reopen合同。页面保持上游的开放文档流：结果卡片、补充搜索、尾部summary、footer依次排列；review尾部保留生成、编辑、重生成、预览和批准，observer在`auto-summary`终态接收同一份已发送给Agent的summary并只读显示，不出现settlement控件，`none`不显示summary。观察面terminal时立即释放server/stream/timer/token与presentation handle，已加载页面保留静态结果与尾部摘要/结果状态，但Timeline不得留下reopen，App/Thread reopen不得恢复过期协议。只有owning Thread位于前台时才自动呈现；后台review只使用既有waiting-for-user activity/attention，后台observer不切route、不抢Right Dock。这个内部seam不得扩张Agent可见`browser_open` schema，也不得建立Curator tab store、Product fact或第二Browser lifecycle。
