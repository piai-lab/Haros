...
OmniMind Agent 全局提示词设置只是在既有 `.omnimind/agent` owner 上增加安全文件投影与 mutation seam。Host 必须复用 bundled runtime 的公开 discovery 来确定五个 global context candidate 中当前实际选中的 source，并继续让该 runtime 唯一拥有 precedence、`SYSTEM.md` / `APPEND_SYSTEM.md` 组合、Project shadow、Session resource snapshot 与 reload；Host 不复制选择或组合算法，不做每轮动态注入，不建立 Prompt registry/profile/history/cache、第二 loader 或跨 Engine 同步。固定候选 allowlist 只用于 typed contract、安全校验和展示，不能反向成为 precedence authority。

Renderer 只提交 resource kind、opaque source id、expected version 与 create/update/remove intent；绝对路径或 `displayPath` 不能成为写入 authority。Server 从唯一 `resolveOmniMindAgentDir` 根解析目标，对候选和固定 system 文件执行 containment、symlink/regular-file/identity、UTF-8、bounded-size 与并发版本检查，再以现有 atomic-write owner 完成 mutation。读取 snapshot 不创建文件；no-op 不写、不 reload。OmniMind 进程内 mutation 串行化，update/remove 在 commit 前重新检查 source、identity 与 expected version；这是对非协作外部 editor 的乐观冲突检测，不是严格的跨进程 compare-and-replace/remove。Node 公开 `fs` 没有把 inode/version 条件与 `rename` 或 `unlink` 组成单个原子操作的 seam，最终检查与 commit 之间仍有极窄 TOCTOU 窗口；不得以 native addon、第二 writer、协作锁或 rollback 子系统填补。首次 create 继续由 shared atomic writer 的 link/`EEXIST` no-clobber 语义保证不覆盖 raced target。若未来 Node 或既有 owner 暴露真实 CAS，只在保持既有调用方行为不变时重新评估。

文件 mutation 与 active Session 是不同 authority。保存成功不热切当前 Session；显式 reload 只复用当前 `omnimindEcosystem.reload({ threadId })` exact-thread seam，busy 时拒绝且不取消 in-flight operation。reload 成功只保证该 Session 的后续安全请求使用重新加载后的资源；失败不回滚磁盘，也不承诺旧 runner 完整回滚。当前 operation 在 admission 时已经冻结的 system prompt、messages 与 tools 不因同时发生的 save/reload 改变。无文件、Settings visit/cancel、absent-empty 与 no-op 必须保持 request bytes、system-prompt digest、reload count 与原生 cache 可用性稳定。
...
- 只有canonical `provider === "omnimind"`的OmniMind Agent让AgentGateway Host tools作为标准Pi Extension tools参与Pi-native Dynamic Tool Loading：新Session先按Built-in policy与平台/服务可用性过滤，由named、hidden、session-scoped inline Extension注册允许的definitions。Pi `AgentSession`拥有registered/all/active truth、active-set mutation、reload/session lifecycle与Provider native/fallback编码；Host Extension只注册自己拥有的Gateway tools，并在exact runtime需要callable入口时提供一个无持久状态的极薄extension-local loader，请求纯additive activation。
- 允许且可用的Gateway definitions全部注册。Session启动时，Host Extension只把自己拥有且可搜索的Host names设为inactive，并让一个极薄loader在整个Session保持active；不凭感觉预留固定Host core。此后动态加载在同一Session内保持monotonic/additive：loader命中或canonical prompt/envelope要求某个exact有界依赖closure时，只请求把缺失的owned definitions加入active set，已经active的definitions继续由Pi Session持有，直到Pi原生reload/new Session/session replacement，或确有安全/政策边界要求通过这些原生边界收缩。
- Host Extension的发现、匹配和activation只能针对自己的immutable AgentGateway name set，不得为任何非owned Pi tool source建立inventory、分类、保留规则或控制面。exact Pi setter若要求完整active list，只能把Pi当前集合当作opaque base原样union owned additions，不能据此识别、重算、移除或“保留”非owned names。Host不建立per-turn active controller、lifecycle active store、dependency registry或第二active truth；loader名称、ranking、limit和算法也不是架构合同。
- stock Pi虽共享Pi-family实现，产品身份仍是非OmniMind Agent，继续通过现有Pi `customTools` seam直接/eager获得Built-in policy允许的Host tools；Codex、Claude、OpenCode等其他Engine继续使用其真实支持的native MCP或等价adapter seam。实现只按canonical Provider身份做窄分支，不能把OmniMind Agent的Pi Registry/active-set或Provider wire语义强加给其他Engine。
- 两条投影共享同一AgentGateway definition与call owner，但不共享Engine私有配置、Package、Session、registry或credential lifecycle。Gateway内部duplicate必须拒绝；cross-source同名只使该Host ownership claim、collided capability及依赖它的dispatch fail closed，不能把第三方winner当成Host tool。Pi-native第三方winner与Session其余能力默认继续并准确diagnostic；只有exact Provider contract不能隔离局部冲突时，才把Session标为unavailable，不silent override、drop或rename。

Pi-native extension-local loader只是exact `0.84.2`下可能需要的可替换实现细节，不是OmniMind稳定子系统、产品工具或全局工具搜索。它只做轻量发现与纯additive `setActiveTools`，不代理执行、不返回完整schemas、不连接server、不建立索引或active store。若Goal、Automation等canonical prompt/envelope直接点名Gateway tools，同一request发送前只需确保exact有界依赖closure已经active；若仍inactive，就做一次owned-set additive activation。职责结束只停止对应prompt、duty或loop，不卸载已经active的schema。完整schema由Pi按exact Provider-native deferred representation或安全fallback进入真实工具面；最终执行继续回到AgentGateway `tools/call`。registered、active、prompt-required与authorized是不同事实：每次真实调用仍按当前Built-in policy、session identity、credential、availability、runtime mode/permission、真实存在的approval、exact turn authority、timeout与cancellation重新判定。

Built-in关闭某组后，新OmniMind Session不注册该组；旧Session的loader按live policy不再发现或激活它，stale call由Gateway立即拒绝。若当前Goal、Automation或其他canonical lifecycle所需的exact closure因此不完整，不能绕过policy自动active或继续loop并伪造完成；必须阻止或暂停对应dispatch/continuation并准确投影unavailable。旧Session中已经active的schema不承诺在下一turn自行移除：exact Pi能通过安全reload/session replacement收缩时沿用原生边界，否则schema可以暂时可见但不可执行，并准确诊断，不为Settings toggle新增per-turn removal controller。已准入且执行中的调用不因普通exposure toggle被伪装成emergency kill，取消仍归turn/session owner。重新开启不把创建时未注册的schema偷偷注入旧Session，只按Pi真实reload/new-session边界生效；reload、resume与fork继续由Pi Session承担active truth，不建第二active store，旧Extension实例与handler也不得继续生效。

Prompt与active tool surface必须在同一request一致：任何发送给模型的prompt或envelope都不得直接点名inactive tool。OmniMind Agent的generic Host guidance只说明额外Host能力可按需发现和加载、需要时使用当前active loader、激活后在下一安全turn调用并且不要猜工具名；不得枚举inactive Browser、Device、Thread、Automation工具，也不得把全catalog或长说明换一个位置重新注入。exact Goal、Automation等canonical lifecycle prompt可以点名其职责所需tools，但同一request发送前对应exact有界closure必须已经active；tool-specific duty不得反向进入全局prompt。stock Pi和其他Engine继续获得与其完整filtered schema一致的直接工具指导。
