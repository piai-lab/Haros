...
Ask User 最终只允许一个 product-owned、bundled Pi-native `ask_user` Tool；多题、自由回答、Preview、Notes 与 Review 都是同一 request/result 的能力，不增加 Tool。其 fork 只拥有 Tool definition、validation、structured result 与窄 interaction port，canonical request/result、Host capability、状态 settlement、跨 Engine projection 与 Composer UI 仍由 OmniMind owner 拥有。Tool 在 registration 前必须断言 same-name winner 的 source/instance provenance；foreign winner、无 canonical UI、reload 漂移或缺失 lifecycle barrier 时 fail closed，不改名注册第二 Ask，也不以普通文本追问降级。Ask 与 Approval 始终分离，User Input answer 不改变 runtimeMode 或授权。

Pi 的 `executionMode: sequential` 只是必要条件，不是充分屏障。含 canonical Ask 的模型同批调用必须由 composition seam 在任何 sibling 副作用前识别并执行 Ask-first，Ask terminal 后终止同批其余 siblings，把结构化回答作为真实 Tool result 进入新的模型 continuation，由模型重新规划；不得按原 batch 自动续跑。每个 waiting request 以 Session generation、run、toolCall、request identity fencing，submit/cancel/abort/timeout/unavailable 只能 settle 一次，late/double/foreign answer 拒绝。App/Server 退出或 Session replacement 后旧 Tool Promise 不可恢复，Product State 只能标 stale；新 Session 必须创建新 request。上述 runtime、provenance、no-UI 与 restart gates 未闭合前，即使 canonical contract/UI 已存在，也不得向模型注册或公开 Tool schema。
