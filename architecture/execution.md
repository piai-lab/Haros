...
Ask User只允许一个product-owned、bundled Pi-native `ask_user` Tool；多题、自由回答、Preview、Notes与Review都是同一request/result的能力，不增加Tool。其fork只拥有Tool definition、validation、structured result与窄interaction port，canonical request/result、Host capability、状态settlement、跨Engine projection与Composer UI仍由OmniMind owner拥有。Session只在compatible presenter存在时composition该Extension；registration/reload枚举全部同名Extension并断言唯一bundled source provenance。foreign winner、重复loser、无canonical UI、reload漂移或缺失lifecycle barrier时fail closed，不改名注册第二Ask，也不以普通文本追问降级。Ask与Approval始终分离，User Input answer不改变runtimeMode或授权。
