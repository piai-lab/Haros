...
Converge 要求模型先调查可确认事实，更主动澄清会改变路径的意图与细节，关键用户决定优先使用 canonical Ask User，确认前不实施，且无权自行退出模式。若 canonical Ask User 在当前 Engine/turn 中不可用，模型可以在普通 Assistant 回复中给出同一问题的清晰 `A / B / C` 选项并允许自定义答案，然后结束本次回复等待下一条用户消息；这只是普通对话，不创建、伪造、resolve 或降级 canonical User Input request，原 Ask Tool 的注册、lease 与 terminal lifecycle 仍按上文 fail closed。Learn 采用 answer-first、example-first、adult-respect 的持续教学，并按真实结构使用现有 Mermaid/ASCII/table 表达，不建立测验门、学习档案或掌握度状态。两种模式都由用户通过 Composer 明确退出；一次发送、回答或 Ask settlement 不会自动清除。
