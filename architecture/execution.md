...
- 允许且可用的Gateway definitions全部注册；每个安全request/turn边界由canonical lifecycle派生最小Gateway active set：extension-local loader、该request明确要求且仍被Built-in policy与availability允许的最小Gateway bundle，以及此前由loader按需激活且仍有效的definitions；其余Gateway tools保持inactive并按需additive activation。具体lifecycle bundle只从现有Product owner的canonical state、prompt/envelope与exact schemas派生，不预设固定core、不在架构硬编码tool names，也不持久化第二active truth。
- lifecycle condition activation与退出后的removal只发生在安全request/turn边界，不热切已admitted request或call。Host Extension的发现、匹配、激活与移除只能针对它自己的immutable AgentGateway name set，不得为任何非owned Pi tool source建立inventory、分类、保留规则或控制面。exact Pi setter若要求完整active list，只能把Pi当前集合当作opaque base原样union owned additions，不能据此识别、重算或修改非owned names；非owned session tools的唯一owner与证据路由见[`pi-native-session-tools-review.md`](../research/pi-native-session-tools-review.md)。loader名称、ranking、limit、算法与通用依赖图都不是架构合同。
...
Pi-native extension-local loader只是exact `0.84.2`下可能需要的可替换实现细节，不是OmniMind稳定子系统、产品工具或全局工具搜索。它只做轻量发现与additive `setActiveTools`，不代理执行、不返回完整schemas、不连接server、不建立索引或active store。canonical lifecycle在request前的condition activation/removal是独立的安全边界投影，不伪装成loader search，也不建立第二owner。完整schema只在工具active后的安全agent request由Pi以Provider-native deferred representation或安全fallback进入真实工具面；最终执行继续回到AgentGateway `tools/call`。registered、active、lifecycle-required与authorized是不同事实：每次真实调用仍按当前Built-in policy、session identity、credential、availability、runtime mode/permission、真实存在的approval、exact turn authority、timeout与cancellation重新判定。

Built-in关闭某组后，新OmniMind Session不注册该组；旧Session的loader与lifecycle projection按live policy不再发现或激活它，stale call由Gateway立即拒绝。若当前Goal、Automation或其他canonical lifecycle依赖的最小bundle因此不完整，不能绕过policy自动active或继续loop并伪造完成；必须在dispatch/safe boundary阻止或暂停该lifecycle并准确投影unavailable，具体用户呈现继续由现有Product owner与focused Gate B证据决定。已准入且执行中的调用不因普通exposure toggle被伪装成emergency kill，取消仍归turn/session owner。重新开启不把创建时未注册的schema偷偷注入旧Session，只按Pi真实reload/new-session边界生效。reload、resume与fork优先让Pi Session承担active truth；无法原生保持时允许安全重载或重新发现，但不得建立第二active store，旧Extension实例与handler也不得继续生效。

Prompt与active tool surface必须在同一request一致：任何发送给模型的prompt或envelope都不得直接点名inactive tool。OmniMind Agent的generic Host guidance只说明额外Host能力可按需发现和加载、需要时使用当前active loader、激活后在下一安全turn调用并且不要猜工具名；不得枚举inactive Browser、Device、Thread、Automation工具，也不得把全catalog或长说明换一个位置重新注入。exact Goal、Automation等canonical lifecycle prompt可以点名其职责所需tools，但同一request发送前对应最小bundle必须已经active；bundle不得反向进入全局prompt。stock Pi和其他Engine继续获得与其完整filtered schema一致的直接工具指导。
