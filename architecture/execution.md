...
Web Access的不可用收缩按工具真实依赖执行，而不是整包开关。App与Session启动不发health/search probe；Session init只根据配置、credential source、endpoint和native auth投影structurally possible。第一次真实搜索沿canonical route惰性获得availability证据；用户显式“重新检查”才主动运行同一最小真实route，可能消耗Provider额度，也不生成永久connected truth；同一显式动作pending期间必须single-flight，不能因连点、重渲染或超时恢复重复计费。只有当前configured/default search candidate set被可靠穷尽时，Extension才从当前Pi Session active set移除自己拥有的`web_search`与`source_check`；`fetch_content`与`get_search_content`仍可依赖direct URL、本地解析或已有结果而保留。单个named Provider失败或只有transient/network错误只能投影degraded，不能证明全局unavailable。Settings原子保存或显式刷新成功后，package config service只发布process-local revision invalidation；每个live Extension instance自行重读并只恢复自己曾移除的两个工具，listener随Session shutdown清理。这个signal不是持久真相，也不维护全局Session registry；外部文件编辑由Settings刷新、native reload或新Session重新发现，不增加file watcher、后台timer、健康daemon或AgentGateway协调器。

Web Access Curator是pending `web_search` tool call的短时交互面，不是普通浏览历史。每个pending call第一次呈现时通过Browser owner内部typed presentation seam创建独立ephemeral Tab，绝不能复用并导航用户当前Tab；Host只在内存保留当前Thread/toolCall与`tabId`/token URL/expiry的最小handle，后续由对应Timeline row精确聚焦，Tab已关闭且Curator仍pending时才重建。只有pending Timeline row拥有exact reopen；terminal后立即移除该动作，只留普通tool result/activity，App/Thread reopen不能显示无法恢复的假入口。只有owning Thread位于前台时才自动呈现；后台Thread只使用既有waiting-for-user activity/attention，不能切route、抢Right Dock或新增通知/调度控制面，用户进入该Thread后再按exact activity打开，未处理则继续服从上游idle timeout。这个内部seam不得扩张Agent可见`browser_open` schema，也不得建立Curator tab store、Product fact或第二Browser lifecycle。

关闭Curator Tab、Right Dock或Browser pane只关闭展示，不取消tool call。单个tool call terminal只清理该call自己的Curator server、stream、timer、request、presentation handle和临时资源；Run abort只中止该Run拥有的in-flight calls；`session_start/session_tree`保留上游语义但只作用于当前Extension instance和对应branch；`session_shutdown`才清理整个instance、全部剩余请求、cache、storedResults、listener与临时资源。可恢复presentation失败保持pending并允许retry；Curator server/protocol或Host handoff不可恢复失败必须typed-error settle并按call scope cleanup。Curator控制台是internal-only surface：隐藏`Open externally`、raw-link copy和raw token地址；其中的来源链接打开为普通OmniMind Browser Tab，之后遵循普通Browser的外部打开合同。ephemeral Tab从第一次投影起就必须排除Browser recent history、localStorage与tab restore；loopback response禁用cache/referrer传播，token不得进入Product event、Timeline raw payload、log、diagnostics或截图。Curator创建时使用当前OmniMind locale与resolved light/dark theme的短时presentation snapshot；不从OS/browser默认猜测，也不持久化第二份locale/theme状态。
