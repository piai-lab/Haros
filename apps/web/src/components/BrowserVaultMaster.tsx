...
import type { BrowserVaultMethods } from "@harnessos/contracts";
...
export function BrowserVaultMaster({
  ...
  const submit = async () => {
    ...
    if (action.kind === "setup" && (password.length < 12 || password !== confirmation)) {
      setError(t("browser.vaultMasterMismatch"));
      return;
    }
    ...
    try {
      ...
      if (request === generation.current) setError(browserVaultErrorMessage(cause, t));
    } finally {
      ...
      {revealed !== null ? (
        <>
          <label className="block space-y-1 text-sm">
            <span>{t("browser.vaultPassword")}</span>
            <Input
              aria-label={t("browser.vaultRevealed")}
              value={revealed}
              readOnly
              autoComplete="off"
              className="font-mono"
        ...
        >
          <label className="block space-y-1 text-sm">
            <span>
              {action.kind === "setup" ? t("browser.vaultNewMaster") : t("browser.vaultMaster")}
            </span>
            <Input
              type="password"
              autoFocus
          ...
          {action.kind === "setup" ? (
            <>
              <label className="block space-y-1 text-sm">
                <span>{t("browser.vaultConfirmMaster")}</span>
                <Input
                  type="password"
                  value={confirmation}
                  maxLength={1024}
              ...
              <p className="text-xs text-muted-foreground">{t("browser.vaultKeepMaster")}</p>
            </>
          ) : null}
          {error ? (
