// FILE: EngineUsageSettingsPanel.tsx
// Purpose: Settings → Usage panel. One card per supported engine showing live remaining
// quota/credits with linear progress meters, the engine brand icon, and plan/status pills.
// Usage is fetched from each CLI's local sign-in state by the server. Engines that use
...
import { EngineUsageLimitRows } from "~/components/EngineUsageLimitRows";
import { EngineUsageLineList } from "~/components/EngineUsageLineList";
      ...
      <div className="space-y-3.5 p-4">
        ...
        {status === "ok" && hasUsage ? (
          <>
            ...
            {meterRows.length > 0 ? (
              <EngineUsageLimitRows rows={meterRows} surface="settings" />
            ) : null}
            {usageLines.length > 0 ? (
              <EngineUsageLineList
                className={cn(
...
const HISTORY_GROUPS = ["engine", "model", "workspace", "date"] as const;
...
export function UsageHistorySection() {
  ...
  const [groupBy, setGroupBy] = useState<UsageHistoryGroupBy>("engine");
                      ...
                      <span className="text-right text-muted-foreground">
                        {engine.status === "unsupported"
                          ...
                          : engine.status === "paused"
                            ...
                            : engine.progress.skippedFiles > 0
                              ? t("settings.usageHistoryFilesSkipped", {
                                  count: engine.progress.skippedFiles,
                                })
                              : engine.status === "partial" || engine.detailCode
      ...
      >
        {showInitialLoading ? (
          ...
          <div className="flex flex-col gap-3">
            {cards.map((snapshot) => (
              <EngineUsageCard key={snapshot.engine} snapshot={snapshot} />
            ))}
          </div>
        )}
