...
export function resolveDiffLineBlameTarget(
  ...
  const viewportWidth = typeof window === "undefined" ? BLAME_POPOVER_WIDTH_PX : window.innerWidth;
  const viewportHeight =
    typeof window === "undefined" ? BLAME_POPOVER_HEIGHT_PX : window.innerHeight;
  const preferredLeft = line.event.clientX - BLAME_POPOVER_WIDTH_PX / 2;
  const left = Math.max(8, Math.min(preferredLeft, viewportWidth - BLAME_POPOVER_WIDTH_PX - 8));
  const preferredTop = lineRect.bottom + 8;
  const top =
    preferredTop + BLAME_POPOVER_HEIGHT_PX > viewportHeight - 8
      ? Math.max(8, lineRect.top - BLAME_POPOVER_HEIGHT_PX - 8)
      : preferredTop;
  return {
    filePath,
...
export function DiffLineBlamePopover(props: {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label={t("diff.blameAria", { line: props.target.line })}
    ...
    >
      {isLoadingBlame ? (
        <p className="text-[11px] text-muted-foreground">{t("diff.blameLoading")}</p>
      ) : !blame ? (
        <p className="text-[11px] text-muted-foreground">{t("diff.blameUnavailable")}</p>
      ) : blame.uncommitted ? (
        <p className="text-[12px] text-foreground">{t("diff.blameUncommitted")}</p>
      ) : (
        <>
          ...
          {blame.authorTime.length > 0 ? (
            <p className="text-[11px] text-muted-foreground/75">
              {formatShortTimestamp(blame.authorTime, props.timestampFormat)}
            </p>
          ) : null}
        </>
      )}
      {handleReferenceInChat || (blame && !blame.uncommitted) ? (
        <div className="flex items-center gap-1 border-t border-border/60 pt-1.5">
          ...
          {handleReferenceInChat ? (
            <BlameActionButton
              label={t("diff.blameReferenceLine")}
