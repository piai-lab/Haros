  ...
  const submit = () => {
    ...
    void mutation
      ...
      .catch((error: unknown) => {
        ...
        toastManager.add({
          type: "error",
          title: t("pullRequest.postCommentFailed"),
          description:
            error instanceof Error ? error.message : t("pullRequest.githubCommentFailed"),
        });
      })
      .finally(() => {
  ...
  return (
    <div className="flex items-center gap-2 rounded-3xl border border-border/60 bg-background py-1 pl-3 pr-1.5 shadow-sm">
      <span
        ...
        title={t("pullRequest.commentingAsGithub")}
      >
      ...
      <textarea
        ...
        placeholder={t("pullRequest.leaveComment")}
        aria-label={t("pullRequest.leaveComment")}
      ...
      <button
        type="button"
        disabled={!canSubmit}
        aria-label={t("pullRequest.postComment")}
        title={t("pullRequest.postComment")}
        onClick={() => void submit()}
