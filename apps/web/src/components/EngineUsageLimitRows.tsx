// FILE: EngineUsageLimitRows.tsx
// Purpose: Shared engine usage limit-row renderer for Settings and compact
...
export type EngineUsageLimitRowsSurface = "settings" | "popover";
...
function SettingsUsageLimitRow({
  ...
  return (
    <div className="space-y-1.5">
      ...
      <EngineUsageTrack row={row} surface="settings" ariaLabel={ariaLabel} />
      <div className="flex items-center justify-between text-[11px] tabular-nums text-muted-foreground">
        <span>{row.leftText}</span>
        {row.resetText ? <span>{row.resetText}</span> : null}
      </div>
      <EngineUsagePaceLine row={row} surface="settings" />
    </div>
  );
}
...
function PopoverUsageLimitRow({
  ...
  return (
    <div className="space-y-1 text-[length:var(--app-font-size-chat-meta,10px)] leading-tight">
      ...
      <EngineUsageTrack row={row} surface="popover" ariaLabel={ariaLabel} />
      <EngineUsagePaceLine row={row} surface="popover" />
    </div>
  );
}

export function EngineUsageLimitRows({
  rows,
  surface,
}: {
