// FILE: ProviderUsageLimitRows.browser.tsx
// Purpose: Browser proof for localized provider-window labels and progress-track accessibility.
...
describe("ProviderUsageLimitRows", () => {
  ...
  it.each([
    {
      locale: "en" as const,
      visibleLabels: ["Weekly", "Weekly (overage)", "Other limit · New Provider Window"],
      ariaLabels: [
        "Weekly remaining",
        "Weekly (overage) remaining",
        "Other limit · New Provider Window remaining",
      ],
    },
    {
      locale: "zh-CN" as const,
      visibleLabels: ["Weekly", "每周（超额用量）", "其他限额 · New Provider Window"],
      ariaLabels: [
        "Weekly 剩余量",
        "每周（超额用量） 剩余量",
        "其他限额 · New Provider Window 剩余量",
      ],
    },
    ...
    const screen = await render(
      <I18nProvider>
        <ProviderUsageLimitRows rows={rows} surface="settings" />
      </I18nProvider>,
    );
