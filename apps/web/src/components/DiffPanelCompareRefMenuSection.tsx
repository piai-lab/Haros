...
function truncateCommitSubject(subject: string, emptyLabel: string): string {
...
export function DiffPanelCompareRefMenuSection(props: {
  ...
  return (
    <MenuGroup>
      <MenuGroupLabel>{t("diff.compareWith")}</MenuGroupLabel>
      <MenuRadioGroup
        ...
        {commits.map((commit) => (
          <MenuRadioItem key={commit.sha} value={buildDiffPanelCompareRefValue(commit.sha)}>
            ...
            <span className="min-w-0 flex-1 truncate text-[11px]">
              {truncateCommitSubject(commit.subject, t("diff.noSubject"))}
            </span>
          </MenuRadioItem>
        ))}
      </MenuRadioGroup>
      <div className="px-2 py-1">
        <Input
          ...
          size="sm"
          type="text"
          placeholder={t("diff.compareRefPlaceholder")}
          value={refDraft}
