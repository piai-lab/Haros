// FILE: OmniMindLogoButton.browser.tsx
...
describe("OmniMindLogoButton", () => {
  ...
  it("keeps both official theme variants and a visible keyboard focus ring", async () => {
    const screen = await render(
      <OmniMindLogoButton aria-label="Focus composer" size={64} variant="satin" />,
    );
    const button = screen.getByRole("button", { name: "Focus composer" });
    const images = button.element().querySelectorAll("img");
    expect(images[0]?.getAttribute("src")).toBe("/brand/omnimind-logo-satin.svg");
    expect(images[1]?.getAttribute("src")).toBe("/brand/omnimind-logo-satin-dark.svg");
    await vi.waitFor(() => {
