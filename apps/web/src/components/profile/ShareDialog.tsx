...
// Purpose: Preview and locally copy/save an OmniMind activity PNG; no external sharing owner.
...
export function ShareDialog({
  ...
  const handleCopy = useCallback(async () => {
    ...
    setStatus(
      t(
        copied
          ? "settings.profileImageCopied"
          : "settings.profileImageCopyFailed",
      ),
    );
    setBusy(null);
  }, [render, t]);
  ...
  const handleSave = useCallback(async () => {
    ...
    const blob = await render();
    if (blob)
      downloadBlob(blob, `omnimind-activity-${stats.timezone.today}.png`);
    setStatus(
      t(
        blob
          ? "settings.profileImageSaved"
          : "settings.profileImageRenderFailed",
      ),
    );
    setBusy(null);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogPopup className="sm:max-w-[560px]">
        <DialogTitle className="text-center text-xl">
          {t("settings.exportProfileActivity")}
        </DialogTitle>
