...
describe("Product submission health wiring", () => {
  it("retains durable Queue ownership and returns before admission when dispatch is unavailable", () => {
    ...
    const ownershipTransfer = productJourney.indexOf("productQueueOwnershipTransferred = true");
    ...
    expect(queueOnlyPath).toContain("return;");
    expect(queueOnlyPath).toContain(
