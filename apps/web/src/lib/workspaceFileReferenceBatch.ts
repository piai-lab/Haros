import type { ProjectResolveWorkspaceFileReferencesResult } from "@harnessos/contracts";
...
async function flushWorkspaceFileReferenceBatch(cwd: string): Promise<void> {
  ...
  try {
    const resolvedByPath = await requestWorkspaceFileReferenceChunks(cwd, [
      ...new Set(pending.map((entry) => entry.relativePath)),
    ]);
...
export function resolveWorkspaceFileReferenceBatched(input: {
  ...
  return new Promise((resolve, reject) => {
    ...
    else pendingByCwd.set(input.cwd, [entry]);
