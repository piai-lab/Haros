// FILE: engineDiscoveryReactQuery.test.ts
// Purpose: Locks engine model discovery query semantics — retry policy,
...
describe("engine execution capability identity", () => {
  const capabilities = {
  ...
  it("rejects a late projection for a different Engine or model", () => {
    expect(
      engineExecutionCapabilitiesMatchSelection(capabilities, {
        engine: "codex",
        model: "gpt-test",
      }),
    ).toBe(true);
...
describe("providerModelsQueryOptions", () => {
  ...
  it("refreshes the shared OmniMind catalog after engine-prefix invalidation", async () => {
