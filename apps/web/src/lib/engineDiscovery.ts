...
// Purpose: Shares engine-discovery helpers across chat and browser surfaces.
// Layer: Web lib
// Exports: cwd resolution, search normalization, and engine skill/plugin display helpers.
...
export interface EngineDiscoverySearchField {
...
export const ENGINE_DISCOVERY_SECONDARY_FIELD_WEIGHT = 200;
const ENGINE_DISCOVERY_TERTIARY_FIELD_WEIGHT = 400;
...
export function buildSkillSearchFields(
  skill: Pick<EngineSkillDescriptor, "name" | "description" | "interface"> &
    Partial<Pick<EngineSkillDescriptor, "path">>,
): EngineDiscoverySearchField[] {
  return [
    { value: skill.name },
    { value: skill.interface?.displayName },
    {
      value: skill.interface?.shortDescription,
      weight: ENGINE_DISCOVERY_SECONDARY_FIELD_WEIGHT,
    },
    { value: skill.description, weight: ENGINE_DISCOVERY_SECONDARY_FIELD_WEIGHT },
    { value: skill.path, weight: ENGINE_DISCOVERY_TERTIARY_FIELD_WEIGHT },
  ];
}
...
export function buildPluginSearchFields(
  plugin: Pick<EnginePluginDescriptor, "name" | "interface">,
): EngineDiscoverySearchField[] {
  return [
    { value: plugin.name },
    ...
    { value: plugin.interface?.category, weight: ENGINE_DISCOVERY_SECONDARY_FIELD_WEIGHT },
    { value: plugin.interface?.developerName, weight: ENGINE_DISCOVERY_SECONDARY_FIELD_WEIGHT },
  ];
}

export function buildCommandSearchFields(
  command: Pick<EngineNativeCommandDescriptor, "name" | "description">,
): EngineDiscoverySearchField[] {
  return [
    { value: command.name },
    { value: command.description, weight: ENGINE_DISCOVERY_SECONDARY_FIELD_WEIGHT },
  ];
}
