...
function resolveHarosPublicAssetOpenTarget(path: string, workspaceRoot: string | null) {
  if (!workspaceRoot) {
    return null;
  }
...
export function resolveWorkspaceFileOpenTarget(
  ...
  return resolveHarosPublicAssetOpenTarget(withoutPosition, workspaceRoot);
}
