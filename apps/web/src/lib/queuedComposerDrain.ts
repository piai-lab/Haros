...
// Purpose: Auto-dispatch queued Composer turns for mounted and background threads.
// Layer: Web subscription utility

import type { AssistantDeliveryMode, ThreadId } from "@harnessos/contracts";
...
export function clearQueuedComposerSteerGate(threadId: ThreadId): void {
  if (steerGatesByThreadId.delete(threadId)) requestQueuedComposerDrainPass();
}
...
export function tryBeginQueuedComposerAutoDispatch(threadId: ThreadId): boolean {
  if (autoDispatchLocks.has(threadId)) return false;
  autoDispatchLocks.add(threadId);
  return true;
}

export function endQueuedComposerAutoDispatch(threadId: ThreadId): void {
  if (autoDispatchLocks.delete(threadId)) requestQueuedComposerDrainPass();
}
...
export function startQueuedComposerDrainWatcher(options?: {
  ...
  drainStartCount += 1;
  if (options?.dispatch) dispatchQueuedTurn = options.dispatch;
  if (options?.now) nowMs = options.now;
  if (options?.assistantDeliveryMode) assistantDeliveryMode = options.assistantDeliveryMode;
  if (drainStartCount === 1) {
    const unsubscribeDrafts = useComposerDraftStore.subscribe((current, previous) => {
      if (!haveQueuedTurnsChanged(current.draftsByThreadId, previous.draftsByThreadId)) return;
      resetRetriesForChangedQueueHeads(current.draftsByThreadId);
      requestQueuedComposerDrainPass();
    });
    const unsubscribeStore = useStore.subscribe((current, previous) => {
      if (!hasRelevantThreadStateChanged(current, previous)) return;
...
function collectThreadIdsWithQueuedTurns(): ThreadId[] {
  ...
  for (const [threadId, draft] of Object.entries(drafts)) {
    if (draft.queuedTurns.length > 0) threadIds.push(threadId as ThreadId);
  }
  return threadIds;
}
...
function haveQueuedTurnsChanged(
  ...
  for (const threadId of threadIds) {
    if (current[threadId]?.queuedTurns !== previous[threadId]?.queuedTurns) return true;
  }
  return false;
}
...
function resetRetriesForChangedQueueHeads(
  ...
  for (const [threadId, retryState] of retryStateByThreadId) {
    if (draftsByThreadId[threadId]?.queuedTurns[0]?.id !== retryState.queuedTurnId) {
...
function hasRelevantThreadStateChanged(current: AppState, previous: AppState): boolean {
  for (const threadId of collectTrackedThreadIds()) {
    if (threadDrainSignal(current, threadId) !== threadDrainSignal(previous, threadId)) return true;
  }
  return false;
}
...
function readQueuedComposerAutoDispatchGates(threadId: ThreadId): QueuedComposerAutoDispatchGates {
  ...
  return {
    hasQueueableLiveTurn: phase === "running" && thread?.session?.activeTurnId != null,
    phase,
...
function advanceSteerGates(): number | null {
  ...
  for (const [threadId, gate] of [...steerGatesByThreadId.entries()]) {
    ...
    steerGatesByThreadId.set(threadId, transition.gate);
...
function retryDelayForThread(threadId: ThreadId, queuedTurnId: string): number | null | undefined {
  ...
  if (!retryState || retryState.queuedTurnId !== queuedTurnId) return undefined;
  return retryState.retryAt === null ? null : retryState.retryAt - nowMs();
}
...
function runQueuedComposerDrainPass(): void {
  ...
  for (const threadId of collectThreadIdsWithQueuedTurns()) {
    ...
    if (!shouldAutoDispatchQueuedComposerTurn(readQueuedComposerAutoDispatchGates(threadId))) {
      continue;
    }
    if (!tryBeginQueuedComposerAutoDispatch(threadId)) continue;
    const localDispatch = {
      ...createLocalDispatchSnapshot(getThreadFromState(useStore.getState(), threadId)),
