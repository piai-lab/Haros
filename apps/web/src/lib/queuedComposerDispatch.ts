...
// Purpose: Dispatch a snapshotted queued Composer turn without a mounted ChatView.
// Layer: Web orchestration helper

import type { AssistantDeliveryMode, ThreadId } from "@harnessos/contracts";

import { persistEngineSelectionBeforeRuntimeMode } from "../components/ChatView.logic";
...
export async function dispatchQueuedComposerTurnHeadless(input: {
  ...
  if (queuedTurn.kind === "plan-follow-up") {
    ...
    const outgoingMessageText = formatOutgoingComposerPrompt({
      engine: queuedTurn.selectedEngine,
    ...
    try {
      await persistQueuedTurnThreadSettings({ api, thread, queuedTurn, createdAt });
      ...
      await api.orchestration.dispatchCommand({
        ...
        ...(queuedTurn.modelPresentationIdentity
          ? { modelPresentationIdentity: queuedTurn.modelPresentationIdentity }
          : {}),
        ...(queuedTurn.engineOptionsForDispatch
          ? { engineOptions: queuedTurn.engineOptionsForDispatch }
          : {}),
  ...
  if (!outgoingTextSeed.trim() && queuedTurn.images.length === 0) return false;
  ...
  const mentionedSkills = filterPromptSkillReferences(
    outgoingMessageText,
    queuedTurn.skills,
    queuedTurn.selectedEngine,
  );
  const mentionedMentions = filterPromptEngineMentionReferences(
    outgoingMessageText,
    queuedTurn.mentions,
  );
...
async function persistQueuedTurnThreadSettings(input: {
  ...
  await persistEngineSelectionBeforeRuntimeMode({
    currentEngineSelection: input.thread.engineSelection,
    nextEngineSelection: input.queuedTurn.engineSelection,
    currentRuntimeMode: input.thread.runtimeMode,
    nextRuntimeMode: input.queuedTurn.runtimeMode,
    persistEngineSelection: (engineSelection) =>
