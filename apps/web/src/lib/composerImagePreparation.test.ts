...
describe("prepareComposerImageFile", () => {
  ...
  it("leaves engine-safe images byte-for-byte untouched", async () => {
  ...
  it("rejects raw images above the bounded import limit before decoding", async () => {
    ...
    Object.defineProperty(file, "size", {
      configurable: true,
      value: ENGINE_SEND_TURN_MAX_IMAGE_IMPORT_BYTES + 1,
    });
  ...
  it("converts oversized Retina PNGs into bounded JPEG files", async () => {
    ...
    expect(prepared.size).toBeLessThanOrEqual(ENGINE_SEND_TURN_MAX_IMAGE_BYTES);
