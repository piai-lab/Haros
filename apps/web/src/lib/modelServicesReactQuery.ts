...
export const modelServicesQueryKeys = {
...
function isAddServiceIntentQueryKey(queryKey: readonly unknown[]): boolean {
  return (
    queryKey[0] === modelServicesQueryKeys.all[0] &&
    ((queryKey[1] === "list" && queryKey[2] === "add_service") ||
      (queryKey[1] === "detail" && queryKey[3] === "add_service"))
  );
}

export async function cancelHarosModelServicesAddIntentQueries(
...
export function modelServicesListQueryOptions(input: { enabled: boolean; intent?: "add_service" }) {
  return queryOptions({
    queryKey: modelServicesQueryKeys.list(input.intent ?? null),
    enabled: input.enabled,
    queryFn: async ({ signal }) =>
      ensureNativeApi().modelServices.list(input.intent ? { intent: input.intent } : {}, {
        signal,
      }),
    staleTime: 10_000,
...
export function modelServiceDetailQueryOptions(input: {
  enabled: boolean;
  serviceId: string | null;
  intent?: "add_service";
}) {
  return queryOptions({
    queryKey: modelServicesQueryKeys.detail(input.serviceId, input.intent ?? null),
    enabled: input.enabled && input.serviceId !== null,
    queryFn: async ({ signal }) => {
      if (input.serviceId === null) {
        throw new Error("Model service detail requires a service id");
      }
      return ensureNativeApi().modelServices.get(
