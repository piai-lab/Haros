...
import { isWorkspaceFileWriteConflictError } from "@harnessos/shared/workspaceFileWrite";
import type { QueryClient } from "@tanstack/react-query";

import { ensureNativeApi } from "~/nativeApi";
import { refreshGitQueriesForCwd } from "./gitReactQuery";
      ...
      .then(async () => {
        ...
        while (this.dirty) {
          ...
          try {
            ...
            void refreshGitQueriesForCwd(this.client, this.cwd).catch(() => undefined);
            guarded = true;
          } catch (error) {
            this.dispatch({
              type: "saveFailed",
