...
export const COMPOSER_IMAGE_MAX_IMPORT_BYTES = CHAT_IMAGE_IMPORT_MAX_BYTES;
...
export async function prepareComposerImageFile(file: File): Promise<File> {
  ...
  try {
    ...
    if (cause instanceof ComposerImagePreparationError) throw cause;
    throw new ComposerImagePreparationError(`OmniMind could not optimize '${imageName(file)}'.`, {
      cause,
    });
  }
}
