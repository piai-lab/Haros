...
} from "./engineUsageDisplay";

describe("engineUsageDisplay", () => {
  afterEach(() => {
    vi.useRealTimers();
  });
  ...
  it("selects the most constrained display row for compact header chips", () => {
    ...
    const primary = selectPrimaryEngineUsageDisplayRow(rows);
  ...
  it("centralizes reserve and eta details for display rows", () => {
    ...
    const [row] = deriveEngineUsageDisplayRows([
      {
        engine: "codex",
    ...
    expect(row ? engineUsagePaceDetails(row) : null).toEqual({
