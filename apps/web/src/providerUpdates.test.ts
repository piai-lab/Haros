...
function serverSettings(
  overrides: Partial<ServerSettingsView["providers"]> = {},
): ServerSettingsView {
  const provider = {
    enabled: true,
    binaryPath: "",
...
describe("getVisibleProviderUpdateStatuses", () => {
  it("excludes providers hidden from OmniMind so unchecked providers do not nag", () => {
...
describe("withProviderUpdateTimeout", () => {
  it("keeps the transport watchdog beyond the bounded Homebrew update window", () => {
    expect(PROVIDER_UPDATE_REQUEST_TIMEOUT_MS).toBe(60 * 60_000 + 15_000);
  });
...
describe("shouldPromptProviderUpdate", () => {
  ...
  it("keeps prompting for providers OmniMind can prove are behind", () => {
