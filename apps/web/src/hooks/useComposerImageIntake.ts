...
const ATTACHMENT_LIMIT_ERROR = `You can attach up to ${ENGINE_SEND_TURN_MAX_ATTACHMENTS} references per message.`;
    ...
    const task = this.#tail
      ...
      .then(async () => {
        ...
        const rejectedAtCapacity =
          ...
          job.existingAttachmentCount() >= ENGINE_SEND_TURN_MAX_ATTACHMENTS;
        const error = rejectedAtCapacity ? ATTACHMENT_LIMIT_ERROR : result.error;
        job.onError(error);
      })
      .catch((cause) => {
        if (this.#isStale(generation)) return;
        job.onError(cause instanceof Error ? cause.message : "HarnessOS could not prepare image.");
      })
