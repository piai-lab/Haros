// FILE: useEngineStatusRefresh.ts
  ...
  return async (options?: RefreshProviderStatusesOptions) => {
    ...
    try {
      ...
      if (!options?.silent) {
        toastManager.add({
          type: "error",
          title: "Unable to refresh engine status",
          description:
            error instanceof Error ? error.message : "Unknown error refreshing engine status.",
        });
      }
      return null;
    }
...
type EngineStatusRefreshOptions = {
...
export function useEngineStatusRefresh(options: EngineStatusRefreshOptions): void {
  const queryClient = useQueryClient();
