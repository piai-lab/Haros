...
function shouldKeepBuiltInSlashCommandDespiteNativeCollision(
  ...
  return (
    ...
    command === "export" ||
    command === "feedback" ||
    // /fork always uses Haros's thread fork lifecycle. A engine-native
  ...
  const availableCommands: ComposerSlashCommand[] =
    input.engine !== "claude"
      ...
      : [
          ...
          // creates a Haros split/context clone before the engine sees the first turn.
          ...
          // /export is app-level too — Haros owns the thread transcript, so the download
