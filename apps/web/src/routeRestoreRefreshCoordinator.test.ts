...
function shellSnapshot(
  ...
  options: {
    ...
    readonly requiresEmptyProjectShellRepair?: boolean;
  } = {},
  ...
  return {
    ...
    snapshotSequence: 1,
    requiresEmptyProjectShellRepair: options.requiresEmptyProjectShellRepair,
...
describe("route restore refresh coordinator", () => {
  ...
  it("uses the full snapshot only as a probe and re-reads the fenced shell", async () => {
    ...
    const getShellSnapshot = vi
      .fn()
      .mockResolvedValueOnce(shellSnapshot([], { projectIds: ["project-1"] }))
  ...
  it("repairs an empty projection then consumes a fresh fenced shell snapshot", async () => {
    ...
    const getShellSnapshot = vi
      .fn()
      .mockResolvedValueOnce(shellSnapshot([], { requiresEmptyProjectShellRepair: true }))
  ...
  ] as const)("does not repair a %s authoritative empty shell", async (_state, marker) => {
    let hasThreads = false;
    const getShellSnapshot = vi.fn().mockResolvedValue(
      shellSnapshot([], {
        ...(marker !== undefined ? { requiresEmptyProjectShellRepair: marker } : {}),
      }),
    );
    const getSnapshot = vi.fn();
    const repairState = vi.fn();
