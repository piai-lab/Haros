...
func selectWindow(
        ...
        if application.bundleIdentifier == excludedBundleIdentifier {
            return .failure(
                AppSnapFailure(
                    ...
                    message: "Haros cannot capture its own window."
                )
            )
        }
        let title = candidate[kCGWindowName as String] as? String
    ...
    return .failure(
        AppSnapFailure(
            ...
            message: "The requested window is no longer on screen."
        )
    )
}
...
final class OneFrameWindowCapture: NSObject, SCStreamOutput, SCStreamDelegate {
    ...
    private let outputQueue = DispatchQueue(label: "ai.piai.harnessos.appsnap.stream-output")
...
final class AppSnapCaptureCoordinator {
    ...
    private let queue = DispatchQueue(label: "ai.piai.harnessos.appsnap.capture")
