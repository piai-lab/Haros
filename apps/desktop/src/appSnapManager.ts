...
const MAX_PENDING_CAPTURES = ENGINE_SEND_TURN_MAX_ATTACHMENTS;
...
function parseStoredPendingCapture(value: unknown): StoredPendingAppSnapCapture | null {
  ...
  if (
    ...
    sizeBytes <= 0 ||
    sizeBytes > ENGINE_SEND_TURN_MAX_IMAGE_BYTES
  ) {
    return null;
  }
  return {
...
async function readValidatedPendingPng(filePath: string, expectedBytes?: number): Promise<Buffer> {
  const bytes = await readRegularFile(filePath, ENGINE_SEND_TURN_MAX_IMAGE_BYTES, expectedBytes);
...
export class DesktopAppSnapManager {
  ...
  async listWindows(): Promise<DesktopAppSnapWindowEntry[]> {
    if (this.#platform !== "macos") {
      throw new Error("AppSnap window listing is available only on macOS.");
    }
  ...
  #settleCaptureRequest(
    ...
    if (error) {
      ...
      request.reject(new Error("AppSnap did not return a capture."));
    }
    return true;
  }
  ...
  #handleWatchMessage(child: AppSnapHelperProcess, message: AppSnapHelperMessage): void {
    ...
    if (message.type === "captured") {
      ...
      if (this.#timedOutCaptureRequestIds.has(message.id) || message.id.startsWith("picker-")) {
        this.#captureReadQueue = this.#captureReadQueue
          .then(async () => {
    ...
    // Benign overlap errors surface as a toast without yanking Haros to the
  ...
  async #consumeCapture(
    ...
    if (discardedRecord) {
      ...
      this.#emitCaptureError(
        ...
        `Haros could retain only the latest ${MAX_PENDING_CAPTURES} AppSnaps while the composer was unavailable. The oldest capture was discarded.`,
  ...
  async #consumeRequestCapture(
    ...
    const pendingRecord = await this.#persistPendingCapture(capture);
    await FS.promises.unlink(capturePath).catch(() => undefined);
    if (!this.#settleCaptureRequest(capture.id, capture)) {
