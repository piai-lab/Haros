...
describe("AppSnap helper protocol", () => {
  ...
  it("retains a full composer batch and reports any overflow", async () => {
    ...
    try {
      ...
      for (let index = 0; index <= CHAT_TURN_MAX_ATTACHMENTS; index += 1) {
      ...
      await vi.waitFor(() => {
        expect(onCaptured).toHaveBeenCalledTimes(CHAT_TURN_MAX_ATTACHMENTS + 1);
      });
      const pendingCaptures = await manager.listPendingCaptures();
      expect(pendingCaptures).toHaveLength(CHAT_TURN_MAX_ATTACHMENTS);
