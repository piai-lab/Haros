...
describe("WebMCP browser bridge", () => {
  ...
  it("routes cancellation to the exact in-page invocation before terminating JavaScript", async () => {
    cdp.callFunctionOn
      .mockResolvedValueOnce({
        value: {
          available: true,
      ...
      .mockImplementationOnce(async (...args: unknown[]) => {
        const options = args[3] as { readonly onAbort?: () => Promise<void> };
        await options.onAbort?.();
        return { value: { status: "failed", error: { name: "AbortError", message: "cancelled" } } };
      });
    const signal = new AbortController().signal;
    ...
    await expect(
      invokeWebMcpTool(
        runtime,
        {
          discoveryId: discovery.handle!.discoveryId,
          toolId: "w1" as never,
          arguments: {},
        },
        discovery.handle!,
      ...
      browserError: { code: "BrowserAmbiguousResult", effectMayHaveCommitted: true },
    });

    expect(runtime.webContents.debugger.sendCommand).toHaveBeenCalledWith(
      "Runtime.evaluate",
      expect.objectContaining({
        expression: expect.stringContaining("__harnessosWebMcpBridgeV1?.cancel"),
        userGesture: false,
      }),
    );
  });

  it("rejects schema-invalid arguments before page execution", async () => {
    cdp.callFunctionOn
      .mockResolvedValueOnce({
        value: {
          available: true,
      ...
      .mockResolvedValueOnce({ value: { status: "invalid_arguments" } });
    const signal = new AbortController().signal;
    const discovery = await discoverWebMcpTools(runtime, {}, 1, signal);
    ...
    await expect(
      invokeWebMcpTool(
        runtime,
        {
          discoveryId: discovery.handle!.discoveryId,
          toolId: "w1" as never,
          arguments: { hiddenOverride: true },
        },
        discovery.handle!,
        signal,
      ),
    ).rejects.toMatchObject({
      browserError: { code: "BrowserInputUnsupported", effectMayHaveCommitted: false },
    });
  });

  it("treats every page failure after execution begins as ambiguous", async () => {
    cdp.callFunctionOn
      .mockResolvedValueOnce({
        value: {
          available: true,
      ...
      .mockResolvedValueOnce({
        value: { status: "failed", error: { name: "Error", message: "charged, then failed" } },
      });
    const signal = new AbortController().signal;
    const discovery = await discoverWebMcpTools(runtime, {}, 1, signal);
  ...
  it.each([
    ...
    ["tab destruction", () => webContentsEvents.emit("destroyed")],
  ])("invalidates discovery on %s", async (_label, invalidate) => {
    cdp.callFunctionOn.mockResolvedValue({
      value: {
        available: true,
        implementation: "compatibility",
