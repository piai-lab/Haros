...
function makeTempDir(): string {
  const directory = FS.mkdtempSync(Path.join(OS.tmpdir(), "omnimind-profile-test-"));
...
describe("desktopUserDataProfile", () => {
  ...
  it("honors an explicit Electron user-data directory instead of the installed profile", () => {
    expect(
      resolveDesktopUserDataPath({
        appDataBase: "/Users/tester/Library/Application Support",
        userDataDirectoryName: "omnimind",
        explicitUserDataPath: "/tmp/omnimind-desktop-smoke/electron",
      }),
    ).toBe("/tmp/omnimind-desktop-smoke/electron");
  });
  ...
  it("repairs missing browser data from the profile recorded by the bridge", () => {
    ...
    const targetPath = Path.join(appDataBase, "omnimind");
    ...
    const targetPartitionPath = Path.join(targetPath, "Partitions", "omnimind-browser");
    ...
    FS.writeFileSync(
      Path.join(targetPath, "omnimind-profile-seed.json"),
  ...
  it("rejects bridge manifests that point outside the OmniMind profile parent", () => {
    const appDataBase = makeTempDir();
  ...
  it("never adds a foreign SQLite sidecar beside an existing OmniMind database", () => {
    const appDataBase = makeTempDir();
    const targetPath = Path.join(appDataBase, "omnimind");
    const sourcePath = Path.join(appDataBase, "previous-profile");
    const sourcePartitionPath = Path.join(sourcePath, "Partitions", "previous-browser");
  ...
  it("replaces an orphaned target sidecar with one from the repaired database generation", () => {
    ...
    expect(
      FS.readdirSync(targetPartitionPath).some((entryName) =>
        entryName.startsWith(".omnimind-bridge-"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores a malformed bridge manifest without attempting a repair", () => {
    ...
    const targetPath = Path.join(appDataBase, "omnimind");
    FS.mkdirSync(targetPath, { recursive: true });
    FS.writeFileSync(Path.join(targetPath, "omnimind-profile-seed.json"), "{");
