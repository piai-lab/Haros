...
describe("desktop migration recovery", () => {
  it("targets the same production database and bundled restore authority as Product Service", () => {
    expect(
      resolveDesktopMigrationRecoveryPaths({
        baseDir: Path.join(Path.sep, "home", "omnimind"),
        appRoot: Path.join(Path.sep, "app"),
        isDevelopment: false,
      }),
    ).toEqual({
      dbPath: Path.join(Path.sep, "home", "omnimind", "userdata", "state.sqlite"),
  ...
  it("uses the isolated development database when the desktop backend receives a dev URL", () => {
    ...
    expect(paths.dbPath).toBe(Path.join(Path.sep, "home", "omnimind", "dev", "state.sqlite"));
  });

  it("continues only when the Product Service command clears the durable marker", async () => {
