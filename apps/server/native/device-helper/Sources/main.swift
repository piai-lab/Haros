...
final class HelperSession {
  ...
  private(set) var hid: OmniMindHIDBridge?
  private(set) var accessibility: OmniMindAXBridge?
  ...
  func attach(udid: String) throws -> [String: Any] {
    ...
    let hidBridge = OmniMindHIDBridge()
  ...
  func requireHID() throws -> OmniMindHIDBridge {
  ...
  func requireAccessibility() throws -> OmniMindAXBridge {
...
///
/// Every injection path in OmniMindHIDBridge returns silently when it has no
...
private func withHIDDelivery(_ hid: OmniMindHIDBridge, _ body: () -> Void) throws {
...
func handle(method: String, params: Params, session: HelperSession) throws -> Any {
  ...
  case "button":
    ...
    var button = OmniMindHardwareButton.home
    guard OmniMindHardwareButtonFromName(name, &button) else {
      throw RPCError(
        .invalidParams,
