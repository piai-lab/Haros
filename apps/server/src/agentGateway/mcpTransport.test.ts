...
import { ProjectId, ThreadId, TurnId, type OrchestrationThreadShell } from "@omnimind/contracts";
...
describe("makeAgentGatewayMcpTransport exposure policy", () => {
  it.effect("allows discovery without a turn but rejects every tool call before policy lookup", () =>
    Effect.gen(function* () {
      let handlerCalls = 0;
      ...
      const result = (denied.body as {
        result: { content: ReadonlyArray<{ type: "text"; text: string }> };
      }).result;
      assert.equal(JSON.parse(result.content[0]?.text ?? "{}").error?.code, "caller_turn_inactive");
