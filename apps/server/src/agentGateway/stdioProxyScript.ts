/**
 * Stdio-to-HTTP proxy script for the OmniMind agent gateway.
 *
...
function localInactiveResponse(message) {
  ...
  // CLI launched outside OmniMind therefore sees a valid empty MCP server
  // instead of a noisy failed integration. OmniMind-managed processes receive
    ...
    return [
      {
        ...
        result: {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: "omnimind", version: "1.0.0" },
        },
      },
    ];
  }
  ...
  return [
    {
      jsonrpc: "2.0",
      id,
      error: { code: -32601, message: "OmniMind is not active for this Antigravity session." },
    },
  ];
}
...
async function resolveToken() {
  ...
  if (!tokenResolution) {
    ...
    tokenResolution = (async () => {
      ...
      if (!response.ok) {
        throw new Error("OmniMind gateway bootstrap failed with HTTP " + response.status);
      }
      const payload = await response.json();
      if (!isRecord(payload) || typeof payload.bearerToken !== "string") {
        throw new Error("OmniMind gateway bootstrap returned an invalid response");
      }
    ...
    return [
      {
        jsonrpc: "2.0",
        id,
        error: { code: -32603, message: "OmniMind gateway request failed: " + String(error) },
      },
    ];
  }
}
