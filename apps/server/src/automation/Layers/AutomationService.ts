...
import { resolveAutomationStopAfterConsecutiveFailures } from "../stopPolicy.ts";
import {
...
function nextDefinitionUpdatedAt(previousUpdatedAt: string, now = isoNow()): string {
  const previousMs = Date.parse(previousUpdatedAt);
  const nowMs = Date.parse(now);
  if (!Number.isFinite(previousMs)) return now;
  return new Date(
    Math.max(Number.isFinite(nowMs) ? nowMs : Date.now(), previousMs + 1),
  ).toISOString();
}

function definitionConflict(): AutomationServiceError {
  return new AutomationServiceError({
    message: "Automation changed since it was opened. Reload and review before saving again.",
    code: "AUTOMATION_DEFINITION_CONFLICT",
  });
}

function sameDefinitionField(left: unknown, right: unknown): boolean {
  if (left === right) return true;
  return JSON.stringify(left) === JSON.stringify(right);
}

function touchedDefinitionFieldsUnchanged(
  before: AutomationDefinition,
  after: AutomationDefinition,
  input: AutomationUpdateInput,
): boolean {
  for (const key of Object.keys(input) as Array<keyof AutomationUpdateInput>) {
    if (key === "id" || key === "expectedDefinitionRevision" || key === "proposalState") continue;
    if (key === "stopOnError" || key === "stopAfterConsecutiveFailures") {
      if (
        !sameDefinitionField(
          before.stopAfterConsecutiveFailures,
          after.stopAfterConsecutiveFailures,
        )
      ) {
        return false;
      }
    ...
    if (
      !sameDefinitionField(
        before[key as keyof AutomationDefinition],
        after[key as keyof AutomationDefinition],
      )
    ) {
      return false;
    }
...
const DEFERRED_ONE_SHOT_SUPERSESSION_FIELDS = [
  "projectId",
  "sourceThreadId",
  "prompt",
  "schedule",
  ...
  "misfirePolicy",
  "acknowledgedRisks",
] as const satisfies ReadonlyArray<keyof AutomationDefinition>;

function updateSupersedesDeferredOneShotOwner(
  current: AutomationDefinition,
  updated: AutomationDefinition,
): boolean {
  return DEFERRED_ONE_SHOT_SUPERSESSION_FIELDS.some(
    (field) => !sameDefinitionField(current[field], updated[field]),
  );
}
...
function mergeDefinitionUpdate(
  ...
  const stopAfterConsecutiveFailures = resolveAutomationStopAfterConsecutiveFailures({
    ...
    current: current.stopAfterConsecutiveFailures,
  });
  ...
  const nextDefinition: AutomationDefinition = {
    ...
    iterationCount: resetExhaustedIteration ? 0 : current.iterationCount,
    updatedAt: now,
  };
...
export const AutomationServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
      ...
      return Effect.gen(function* () {
        ...
        if (automationOwnsItsThread(definition.mode)) {
          const attached = yield* automationRepository
            .attachDefinitionThread({
              ...
              threadId: plannedThreadId,
              updatedAt: now,
              expectedDefinitionRevision: definition.definitionRevision + 1,
            })
            .pipe(Effect.mapError(toServiceError("Failed to attach the automation thread.")));
          if (attached) {
            yield* publishDefinition(definition.id);
          } else {
            return yield* cleanupUnattachedWorktree({
              definition,
              run,
              project,
              environment,
              reason: "dedicated-thread-owner-changed",
            }).pipe(
              Effect.flatMap(() =>
                Effect.fail(
                  new AutomationServiceError({
                    message: "Automation ownership changed before its dedicated thread attached.",
                  }),
                ),
              ),
            );
        ...
        Effect.catch((error) =>
          Effect.gen(function* () {
            ...
            yield* publishRunResult(failed.run, "failed", failedAt, summary, true);
            if (failed.autoDisabled) {
              yield* publishDefinition(failed.run.automationId);
            }
            return yield* Effect.fail(error);
      ...
      return automationRepository
        .saveDefinition({
          definition: {
            ...definition,
            nextRunAt,
            updatedAt: nextDefinitionUpdatedAt(definition.updatedAt, now),
          },
        ...
        .pipe(
          ...
          Effect.flatMap(
            Option.match({
              ...
              onNone: () => requireDefinition(definition.id),
            }),
          ),
        );
    };
    ...
    const claimSkippedScheduledRun = (
      definition: AutomationDefinition,
      scheduledFor: string,
      ...
      Effect.gen(function* () {
        ...
        Effect.mapError(toServiceError("Failed to claim skipped automation occurrence.")),
        Effect.tap((run) =>
          Option.match(run, {
            onNone: () => Effect.void,
            onSome: (claimed) => publish({ type: "run-upserted", run: claimed }),
    ...
    const update: AutomationServiceShape["update"] = (input) =>
      Effect.gen(function* () {
        const initial = yield* requireDefinition(input.id);
        if (initial.definitionRevision !== input.expectedDefinitionRevision) {
        ...
        if (initial.proposalState === "pending") {
          return yield* Effect.fail(
            new AutomationServiceError({
        ...
        let current = initial;
        for (let attempt = 0; attempt < AUTOMATION_DEFINITION_UPDATE_MAX_ATTEMPTS; attempt += 1) {
          const now = isoNow();
          const updated = mergeDefinitionUpdate(
            current,
            input,
            nextDefinitionUpdatedAt(current.updatedAt, now),
            jitterContextFor(current.id),
          );
    ...
    const resolveProposal: AutomationServiceShape["resolveProposal"] = (input) =>
      Effect.gen(function* () {
        ...
        if (current.definitionRevision !== input.expectedDefinitionRevision) {
          return yield* Effect.fail(definitionConflict());
        }
        ...
        const definition: AutomationDefinition = {
          ...
          disabledReason: accepted ? null : "user",
          disabledAt: accepted ? null : now,
          updatedAt: nextDefinitionUpdatedAt(current.updatedAt, now),
        ...
        const persisted = yield* automationRepository.getDefinitionById({ id: definition.id }).pipe(
          Effect.mapError(toServiceError("Failed to load resolved automation proposal.")),
          Effect.flatMap(
            Option.match({
              onNone: () =>
                Effect.fail(
                  new AutomationServiceError({ message: "Automation proposal no longer exists." }),
                ),
              onSome: Effect.succeed,
            }),
          ),
        );
        yield* publishProposalActivity(persisted, input.resolution, now);
        yield* publish(
          accepted
            ? { type: "definition-upserted", definition: persisted }
            : { type: "definition-deleted", automationId: persisted.id },
        );
    ...
    const cancelRunById = (input: { readonly runId: AutomationRunId }) =>
      Effect.gen(function* () {
        ...
        const cancelled = yield* publishRunResult(
          run,
          "cancelled",
          now,
        ...
        yield* publishDefinition(run.automationId);
        return cancelled;
      });
    ...
    const deleteAutomation: AutomationServiceShape["delete"] = (input) =>
      Effect.gen(function* () {
        ...
        const archived = yield* automationRepository
          .archiveDefinition({
            id: input.id,
            archivedAt: isoNow(),
    ...
    const restartExhaustedBoundedDefinition = (definition: AutomationDefinition, now: string) =>
      Effect.gen(function* () {
        if (
          definition.archivedAt !== null ||
          definition.enabled ||
          definition.disabledReason !== "max-iterations" ||
          definition.maxIterations === null ||
          definition.iterationCount < definition.maxIterations
        ) {
          return definition;
        ...
        const restarted = {
          ...
          iterationCount: 0,
          nextRunAt,
          consecutiveFailureCount: enabled ? 0 : definition.consecutiveFailureCount,
          disabledReason: enabled ? null : definition.disabledReason,
          disabledAt: enabled ? null : definition.disabledAt,
          updatedAt: nextDefinitionUpdatedAt(definition.updatedAt, now),
    ...
    const runNow: AutomationServiceShape["runNow"] = (input) =>
      Effect.gen(function* () {
        ...
        if (definition.archivedAt !== null || definition.disabledReason === "failures") {
          return yield* Effect.fail(
            new AutomationServiceError({
              message: "This automation is disabled and cannot be run manually.",
            }),
          );
        }
    ...
    const markScheduledRunSkipped = (
      ...
      Effect.gen(function* () {
        const skippedResult = yield* automationRepository
          .markRunSkipped({
            id: run.id,
            reason,
            finishedAt: now,
        ...
        const skipped = skippedResult.run;
        yield* publish({ type: "run-upserted", run: skipped });
        if (skippedResult.definitionDisabled) {
          yield* publishDefinition(skipped.automationId);
        }
        if (skippedResult.transitioned) {
          yield* maybeStopLoop(skipped, "skipped", now);
        }
        return skipped;
      });
    ...
    const runDueDefinition = (definition: AutomationDefinition, now: string) =>
      Effect.gen(function* () {
        ...
        if (occurrence.skip) {
          const claimed = yield* claimSkippedScheduledRun(
            definition,
            scheduledFor,
            nextRunAt,
            now,
    ...
    const retryDeferredRun = (run: AutomationRun, now: string) =>
      Effect.gen(function* () {
        const legacyCleanup = yield* automationRepository
          .cleanupUnownedDeferredOneShot({ id: run.id, finishedAt: now })
          .pipe(Effect.mapError(toServiceError("Failed to clean up deferred automation run.")));
        if (legacyCleanup.transitioned) {
          yield* publish({ type: "run-upserted", run: legacyCleanup.run });
          yield* maybeStopLoop(legacyCleanup.run, "skipped", now);
          return Option.none<AutomationRunNowResult>();
        }
        const definition = yield* requireDefinition(run.automationId);
        ...
        if (reserved.state === "skipped") {
          yield* publish({ type: "run-upserted", run: reserved.run });
          yield* maybeStopLoop(reserved.run, "skipped", now);
          return Option.none<AutomationRunNowResult>();
        }
        ...
        const result = yield* dispatchRun(definition, reserved.run, now).pipe(
          Effect.catch(() =>
            automationRepository.getRunById({ id: run.id }).pipe(
              Effect.mapError(toServiceError("Failed to load automation run.")),
              Effect.map((runOption) =>
