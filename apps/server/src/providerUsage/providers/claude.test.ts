...
function makeClaudeHome(creds: Record<string, unknown>) {
  const homeDir = mkdtempSync(nodePath.join(os.tmpdir(), "omnimind-claude-usage-"));
...
function makeClaudeConfigDir(creds: Record<string, unknown>) {
  const configDir = mkdtempSync(nodePath.join(os.tmpdir(), "omnimind-claude-config-"));
...
describe("claudeUsageFetcher", () => {
  it("prefers the current macOS account before the service-only keychain fallback", async () => {
    const homeDir = mkdtempSync(nodePath.join(os.tmpdir(), "omnimind-claude-keychain-"));
    tempDirs.push(homeDir);
