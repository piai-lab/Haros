...
import { ProjectId, ThreadId, TurnId, type OrchestrationThreadShell } from "@harnessos/contracts";
...
import { makeHostGatewaySessionRegistry } from "./Layers/HostGatewaySessionRegistry.ts";
import type { HostGatewayCredentialsShape } from "./Services/HostGatewayCredentials.ts";
...
import { acquireHostGatewaySessionLease, type HostGatewaySessionLease } from "./sessionLease.ts";
...
function makeTransport(input: {
  ...
  const sessionRegistry = makeHostGatewaySessionRegistry({
    randomId: () => {
  ...
  } as unknown as HostGatewayCredentialsShape;
  const tokenAliases = new Map<string, string>();
  const sessionKeyAliases = new Map<string, string>();
  const leases = new Map<string, HostGatewaySessionLease>();
  const startRuntime = (threadId: string, tokenAlias: string): HostGatewaySessionLease => {
    const lease = acquireHostGatewaySessionLease(
      credentials,
  ...
  const transport = makeHostGatewayMcpTransport({
    credentials,
    snapshotQuery,
    tools: [input.tool],
...
describe("makeHostGatewayMcpTransport exposure policy", () => {
  it.effect(
...
describe("makeHostGatewayMcpTransport cancellation", () => {
  it.effect(
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        let hostCalls = 0;
        const browserWait = makeHostGatewayBrowserTools({
          available: true,
          execute: () => {
            hostCalls += 1;
