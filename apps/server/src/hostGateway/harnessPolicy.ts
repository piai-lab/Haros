 ...
 */
export function takeHarnessOSHarnessPolicyForEngineSession(
  state: HarnessOSHarnessPolicyDeliveryState,
  input: {
...
export function takeHarnessOSHarnessPolicyTextPartForEngineSession(
  state: HarnessOSHarnessPolicyDeliveryState,
  input: {
    readonly scopedGatewayConnectionAvailable: boolean;
  },
): { readonly type: "text"; readonly text: string } | null {
  const text = takeHarnessOSHarnessPolicyForEngineSession(state, input);
