...
async function setup() {
  ...
  const tools = makeHostGatewayDeviceTools({
    manager,
...
describe("HostGateway device tools surface", () => {
...
describe("HostGateway device tool handlers", () => {
...
describe("HostGateway device tools without an approval gate", () => {
  ...
  it("fails closed for a gated engine when no verified approval fact is supplied", async () => {
    ...
    const manager = new DeviceManager({ backend });
    await manager.boot(DEVICE);
    const tool = makeHostGatewayDeviceTools({ manager }).find(
      (entry) => entry.definition.name === "device_tap",
    );
    if (!tool) throw new Error("device_tap is missing");

    const result = await Effect.runPromise(
      tool.handler({ udid: DEVICE, x: 5, y: 5 }, makeContext("codex")),
    );
...
describe("HostGateway device tools auto-attach", () => {
...
describe("HostGateway device tools surface the pane on any interaction", () => {
