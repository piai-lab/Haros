...
export type HostGatewayTargetErrorCode =
  | "engine_unavailable"
  | "model_unavailable"
  | "model_option_unavailable";

export class HostGatewayTargetError extends Error {
  readonly code: HostGatewayTargetErrorCode;
  readonly details?: unknown;

  constructor(code: HostGatewayTargetErrorCode, message: string, details?: unknown) {
    super(message);
    this.name = "HostGatewayTargetError";
    this.code = code;
...
export interface HostGatewayEngineAvailability {
  readonly enabled: boolean;
...
export const HOST_GATEWAY_TARGET_OPTIONS_DESCRIPTION =
...
export type HostGatewayTargetOptionValue = string | number | boolean;

export interface HostGatewayTargetOptionRule {
  readonly key: string;
  readonly valueType: "string" | "number" | "boolean";
  readonly allowedValues: ReadonlyArray<HostGatewayTargetOptionValue>;
  readonly allowedValuesSource: "engine-contract" | "model-discovery";
  readonly allowsCustomValue?: boolean;
}

export interface HostGatewayTargetOptionGuidance {
  readonly primaryOptionKey: string;
  readonly alternativeOptionKeys: ReadonlyArray<string>;
  readonly optionSelectionRule: string;
  readonly engineOptions: ReadonlyArray<HostGatewayTargetOptionRule>;
  readonly optionsByModel: Readonly<Record<string, ReadonlyArray<HostGatewayTargetOptionRule>>>;
  readonly exampleTarget: {
    readonly engine: EngineKind;
    readonly model: string;
    readonly options: Readonly<Record<string, HostGatewayTargetOptionValue>>;
  } | null;
}
...
interface EngineTargetOptionRuleSpec extends Omit<HostGatewayTargetOptionRule, "key"> {
...
function convertDiscoveredOptionValue(
  value: string,
  valueType: HostGatewayTargetOptionRule["valueType"],
): HostGatewayTargetOptionValue | null {
...
function modelTargetOptionRules(
  ...
  const replaceAllowedValues = (
    key: string,
    values: ReadonlyArray<HostGatewayTargetOptionValue>,
    allowEmpty = false,
  ) => {
    ...
    if (descriptor.type === "select") {
      replaceAllowedValues(
        ...
        descriptor.options
          ...
          .filter((value): value is HostGatewayTargetOptionValue => value !== null),
        true,
      );
...
function preferredExampleOptionValue(
  rule: HostGatewayTargetOptionRule,
): HostGatewayTargetOptionValue | null {
  const preferences: ReadonlyArray<HostGatewayTargetOptionValue> =
...
function exampleOptionsForRules(
  ...
  rules: ReadonlyArray<HostGatewayTargetOptionRule>,
): Readonly<Record<string, HostGatewayTargetOptionValue>> {
...
function failUnavailableOption(
  ...
  throw new HostGatewayTargetError(
    "model_option_unavailable",
...
export function resolveHostGatewayTarget(input: {
  ...
  return Effect.gen(function* () {
    ...
    if (catalog.models.length === 0) {
      ...
      try {
        ...
        if (error instanceof HostGatewayTargetError) return yield* Effect.fail(error);
        throw error;
      }
      return input.target;
    }
