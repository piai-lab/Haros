...
describe("HostGateway stdio proxy", () => {
  it("forwards cancellation immediately and lets a later ping bypass a hung request", async () => {
    ...
    try {
      ...
      const scriptPath = await Effect.runPromise(
        ensureHostGatewayStdioProxyScript(stateDir).pipe(Effect.provide(NodeServices.layer)),
      );
      child = spawn(process.execPath, [scriptPath], {
        env: {
          ...process.env,
          HARNESSOS_HOST_GATEWAY_URL: `http://127.0.0.1:${address.port}/mcp`,
          HARNESSOS_HOST_GATEWAY_TOKEN: "test-token",
        },
  ...
  it("exchanges an ambient one-shot bootstrap and keeps the bearer inside the proxy", async () => {
    ...
    try {
      ...
      const providerEnvironment = {
        ...
        HARNESSOS_HOST_GATEWAY_URL: `http://127.0.0.1:${address.port}/mcp`,
        HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN: "one-shot-bootstrap",
      };
      child = spawn(process.execPath, [scriptPath], {
      ...
      const runProviderDescendant = async (): Promise<unknown> => {
        const descendant = spawn(
          ...
          [
            "-e",
            `const url = process.env.HARNESSOS_HOST_GATEWAY_URL + "/bootstrap";
             fetch(url, { method: "POST", headers: { Authorization: "Bearer " + process.env.HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN } })
               .then((response) => process.stdout.write(JSON.stringify({ status: response.status, bearer: process.env.HARNESSOS_HOST_GATEWAY_TOKEN ?? null })))
    ...
    try {
      ...
      child = spawn(process.execPath, [scriptPath], {
        env: {
          PATH: process.env.PATH,
          HARNESSOS_HOST_GATEWAY_URL: `http://127.0.0.1:${address.port}/mcp`,
          HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN: "hung-bootstrap",
        },
        stdio: ["pipe", "pipe", "pipe"],
      });
