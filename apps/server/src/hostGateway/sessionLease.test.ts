...
describe("HostGatewaySessionLease", () => {
  ...
  it("revokes the session before stopping a background child without a parent turn id", async () => {
    ...
    expect(lease.release).toHaveBeenCalledOnce();
    expect(engineInterrupted).toHaveBeenCalledOnce();
  });
  ...
  it("does nothing when no exact turn is available", async () => {
    ...
    await Effect.runPromise(cancelHostGatewayTurn(lease, undefined));
    expect(lease.cancelTurn).not.toHaveBeenCalled();
  });
  ...
  it("keeps replacement runtimes on independent leases", () => {
    ...
    const previous = acquireHostGatewaySessionLease(credentials, threadId, "grok");
    const replacement = acquireHostGatewaySessionLease(credentials, threadId, "grok");
  ...
  it("does not acquire a credential when the gateway layer is absent", () => {
    expect(
      acquireHostGatewaySessionLease(undefined, ThreadId.makeUnsafe("thread-1"), "droid"),
    ).toBeUndefined();
  });
  ...
  it("releases a live lease when the engine exits spontaneously", async () => {
    ...
    await Effect.runPromise(
      startHostGatewaySessionLeaseExitWatcher(lease, Deferred.await(providerExited)),
    );
    expect(revokeSessionToken).not.toHaveBeenCalled();
  ...
  it("releases an untransferred lease when engine startup is interrupted", async () => {
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        const startupFiber = yield* releaseHostGatewaySessionLeaseOnInterrupt(
          lease,
