...
import { HOST_GATEWAY_MCP_MAX_BODY_BYTES, hostGatewayRouteLayer } from "./httpRoute.ts";
...
async function withGatewayServer(
  ...
  try {
    ...
    const gateway: HostGatewayShape = {
    ...
    await Effect.runPromise(
      Scope.provide(
        Effect.gen(function* () {
          ...
          const httpApp = yield* HttpRouter.toHttpEffect(hostGatewayRouteLayer);
          yield* httpServer.serve(httpApp);
        }).pipe(
          Effect.provide(
            Layer.mergeAll(
              Layer.succeed(HostGateway, gateway),
              Layer.succeed(HostGatewayCredentials, credentials),
              NodeServices.layer,
            ),
          ),
        ),
    ...
    if (!address || typeof address !== "object") {
      throw new Error("Expected HostGateway test server to expose an address");
    }
...
describe("hostGatewayRouteLayer", () => {
  it("authenticates before reading the body and enforces the 1 MiB limit", async () => {
    await withGatewayServer(async ({ origin, handledBodies }) => {
      const oversizedBody = "x".repeat(HOST_GATEWAY_MCP_MAX_BODY_BYTES + 1);
