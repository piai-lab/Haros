...
import {
  ...
  type HostGatewayOperationRepositoryShape,
  type ReserveHostGatewayOperationResult,
...
interface OperationRow {
  ...
  readonly status: HostGatewayOperationRecord["status"];
...
const mapSqlError = (operation: string) => (cause: unknown) =>
  new Error(`HostGateway operation repository failed during ${operation}.`, { cause });

export const makeHostGatewayOperationRepository = Effect.gen(function* () {
  ...
  const reserve: HostGatewayOperationRepositoryShape["reserve"] = (input) =>
    sql
      .withTransaction(
        Effect.gen(function* () {
          ...
          let kind: ReserveHostGatewayOperationResult["kind"];
          ...
          return { kind, operation } satisfies ReserveHostGatewayOperationResult;
        }),
      )
      .pipe(Effect.mapError(mapSqlError("reserve")));

  const markDispatching: HostGatewayOperationRepositoryShape["markDispatching"] = (input) =>
  ...
  const recordWorktreeCreated: HostGatewayOperationRepositoryShape["recordWorktreeCreated"] = (
    input,
  ) =>
    sql
      .withTransaction(
  ...
  const complete: HostGatewayOperationRepositoryShape["complete"] = (input) =>
    sql
      .withTransaction(
        Effect.gen(function* () {
          yield* sql`
  ...
  const markCompensating: HostGatewayOperationRepositoryShape["markCompensating"] = (input) =>
    sql`
      UPDATE host_gateway_operations
  ...
  const recordCompensationFailure: HostGatewayOperationRepositoryShape["recordCompensationFailure"] =
    (input) =>
      sql`
        UPDATE host_gateway_operations
  ...
  const fail: HostGatewayOperationRepositoryShape["fail"] = (input) =>
    sql
      .withTransaction(
        Effect.gen(function* () {
          yield* sql`
  ...
  const getById: HostGatewayOperationRepositoryShape["getById"] = (operationId) =>
    sql<OperationRow>`
      SELECT
        operation_id AS "operationId",
        caller_thread_id AS "callerThreadId",
  ...
  const getByScope: HostGatewayOperationRepositoryShape["getByScope"] = (input) =>
    readByScope(input).pipe(
      Effect.map((rows) => rows[0] ?? null),
      Effect.mapError(mapSqlError("getByScope")),
    );

  const listNonTerminal: HostGatewayOperationRepositoryShape["listNonTerminal"] = () =>
    sql<OperationRow>`
      SELECT
        operation_id AS "operationId",
        caller_thread_id AS "callerThreadId",
  ...
  return {
    ...
    getByScope,
    listNonTerminal,
  } satisfies HostGatewayOperationRepositoryShape;
});

export const HostGatewayOperationRepositoryLive = Layer.effect(
  HostGatewayOperationRepository,
  makeHostGatewayOperationRepository,
);
