...
import { HostGateway } from "../Services/HostGateway.ts";
import { HostGatewayCredentials } from "../Services/HostGatewayCredentials.ts";
import {
  HostGatewayOperationRepository,
  type HostGatewayOperationRecord,
} from "../Services/HostGatewayOperationRepository.ts";
import { HostGatewayLive } from "./HostGateway.ts";
...
interface GatewayHarness {
  ...
  readonly setEngineStatuses: (statuses: ReadonlyArray<ServerEngineStatus>) => void;
...
function makeHarnessLayer(
  ...
  options: {
    ...
    readonly interruptedOperations?: ReadonlyArray<HostGatewayOperationRecord>;
    readonly engineStatuses?: ReadonlyArray<ServerEngineStatus>;
    ...
    readonly engineRuntimeEvents?: ReadonlyArray<PersistedEngineRuntimeEvent>;
    readonly operationalDiagnostics?: ReadonlyArray<OperationalDiagnostic>;
    readonly engineDeliveryBlockers?: ReadonlyArray<EngineBlockingDeliveryEvidence>;
  ...
  const credentialsLayer = Layer.succeed(HostGatewayCredentials, {
  ...
  const deviceServiceLayer = Layer.succeed(DeviceService, {
    supported: true,
  ...
  const eventDeliveriesLayer = Layer.succeed(OrchestrationEventDeliveryRepository, {
    listBlockingDeliveries: (input: { threadId?: string; limit: number }) =>
      Effect.succeed(
        (options.engineDeliveryBlockers ?? [])
  ...
  const engineDiscoveryLayer = Layer.succeed(EngineDiscoveryService, {
    listModels: ({ engine }: { engine: string }) => {
      const modelsByEngine: Record<string, ReadonlyArray<Record<string, unknown>>> = {
        ...
        grok: [{ slug: DEFAULT_MODEL_BY_ENGINE.grok, name: "Grok 4.6" }],
  ...
  const engineHealthLayer = Layer.succeed(EngineHealth, {
    getStatuses: Effect.sync(() => engineStatuses),
    refresh: Effect.sync(() => engineStatuses),
    updateEngine: () => Effect.die("Engine updates are not used by gateway tests."),
    streamChanges: Stream.empty,
  } as unknown as (typeof EngineHealth)["Service"]);

  const operationsByScope = new Map<string, HostGatewayOperationRecord>();
  ...
  const operationLayer = Layer.succeed(HostGatewayOperationRepository, {
    reserve: (input: {
      operationId: string;
      ...
      Effect.gen(function* () {
        ...
        const operation: HostGatewayOperationRecord = {
          ...input,
          status: "reserved",
          resultJson: null,
          errorJson: null,
  ...
  const gatewayLayer = HostGatewayLive.pipe(
  ...
  const makeHarness = Effect.gen(function* () {
    const gateway = yield* HostGateway;
    ...
    return {
      ...
      getBuiltInToolGroups: () => gateway.getBuiltInToolGroups,
      callTool,
      postRaw,
...
describe("HostGateway", () => {
  ...
  it.effect("keeps the canonical built-in catalog inventory explicit in one place", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        { tasks: 12, diagnostics: 4, goals: 1, automations: 7, browser: 25, device: 12 },
      );
  ...
  it.effect("combines durable events, delivery blockers, and stream incidents in diagnosis", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.lengthOf(payload.engineDeliveryBlockers as Array<unknown>, 1);
  ...
  it.effect("creates a standalone cross-engine thread and dispatches the initial turn", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (create.type === "thread.create") {
        ...
        assert.equal(create.engineSelection.model, DEFAULT_MODEL_BY_ENGINE.grok);
  ...
  it.effect("resolves an explicit HEAD from the caller's linked worktree", () => {
    ...
    return Effect.gen(function* () {
      ...
      const resolution = harness.gitExecutions.find(
        (entry) => entry.operation === "HostGateway.resolveWorktreeRef",
      );
  ...
  it.effect("resolves local refs containing pull path segments as Git revisions", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepEqual(
        harness.gitExecutions.find((entry) => entry.operation === "HostGateway.resolveWorktreeRef")
          ?.args,
  ...
  it.effect("compensates deterministic interrupted operations during gateway startup", () => {
    const interrupted: HostGatewayOperationRecord = {
  ...
  it.effect("does not compensate a reserved operation that never began dispatch", () => {
    const reserved: HostGatewayOperationRecord = {
