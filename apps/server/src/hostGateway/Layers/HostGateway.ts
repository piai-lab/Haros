 ...
 * state lives here.
 *
 * @module hostGateway/Layers/HostGateway
 */
...
import { HostGateway, type HostGatewayShape } from "../Services/HostGateway.ts";
import { HostGatewayCredentials } from "../Services/HostGatewayCredentials.ts";
import { HostGatewayOperationRepository } from "../Services/HostGatewayOperationRepository.ts";
...
import { makeHostGatewayMcpTransport } from "../mcpTransport.ts";
import { recoverInterruptedHostGatewayOperations } from "../startupRecovery.ts";
import { makeCreateThreadsHandler } from "../creationCoordinator.ts";
import { makeHostGatewayAutomationTools } from "../automationTools.ts";
import { makeHostGatewayBrowserTools } from "../browserTools.ts";
import { makeHostGatewayDeviceTools } from "../deviceTools.ts";
...
import { renderHostGatewayMcpInstructions } from "../harnessPolicy.ts";
...
function renderHostGatewayInstructions(tools: ReadonlyArray<ToolEntry>): string {
  const groups = Array.from(
    new Set(
      tools.flatMap((tool) =>
        tool.provenance === "host-gateway" && tool.group !== undefined
          ? [tool.group as BuiltInToolGroupId]
          : [],
      ),
    ),
  );
  return renderHostGatewayMcpInstructions(groups);
}
...
export const makeHostGateway = Effect.gen(function* () {
  const credentials = yield* HostGatewayCredentials;
          ...
          yield* Effect.forkDetach(
            pruneProjectedArchivedManagedWorktrees({
              ...
              Effect.catchCause((cause) =>
                Effect.logWarning("HostGateway managed worktree retention failed", {
                  cause: String(cause),
                }),
              ),
            ),
  ...
  const automationTools = makeHostGatewayAutomationTools({
    automationService,
    requireThreadShell,
  ...
  const browserTools = makeHostGatewayBrowserTools(browserAutomationHost, {
  ...
  const tools = makeHostGatewayToolCatalog([
    tagHostGatewayTools({
      group: "tasks",
      available: true,
      tools: [
    ...
    tagHostGatewayTools({
      ...
      tools:
        deviceService?.supported === true
          ? makeHostGatewayDeviceTools({ manager: deviceService.manager })
          : [],
    }),
  ]);
    ...
    Effect.all([
      ...
      Effect.map(([settings, project]) =>
        Option.match(project, {
          onNone: () => [],
          onSome: (entry) =>
            entry.kind ? exposedHostGatewayToolsForProjectKind(tools, settings, entry.kind) : [],
        }),
      ),
    );
  return {
    ...
    handleMcpPost: makeHostGatewayMcpTransport({
      credentials,
      snapshotQuery,
      tools,
      loadExposedTools,
      instructions: renderHostGatewayInstructions,
      requireThreadShell,
    }),
  } satisfies HostGatewayShape;
});

export const HostGatewayLive = Layer.effect(HostGateway, makeHostGateway);
