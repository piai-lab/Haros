...
import { HostGatewayOperationRepositoryLive } from "./HostGatewayOperationRepository.ts";

const layer = it.layer(
  HostGatewayOperationRepositoryLive.pipe(Layer.provideMerge(SqlitePersistenceMemory)),
);
...
layer("HostGatewayOperationRepository", (it) => {
  it.effect("reserves once and replays the same operation", () =>
    Effect.gen(function* () {
      const repository = yield* HostGatewayOperationRepository;
