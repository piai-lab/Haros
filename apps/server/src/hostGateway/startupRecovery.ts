 ...
 */
export function recoverInterruptedHostGatewayOperations(input: {
  readonly operationRepository: Pick<
    HostGatewayOperationRepositoryShape,
    "markCompensating" | "recordCompensationFailure" | "fail"
  > & {
    readonly listNonTerminal: () => Effect.Effect<
      ReadonlyArray<Pick<HostGatewayOperationRecord, "operationId" | "status" | "planJson">>,
      Error
    >;
  };
  ...
  return Effect.gen(function* () {
    const interruptedOperations = yield* input.operationRepository.listNonTerminal().pipe(
      Effect.catch((error) =>
        Effect.logWarning("HostGateway recovery could not list interrupted operations", {
      ...
      (operation) =>
        Effect.gen(function* () {
          ...
          if (recoveryErrors.length > 0) {
            ...
            yield* Effect.logWarning("HostGateway recovery remains incomplete", {
              operationId: operation.operationId,
              errors: recoveryErrors,
            });
            return;
              ...
              yield* input.operationRepository
                ...
                .pipe(
                  Effect.catch((persistenceError) =>
                    Effect.logWarning("HostGateway recovery status could not be persisted", {
                      operationId: operation.operationId,
                      error: errorText(persistenceError),
                    }),
                  ),
                );
              yield* Effect.logWarning("HostGateway recovery failed", {
                operationId: operation.operationId,
                error: detail,
              });
            }),
