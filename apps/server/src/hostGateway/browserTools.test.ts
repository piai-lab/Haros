...
import { makeHostGatewayBrowserTools, normalizeGatewayBrowserArguments } from "./browserTools.ts";
...
describe("HostGateway browser tools", () => {
  ...
  it("rejects a bare old ref after a new snapshot and accepts the explicit current ref", async () => {
    ...
    const tools = makeHostGatewayBrowserTools({ available: true, execute });
  ...
  it("leaves ambiguous aliases invalid instead of guessing", async () => {
    const execute = vi.fn();
    const tools = makeHostGatewayBrowserTools({ available: true, execute: execute as never });
    const type = tools.find((tool) => tool.definition.name === "browser_type")!;
  ...
  it("publishes the complete canonical visible-browser catalogue", () => {
    ...
    const tools = makeHostGatewayBrowserTools(host);
  ...
  it("supplies stable request-scoped idempotency keys for natural open and navigate calls", async () => {
    ...
    expect(keys[0]).toMatch(/^harnessos-mcp-[a-f0-9]{40}$/u);
