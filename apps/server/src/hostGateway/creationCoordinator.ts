...
interface CreationCoordinatorDependencies {
  ...
  readonly operationRepository: HostGatewayOperationRepositoryShape;
  readonly externalMcpRepository?: ExternalMcpRepositoryShape;
  readonly serverConfig: ServerConfigShape;
  readonly loadProviderAvailabilities: Effect.Effect<
    ReadonlyMap<EngineKind, HostGatewayEngineAvailability>,
    unknown
  >;
  readonly getEngineExecutionCapabilities: (
...
type CreationOperationRecord = HostGatewayOperationRecord | ExternalMcpOperationRecord;
...
interface CreationOperationStore {
  ...
  readonly reserve: (input: {
    ...
    Error
  >;
  readonly markDispatching: HostGatewayOperationRepositoryShape["markDispatching"];
  readonly recordWorktreeCreated: HostGatewayOperationRepositoryShape["recordWorktreeCreated"];
  readonly markCompensating: HostGatewayOperationRepositoryShape["markCompensating"];
  readonly recordCompensationFailure: HostGatewayOperationRepositoryShape["recordCompensationFailure"];
  readonly complete: HostGatewayOperationRepositoryShape["complete"];
  readonly fail: HostGatewayOperationRepositoryShape["fail"];
...
export const makeCreateThreadsHandler = Effect.fn(function* (
  ...
  const {
    ...
    getEngineExecutionCapabilities,
    requireThreadShell,
  } = dependencies;
  ...
  const appendThreadCreationRecap = (input: {
    ...
    return orchestrationEngine
      ...
      .pipe(
        Effect.catch((error) =>
          Effect.logWarning("HostGateway could not append thread creation recap", {
            operationId: input.result.operationId,
    ...
    return Effect.gen(function* () {
      ...
      const prepared = yield* Effect.forEach(input.threads, (spec, index) =>
        Effect.gen(function* () {
          ...
          const target = yield* resolveHostGatewayTarget({
            target: spec.target,
          ...
          const executionCapabilities = yield* getEngineExecutionCapabilities(target).pipe(
            Effect.mapError(
              () =>
                new ToolInputError(
          ...
          if (environment === "worktree") {
            ...
            worktreeRef = yield* (
              pullRequest === null
                ? git.execute({
                    operation: "HostGateway.resolveWorktreeRef",
                    cwd: sourceCwd,
            ...
            const sourceHead = yield* git
              .execute({
                operation: "HostGateway.resolveWorktreeCopySource",
                cwd: sourceCwd,
      ...
      const compensateClaimedOperation = (cause: Cause.Cause<unknown>) =>
        Effect.gen(function* () {
          ...
          yield* operationStore.markCompensating({ operationId, now: gatewayIsoNow() }).pipe(
            Effect.catch((error) =>
              Effect.logWarning("HostGateway could not persist compensating status", {
                operationId,
                error: errorText(error),
              }),
            ),
          ...
          yield* operationStore.markTaskStatus(operationId, "failed").pipe(
            Effect.catch((error) =>
              Effect.logWarning("HostGateway could not mark external task failed", {
                operationId,
                error: errorText(error),
              }),
            ),
          ...
          if (compensationErrors.length > 0) {
            yield* operationStore
              ...
              .pipe(
                Effect.catch((error) =>
                  Effect.logWarning("HostGateway compensation status could not be persisted", {
                    operationId,
                    error: errorText(error),
                  }),
                ),
              );
            yield* Effect.logWarning("HostGateway compensation remains pending", {
              operationId,
          ...
          if (statusFailure !== null) {
            ...
            yield* operationStore
              ...
              .pipe(
                Effect.catch((error) =>
                  Effect.logWarning("HostGateway fallback status could not be persisted", {
                    operationId,
                    error: errorText(error),
                  }),
                ),
      ...
      Effect.catch((error) =>
        Effect.succeed(
          error instanceof GatewayToolError || error instanceof HostGatewayTargetError
