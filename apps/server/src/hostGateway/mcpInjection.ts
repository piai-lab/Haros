 ...
 *
 * @module hostGateway/mcpInjection
 */
...
import type {
  ...
  HostGatewayStdioProxySpawn,
} from "./Services/HostGatewayCredentials.ts";

export const HARNESSOS_MCP_SERVER_NAME = "harnessos";
export const HARNESSOS_HOST_GATEWAY_TOKEN_ENV = "HARNESSOS_HOST_GATEWAY_TOKEN";
export const HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN_ENV = "HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN";
export const HARNESSOS_HOST_GATEWAY_URL_ENV = "HARNESSOS_HOST_GATEWAY_URL";

function authorizationHeader(connection: HostGatewayMcpConnection): string {
...
export function buildCodexMcpConfigToml(endpointUrl: string): string {
  return [
    ...
    `bearer_token_env_var = ${JSON.stringify(HARNESSOS_HOST_GATEWAY_TOKEN_ENV)}`,
    "",
    "[shell_environment_policy]",
    `exclude = [${JSON.stringify(HARNESSOS_HOST_GATEWAY_TOKEN_ENV)}]`,
  ].join("\n");
}
...
export function buildOpenCodeMcpServer(
  connection: HostGatewayMcpConnection,
...
export interface HostGatewayMcpToolDescriptor {
...
export type HostGatewayMcpFetch = (
...
function readHostGatewayToolMetadata(value: Record<string, unknown>): {
  readonly group?: BuiltInToolGroupId;
  readonly provenance?: "host-gateway";
} {
  ...
  return { group: group as BuiltInToolGroupId, provenance: "host-gateway" };
}

async function postHostGatewayJsonRpc(input: {
...
export async function listHostGatewayMcpTools(input: {
  readonly connection: HostGatewayMcpConnection;
  readonly fetch?: HostGatewayMcpFetch;
  readonly signal?: AbortSignal;
}): Promise<ReadonlyArray<HostGatewayMcpToolDescriptor>> {
  const result = await postHostGatewayJsonRpc({
    ...input,
    method: "tools/list",
  });
  ...
  return result.tools.map((value) => {
    ...
    return metadata.group === undefined
      ...
      : {
          ...
          provenance: "host-gateway" as const,
        };
  });
}

/** Derive the enabled/available group snapshot from one authenticated tools/list response. */
export function hostGatewayGroupsFromToolDescriptors(
  tools: ReadonlyArray<HostGatewayMcpToolDescriptor>,
): ReadonlyArray<BuiltInToolGroupId> {
  const present = new Set(
    tools.flatMap((tool) =>
      tool.provenance === "host-gateway" && tool.group !== undefined ? [tool.group] : [],
    ),
  );
...
export async function loadHostGatewayHarnessGroups(input: {
  readonly connection: HostGatewayMcpConnection;
  readonly fetch?: HostGatewayMcpFetch;
  readonly signal?: AbortSignal;
}): Promise<ReadonlyArray<BuiltInToolGroupId>> {
  return hostGatewayGroupsFromToolDescriptors(await listHostGatewayMcpTools(input));
}

/** Invoke the canonical gateway dispatcher through its authenticated MCP route. */
export function callHostGatewayMcpTool(input: {
  readonly connection: HostGatewayMcpConnection;
  readonly name: string;
...
export type AcpStdioProxySpawn = HostGatewayStdioProxySpawn;
  ...
  return {
    mcpServers: {
      [HARNESSOS_MCP_SERVER_NAME]: {
        ...
        env: {
          [HARNESSOS_HOST_GATEWAY_URL_ENV]: `$${HARNESSOS_HOST_GATEWAY_URL_ENV}`,
          [HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN_ENV]: `$${HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN_ENV}`,
  ...
  return [
    {
      ...
      env: [
        { name: HARNESSOS_HOST_GATEWAY_URL_ENV, value: input.connection.url },
        { name: HARNESSOS_HOST_GATEWAY_TOKEN_ENV, value: input.connection.bearerToken },
      ],
    },
  ];
}
