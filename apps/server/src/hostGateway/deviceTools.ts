/**
 * HostGateway device tools - the agent's control surface for the device pane.
 *
 ...
 *
 * @module hostGateway/deviceTools
 */
import {
  ...
  return {
    ...mcpToolResultJson({
      error: {
        ...
        message: `${name} requires explicit user approval, but this invocation carries no verifiable approval receipt. The action was refused before it ran; ask the user to perform it from the device pane.`,
      },
    }),
    isError: true,
  };
...
export interface HostGatewayDeviceToolsOptions {
   ...
   */
  readonly isExplicitlyApproved?: (input: {
...
export function makeHostGatewayDeviceTools(
  options: HostGatewayDeviceToolsOptions,
): ReadonlyArray<ToolEntry> {
  const { manager } = options;
  const isExplicitlyApproved = options.isExplicitlyApproved ?? (() => false);
  ...
  const tools: ToolEntry[] = [
    {
      ...
      definition: {
        name: "device_list",
        description:
          "List iOS simulators Haros can drive, with their runtime, boot state, and who booted them. Call this before any other device_* tool to get a udid.",
        inputSchema: {
          type: "object",
          properties: {
            includeShutdown: {
    ...
    },
    {
      ...
      definition: {
        name: "device_boot",
        description:
          'Boot a simulator. Haros caps the number of simulators it boots itself; past the cap this returns kind "boot-limit-reached" with the devices to shut down, which is a refusal to relay to the user, not an error to retry.',
        inputSchema: {
          type: "object",
    ...
    },
    {
      ...
      definition: {
        name: "device_install",
        description:
          "Install a built .app bundle on a booted simulator. Haros never builds the app: run your own build first and pass the resulting bundle path.",
        inputSchema: {
          type: "object",
          properties: {
            udid: UDID_PROPERTY,
