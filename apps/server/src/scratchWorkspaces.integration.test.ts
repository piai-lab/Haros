...
describe("ensureIsolatedScratchWorkspace", () => {
  ...
  it("securely reuses a matching legacy workspace without breaking historical paths", () => {
    ...
    expect(readFileSync(path.join(legacyWorkspace, "resume.txt"), "utf8")).toBe("preserved");
    if (process.platform !== "win32") {
      if ((statSync(legacyRoot).mode & 0o1000) !== 0) {
        expect(resumedWorkspace).toBe(legacyWorkspace);
      } else {
        expect(resumedWorkspace.startsWith(`${migratedRoot}${path.sep}`)).toBe(true);
      }
