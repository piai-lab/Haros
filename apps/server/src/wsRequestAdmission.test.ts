import { ORCHESTRATION_WS_METHODS, WS_METHODS } from "@harnessos/contracts";
...
describe("WsRequestAdmission", () => {
  ...
  it("admits the bounded local prompt snapshot while both expensive-read leases are occupied", async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const promptSnapshot = yield* admission.acquire(1, WS_METHODS.serverGetSettings);
  ...
  it("reserves engine discovery capacity during cold shell restoration", async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        const admission = yield* makeWsRequestAdmission;
        const shellSnapshot = yield* admission.acquire(1, ORCHESTRATION_WS_METHODS.getSnapshot);
        const threadSnapshot = yield* admission.acquire(
          1,
        ...
        yield* admission.release(modelCatalog);
        expect(yield* admission.snapshot).toMatchObject({
          active: 0,
          admittedTotal: 3,
          releasedTotal: 3,
