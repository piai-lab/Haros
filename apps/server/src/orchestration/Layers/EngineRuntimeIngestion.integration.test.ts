...
import {
  ...
  EngineRuntimeIngestionLive,
  selectProviderRuntimeJournalStream,
} from "./EngineRuntimeIngestion.ts";
import {
...
function createProviderServiceHarness() {
  ...
  const service: EngineServiceShape = {
    ...
    getCapabilities: (engine) =>
      Effect.succeed({
        ...
        supportsLiveTurnDiffPatch: engine === "codex",
      }),
  ...
  const setSession = (session: EngineSession): void => {
  ...
  const emit = (event: LegacyProviderRuntimeEvent): void => {
    ...
    Effect.runSync(
      PubSub.publish(runtimeEventPubSub, canonicalEvent as unknown as EngineRuntimeEvent),
    );
  };
...
async function waitForThread(
  ...
  predicate: (thread: EngineRuntimeTestThread) => boolean,
  timeoutMs = 2000,
  threadId: ThreadId = asThreadId("thread-1"),
) {
  const deadline = Date.now() + timeoutMs;
  const poll = async (): Promise<EngineRuntimeTestThread> => {
...
type EngineRuntimeTestReadModel = OrchestrationReadModel;
type EngineRuntimeTestThread = EngineRuntimeTestReadModel["threads"][number];
type EngineRuntimeTestMessage = EngineRuntimeTestThread["messages"][number];
type EngineRuntimeTestProposedPlan = EngineRuntimeTestThread["proposedPlans"][number];
type EngineRuntimeTestActivity = EngineRuntimeTestThread["activities"][number];
type EngineRuntimeTestCheckpoint = EngineRuntimeTestThread["checkpoints"][number];
...
describe("EngineRuntimeIngestion", () => {
  ...
  it("uses an already-persisted runtime stream without appending the event again", async () => {
    const event: EngineRuntimeEvent = {
  ...
  async function createHarness(options?: {
    ...
    const ingestion = await runtime.runPromise(Effect.service(EngineRuntimeIngestionService));
    ...
    await Effect.runPromise(
      engine.dispatch({
        type: "project.create",
        commandId: CommandId.makeUnsafe("cmd-engine-project-create"),
    ...
    return {
      engine,
      emit: engineHarness.emit,
      setEngineSession: engineHarness.setSession,
      runtimeProjectionLeaseCalls: engineHarness.runtimeProjectionLeaseCalls,
      listSessionsObservedWithinRuntimeProjectionLease:
        engineHarness.listSessionsObservedWithinRuntimeProjectionLease,
      drain,
  ...
  it("fences delayed replacement rows against the restored engine binding", async () => {
  ...
  it("REL-01C gate: replays output persisted before subscription without duplicate acceptance", async () => {
    ...
    const event: EngineRuntimeEvent = {
      ...
      threadId: asThreadId("thread-1"),
      payload: {
        message: "Recovered durable engine output",
      },
    };
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        commandId: CommandId.makeUnsafe(
          `engine:${event.eventId}:thread-activity-append:thread-1:${event.eventId}`,
        ),
        threadId: asThreadId("thread-1"),
        activity: {
          id: event.eventId,
          ...
          payload: {
            message: "Recovered durable engine output",
            detail: "Recovered durable engine output",
          },
          turnId: null,
        },
        createdAt: event.createdAt,
  ...
  it("orders buffered assistant segments around equal-timestamp tool rows by journal sequence", async () => {
    ...
    const push = (event: EngineRuntimeEvent) =>
  ...
  it("quarantines a command identity collision without blocking later assistant output", async () => {
    ...
    const collisionEvent: EngineRuntimeEvent = {
      type: "runtime.warning",
  ...
  it("quarantines a previously rejected command without blocking later assistant output", async () => {
    ...
    const rejectedEvent: EngineRuntimeEvent = {
      type: "runtime.warning",
    ...
    const rejectedCommandId = CommandId.makeUnsafe(
      `engine:${rejectedEvent.eventId}:thread-activity-append:${lateThreadId}:runtime.warning:${rejectedEvent.eventId}`,
    );
  ...
  it("REL-01C gate: rebuilds accepted buffered output before a terminal event", async () => {
    ...
    const bufferedEvent: EngineRuntimeEvent = {
      type: "content.delta",
  ...
  it("REL-01C gate: does not re-buffer accepted streaming output during rebuild", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        commandId: CommandId.makeUnsafe(`engine:${event.eventId}:assistant-delta:${messageId}`),
        threadId: asThreadId("thread-1"),
        messageId,
        delta: "streamed once",
        turnId,
  ...
  it("applies engine session.state.changed transitions directly", async () => {
    const harness = await createHarness();
    ...
    thread = await waitForThread(
      ...
      (entry) =>
        entry.session?.status === "error" &&
        entry.session?.activeTurnId === null &&
        entry.session?.lastError === "engine crashed",
    );
    expect(thread.session?.status).toBe("error");
    expect(thread.session?.lastError).toBe("engine crashed");
  ...
  it("clears active turn state when a engine session reports ready", async () => {
    const harness = await createHarness();
  ...
  it("settles orphaned pending interactions when a engine session (re)starts", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    await waitForThread(
      ...
      (thread) =>
        ...
        thread.messages.some(
          (message: EngineRuntimeTestMessage) =>
            message.id === "assistant:item-stop-aborted" && message.streaming === false,
        ),
    );
  });

  it("projects a turn/completed engine notification with interrupted state as interrupted", async () => {
    const harness = await createHarness();
  ...
  it("does not append an image viewed by the engine to the assistant reply", async () => {
    const harness = await createHarness();
  ...
  it("prefers a persisted Studio copy over its engine-home image source", () => {
    expect(
      collectPersistedGeneratedImagePaths([
        {
  ...
  it("does not re-emit message-sent events when the same image_generation completion replays", async () => {
    ...
    // Replay the same image_generation_end event with a fresh eventId (engine would use a
  ...
  it("accepts claude turn lifecycle when seeded thread id is a synthetic placeholder", async () => {
    ...
    await harness.bindLegacyRuntimeSource("claude");
  ...
  it("ignores auxiliary turn completions from a different engine thread", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
  ...
  it("maps canonical content delta/item completed into finalized assistant messages", async () => {
    ...
    const message = thread.messages.find(
      (entry: EngineRuntimeTestMessage) => entry.id === "assistant:item-1",
    );
  ...
  it("does not project reasoning content deltas into transcript work rows", async () => {
    ...
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) => activity.id === "evt-reasoning-turn-completed",
      ),
    );

    expect(
      thread.activities.some(
        (activity: EngineRuntimeTestActivity) => activity.id === "evt-reasoning-delta",
      ),
    ).toBe(false);
  ...
  it.each([
    ...
    async (engine, streamKind) => {
      const harness = await createHarness();
      if (engine !== "codex") {
        await harness.bindLegacyRuntimeSource(engine);
      }
      const createdAt = new Date().toISOString();
      const threadId = asThreadId("thread-1");
      const turnId = asTurnId(`turn-${engine}-streaming-reasoning`);
      const itemId = asItemId(`${engine}-streaming-reasoning`);
      const baseEvent = { engine, createdAt, threadId, turnId, itemId };

      harness.emit({
        ...baseEvent,
        type: "content.delta",
        eventId: asEventId(`evt-${engine}-streaming-reasoning-1`),
      ...
      expect(firstActivity?.id).toMatch(
        new RegExp(
          `^engine-reasoning:thread-1:${engine}-streaming-reasoning:turn:${turnId}:segment:`,
        ),
      );
      ...
      harness.emit({
        ...baseEvent,
        type: "content.delta",
        eventId: asEventId(`evt-${engine}-streaming-reasoning-2`),
        payload: { streamKind, summaryIndex: 0, delta: " grows" },
      });
      harness.emit({
        ...baseEvent,
        type: "content.delta",
        eventId: asEventId(`evt-${engine}-streaming-reasoning-3`),
      ...
      harness.emit({
        ...baseEvent,
        type: "item.completed",
        eventId: asEventId(`evt-${engine}-streaming-reasoning-completed`),
  ...
  it("settles public reasoning after an authoritative interrupt retires its runtime generation", async () => {
    ...
    await Effect.runPromise(
      harness.providerSessionDirectory.upsert({
        ...
        runtimePayload: {
          activeTurnId: null,
          lastRuntimeEvent: ENGINE_INTERRUPT_RUNTIME_FENCED_EVENT,
        },
      }),
    );
    harness.emit({
      type: "turn.aborted",
      eventId: asEventId(`${ENGINE_INTERRUPT_EVENT_ID_PREFIX}fixture`),
      engine: "codex",
      createdAt: new Date().toISOString(),
      threadId,
      turnId,
  ...
  it("streams Codex reasoning under one stable segment id and completes it in place", async () => {
    ...
    const stableActivityIdPrefix =
      "engine-reasoning:thread-1:reasoning-1:turn:turn-reasoning:segment:";
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some((activity: EngineRuntimeTestActivity) => {
        if (
    ...
    const reasoningActivities = thread.activities.filter((activity: EngineRuntimeTestActivity) =>
      activity.id.startsWith(stableActivityIdPrefix),
    );
  ...
  it("projects narrated Antigravity planner steps as completed reasoning", async () => {
    ...
    const stableActivityIdPrefix =
      "engine-reasoning:thread-1:antigravity-reasoning-1:turn:turn-antigravity-reasoning:segment:";
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) =>
          activity.id.startsWith(stableActivityIdPrefix) && activity.kind === "reasoning.completed",
      ),
    );

    expect(
      thread.activities.filter((activity: EngineRuntimeTestActivity) =>
        activity.id.startsWith(stableActivityIdPrefix),
      ),
    ).toEqual([
      expect.objectContaining({
  ...
  it("buffers Codex summary deltas into one completed reasoning activity", async () => {
    ...
    const stableActivityIdPrefix =
      "engine-reasoning:thread-1:reasoning-buffered-1:turn:turn-buffered-reasoning:segment:";
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) =>
          activity.id.startsWith(stableActivityIdPrefix) && activity.kind === "reasoning.completed",
  ...
  it.each(["oa", "pi"] as const)(
    ...
    async (engine) => {
      ...
      const now = new Date().toISOString();
      await harness.bindLegacyRuntimeSource(engine);
      const itemId = `${engine}-reasoning-buffered-1`;
      const baseEvent = {
        engine,
        createdAt: now,
        threadId: asThreadId("thread-1"),
        turnId: asTurnId(`turn-${engine}-reasoning`),
      ...
      harness.emit({
        ...baseEvent,
        type: "content.delta",
        eventId: asEventId(`evt-${engine}-reasoning-delta-1`),
        payload: {
          streamKind: "reasoning_text",
          contentIndex: 0,
          delta: "Inspect the ",
      ...
      harness.emit({
        ...baseEvent,
        type: "content.delta",
        eventId: asEventId(`evt-${engine}-reasoning-delta-2`),
        payload: {
          streamKind: "reasoning_text",
          contentIndex: 0,
      ...
      harness.emit({
        ...baseEvent,
        type: "item.completed",
        eventId: asEventId(`evt-${engine}-reasoning-completed`),
        payload: {
          itemType: "reasoning",
          status: "completed",
          title: "Reasoning",
      ...
      const stableActivityIdPrefix = `engine-reasoning:thread-1:${itemId}:turn:turn-${engine}-reasoning:segment:`;
      const thread = await waitForThread(harness.engine, (entry) =>
        entry.activities.some(
          (activity: EngineRuntimeTestActivity) =>
            activity.id.startsWith(stableActivityIdPrefix) &&
      ...
      expect(
        ...
        expect.objectContaining({
          ...
          payload: expect.objectContaining({
            ...
            data: {
              toolCallId: expect.stringMatching(
                new RegExp(`^${itemId}:turn:turn-${engine}-reasoning:segment:`),
              ),
            },
          }),
        }),
  ...
  it("settles buffered Codex reasoning when a turn is aborted", async () => {
    ...
    harness.emit({
      ...
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-aborted-reasoning"),
      payload: { state: "interrupted", reason: "engine aborted" },
    });

    const stableActivityIdPrefix =
      "engine-reasoning:thread-1:reasoning-aborted-1:turn:turn-aborted-reasoning:segment:";
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) =>
          activity.id.startsWith(stableActivityIdPrefix) &&
    ...
    expect(
      thread.activities.find((activity: EngineRuntimeTestActivity) =>
        activity.id.startsWith(stableActivityIdPrefix),
      ),
    ).toMatchObject({
      tone: "error",
  ...
  it("marks buffered reasoning truncated when a engine exceeds the supported part count", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    const threadId = asThreadId("thread-1");
    ...
    harness.emit({
      ...
      payload: {
        streamKind: "reasoning_summary_text",
        summaryIndex: 0,
        delta: "Visible engine reasoning",
      },
    });
    harness.emit({
      type: "content.delta",
    ...
    const stableActivityIdPrefix = `engine-reasoning:${threadId}:${itemId}:turn:${turnId}:segment:`;
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) =>
          activity.id.startsWith(stableActivityIdPrefix) &&
    ...
    expect(
      ...
      payload: {
        detail: "Visible engine reasoning",
        data: {
          toolCallId: expect.stringMatching(
  ...
  it("marks buffered Codex reasoning failed when the turn completes with an error", async () => {
    ...
    const stableActivityIdPrefix =
      "engine-reasoning:thread-1:reasoning-failed-turn-1:turn:turn-failed-reasoning:segment:";
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) =>
          activity.id.startsWith(stableActivityIdPrefix) &&
  ...
  it("settles and clears buffered Codex reasoning on runtime errors", async () => {
    ...
    const stableActivityIdPrefix =
      "engine-reasoning:thread-1:reasoning-errored-1:turn:turn-errored-reasoning:segment:";
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) =>
          activity.id.startsWith(stableActivityIdPrefix) &&
  ...
  it("omits empty completed Codex reasoning just like thread/read", async () => {
    ...
    expect(
      thread.activities.filter(
        (activity: EngineRuntimeTestActivity) => activity.summary === "Reasoning trace",
      ),
    ).toHaveLength(0);
  });
  ...
  it("persists a compact per-model token breakdown on turn.completed activities", async () => {
    ...
    const thread = await waitForThread(harness.engine, (candidate) =>
      candidate.activities.some(
        (activity: EngineRuntimeTestActivity) => activity.id === "evt-turn-completed-model-usage",
      ),
    );
  ...
  it("projects MCP tool progress into thread activity with preserved tool metadata", async () => {
    ...
    const thread = await waitForThread(harness.engine, (candidate) =>
      candidate.activities.some(
        (activity: EngineRuntimeTestActivity) => activity.id === "evt-mcp-progress",
      ),
    );

    const activity = thread.activities.find(
      (candidate: EngineRuntimeTestActivity) => candidate.id === "evt-mcp-progress",
    );
    expect(activity).toMatchObject({
      kind: "tool.updated",
      tone: "tool",
  ...
  it("uses assistant item completion detail when no assistant deltas were streamed", async () => {
    ...
    const message = thread.messages.find(
      (entry: EngineRuntimeTestMessage) => entry.id === "assistant:item-no-delta",
    );
  ...
  it("projects completed plan items into first-class proposed plans", async () => {
    ...
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.proposedPlans.some(
        (proposedPlan: EngineRuntimeTestProposedPlan) =>
          proposedPlan.id === "plan:thread-1:turn:turn-plan-final",
      ),
    );
    const proposedPlan = thread.proposedPlans.find(
      (entry: EngineRuntimeTestProposedPlan) => entry.id === "plan:thread-1:turn:turn-plan-final",
    );
  ...
  it("marks the source proposed plan implemented only after the target turn starts", async () => {
    ...
    const sourcePlan = sourceThreadWithPlan.proposedPlans.find(
      (entry: EngineRuntimeTestProposedPlan) =>
  ...
  it("finalizes buffered proposed-plan deltas into a first-class proposed plan on turn completion", async () => {
    ...
    const proposedPlan = thread.proposedPlans.find(
      (entry: EngineRuntimeTestProposedPlan) => entry.id === "plan:thread-1:turn:turn-plan-buffer",
    );
  ...
  it("buffers assistant deltas by default until completion", async () => {
    ...
    const message = thread.messages.find(
      (entry: EngineRuntimeTestMessage) => entry.id === "assistant:item-buffered",
    );
  ...
  it("streams assistant deltas when thread.turn.start requests streaming mode", async () => {
    ...
    const liveMessage = liveThread.messages.find(
      (entry: EngineRuntimeTestMessage) => entry.id === "assistant:item-streaming-mode",
    );
  ...
  it("binds overlapping same-thread delivery modes in engine turn order", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    expect(
      liveThread.messages.some(
        (message: EngineRuntimeTestMessage) => message.id === "assistant:item-same-thread-buffered",
      ),
    ).toBe(false);
  ...
  it("does not assign a new same-thread request mode to the already active turn", async () => {
    ...
    expect(
      liveThread.messages.some(
        (message: EngineRuntimeTestMessage) => message.id === "assistant:item-active-a",
      ),
    ).toBe(false);
  ...
  it("isolates overlapping buffered and streaming turns across threads", async () => {
    ...
    expect(
      streamingThread.messages.find(
        (message: EngineRuntimeTestMessage) => message.id === "assistant:item-overlap-streaming",
      )?.streaming,
    ).toBe(true);
    ...
    expect(
      bufferedThread?.messages.some(
        (message: EngineRuntimeTestMessage) => message.id === "assistant:item-overlap-buffered",
      ),
    ).toBe(false);
  ...
  it("flushes the active Codex turn when a steer requests streaming mode", async () => {
    ...
    expect(
      beforeFlushThread?.messages.some(
        (message: EngineRuntimeTestMessage) => message.id === "assistant:item-late-streaming-mode",
      ),
    ).toBe(false);
    ...
    const flushedMessage = flushedThread.messages.find(
      (entry: EngineRuntimeTestMessage) => entry.id === "assistant:item-late-streaming-mode",
    );
  ...
  it("flushes buffered assistant text before session exit clears turn state", async () => {
    ...
    const message = thread.messages.find(
      (entry: EngineRuntimeTestMessage) => entry.id === "assistant:item-buffered-session-exit",
    );
  ...
  it("flushes buffered assistant text before runtime.error marks the session errored", async () => {
    ...
    const message = thread.messages.find(
      (entry: EngineRuntimeTestMessage) => entry.id === "assistant:item-buffered-runtime-error",
    );
    ...
    async (mode) => {
      ...
      const fault: DispatchFault = {
        ...
        matches: (command) =>
          ...
          String(command.commandId).startsWith(`engine:${targetEventId}:assistant-delta:`),
        attempts: [],
        injected: false,
      };
    ...
    async (mode) => {
      ...
      const fault: DispatchFault = {
        ...
        matches: (command) =>
          ...
          String(command.commandId).startsWith(`engine:${completionEventId}:assistant-complete:`),
        observes: (command) => String(command.commandId).startsWith(`engine:${completionEventId}:`),
        observed: [],
        attempts: [],
        injected: false,
      };
      ...
      const fault: DispatchFault = {
        ...
        matches: (command) =>
          ...
          String(command.commandId).includes(
            `engine:${targetEventId}:assistant-delta-buffer-spill:`,
          ),
        attempts: [],
        injected: false,
      };
  ...
  it("spills oversized buffered deltas and still finalizes full assistant text", async () => {
    ...
    const message = thread.messages.find(
      (entry: EngineRuntimeTestMessage) => entry.id === "assistant:item-buffer-spill",
    );
  ...
  it("preserves the assistant turn id when a late item.completed omits it", async () => {
    ...
    const message = thread.messages.find(
      (entry: EngineRuntimeTestMessage) => entry.id === "assistant:item-late-completion",
    );
  ...
  it("keeps an existing assistant turn id when a late completion carries a newer turn id", async () => {
    ...
    const message = thread.messages.find(
      (entry: EngineRuntimeTestMessage) => entry.id === "assistant:item-late-reassign",
    );
  ...
  it("maps canonical request events into approval activities with requestKind", async () => {
    ...
    await waitForThread(
      ...
      (entry) =>
        entry.activities.some(
          (activity: EngineRuntimeTestActivity) => activity.kind === "approval.requested",
        ) &&
        entry.activities.some(
          (activity: EngineRuntimeTestActivity) => activity.kind === "approval.resolved",
        ),
    );
    ...
    const requested = thread?.activities.find(
      (activity: EngineRuntimeTestActivity) => activity.id === "evt-request-opened",
    );
    ...
    const resolved = thread?.activities.find(
      (activity: EngineRuntimeTestActivity) => activity.id === "evt-request-resolved",
    );
  ...
  it("labels OpenCode retry warnings with a engine-specific summary and visible detail", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    await harness.bindLegacyRuntimeSource("opencode");
    ...
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) => activity.id === "evt-opencode-retry-warning",
      ),
    );
    ...
    expect(payload).toMatchObject({
      message: "Engine request failed; retrying.",
      detail: "Engine request failed; retrying.",
  ...
  it("labels Claude background moves as a background notice, not a runtime warning", async () => {
    ...
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) => activity.id === "evt-background-move-notice",
      ),
    );
  ...
  it("maps session/thread lifecycle and item.started into session/activity projections", async () => {
    ...
    const thread = await waitForThread(
      ...
      (entry) =>
        ...
        entry.session?.activeTurnId === null &&
        entry.activities.some(
          (activity: EngineRuntimeTestActivity) => activity.kind === "tool.started",
        ),
    );
  ...
  it("consumes P1 runtime events into thread metadata, diff checkpoints, and activities", async () => {
    ...
    const thread = await waitForThread(
      harness.engine,
      (entry) =>
        entry.title === "Renamed by engine" &&
        entry.activities.some(
          (activity: EngineRuntimeTestActivity) => activity.kind === "turn.tasks.updated",
        ) &&
        entry.activities.some(
          (activity: EngineRuntimeTestActivity) => activity.kind === "tool.updated",
        ) &&
        entry.activities.some(
          (activity: EngineRuntimeTestActivity) => activity.kind === "runtime.warning",
        ) &&
        entry.checkpoints.some(
          (checkpoint: EngineRuntimeTestCheckpoint) => checkpoint.turnId === "turn-p1",
        ),
    );

    expect(thread.title).toBe("Renamed by engine");

    const taskActivity = thread.activities.find(
      (activity: EngineRuntimeTestActivity) => activity.id === "evt-turn-tasks-updated",
    );
    const taskPayload =
    ...
    const toolUpdate = thread.activities.find(
      (activity: EngineRuntimeTestActivity) => activity.id === "evt-item-updated",
    );
    ...
    const warning = thread.activities.find(
      (activity: EngineRuntimeTestActivity) => activity.id === "evt-runtime-warning",
    );
    ...
    expect(warningPayload?.message).toBe("Engine got slow");

    const checkpoint = thread.checkpoints.find(
      (entry: EngineRuntimeTestCheckpoint) => entry.turnId === "turn-p1",
    );
    expect(checkpoint?.status).toBe("missing");
    expect(checkpoint?.assistantMessageId).toBeNull();
    expect(checkpoint?.checkpointRef).toBe("engine-diff:evt-turn-diff-updated");
    expect(checkpoint?.files).toEqual([
      { path: "file.txt", kind: "modified", additions: 2, deletions: 1 },
    ]);
  });

  it("updates live engine diff placeholders for the same turn", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    await waitForThread(harness.engine, (entry) =>
      entry.checkpoints.some(
        (checkpoint: EngineRuntimeTestCheckpoint) =>
    ...
    const checkpoints = thread.checkpoints.filter(
      (checkpoint: EngineRuntimeTestCheckpoint) => checkpoint.turnId === "turn-live",
    );
    expect(checkpoints).toHaveLength(1);
    expect(checkpoints[0]).toMatchObject({
      checkpointTurnCount: 1,
      checkpointRef: "engine-diff:evt-turn-diff-first",
      status: "missing",
      files: [
        { path: "file.txt", kind: "modified", additions: 2, deletions: 1 },
  ...
  it("does not parse live diff files for engines without patch capability", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    await harness.bindLegacyRuntimeSource("claude");
    ...
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.checkpoints.some(
        (checkpoint: EngineRuntimeTestCheckpoint) => checkpoint.turnId === "turn-claude",
      ),
    );

    const checkpoint = thread.checkpoints.find(
      (entry: EngineRuntimeTestCheckpoint) => entry.turnId === "turn-claude",
    );
    expect(checkpoint).toMatchObject({
      checkpointRef: "engine-diff:evt-claude-diff-placeholder",
      status: "missing",
      files: [],
    });
  });
  ...
  it("projects context window updates into normalized thread activities", async () => {
    ...
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) => activity.kind === "context-window.updated",
      ),
    );
  ...
  it("projects configured Claude context windows into normalized thread activities", async () => {
    ...
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) => activity.kind === "context-window.configured",
      ),
    );
  ...
  it("projects compacted thread state into context compaction activities", async () => {
    ...
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) => activity.kind === "context-compaction",
      ),
    );

    const activity = thread.activities.find(
      (candidate: EngineRuntimeTestActivity) => candidate.kind === "context-compaction",
    );
  ...
  it("projects context compaction progress updates into thread activities", async () => {
    ...
    const activity = thread.activities.find(
      (candidate: EngineRuntimeTestActivity) =>
  ...
  it("projects Codex task lifecycle chunks into thread activities", async () => {
    ...
    const thread = await waitForThread(
      ...
      (entry) =>
        entry.activities.some(
          (activity: EngineRuntimeTestActivity) => activity.kind === "task.completed",
        ) &&
        entry.proposedPlans.some(
          (proposedPlan: EngineRuntimeTestProposedPlan) =>
    ...
    const started = thread.activities.find(
      (activity: EngineRuntimeTestActivity) => activity.id === "evt-task-started",
    );
    const progress = thread.activities.find(
      (activity: EngineRuntimeTestActivity) => activity.id === "evt-task-progress",
    );
    const completed = thread.activities.find(
      (activity: EngineRuntimeTestActivity) => activity.id === "evt-task-completed",
    );
    ...
    expect(
      thread.proposedPlans.find(
        (entry: EngineRuntimeTestProposedPlan) => entry.id === "plan:thread-1:turn:turn-task-1",
      )?.planMarkdown,
    ).toBe("# Plan title");
  });

  it("still appends turn.completed activity when the engine omits cost metadata", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) => activity.id === "evt-turn-completed-no-cost",
      ),
    );
  ...
  it("projects structured user input request and resolution as thread activities", async () => {
    ...
    const thread = await waitForThread(
      ...
      (entry) =>
        ...
        entry.activities.some(
          (activity: EngineRuntimeTestActivity) => activity.kind === "user-input.requested",
        ) &&
        entry.activities.some(
          (activity: EngineRuntimeTestActivity) => activity.kind === "user-input.resolved",
        ),
    );

    const requested = thread.activities.find(
      (activity: EngineRuntimeTestActivity) => activity.id === "evt-user-input-requested",
    );
    ...
    const resolved = thread.activities.find(
      (activity: EngineRuntimeTestActivity) => activity.id === "evt-user-input-resolved",
    );
    const resolvedPayload =
      resolved?.payload && typeof resolved.payload === "object"
        ? (resolved.payload as Record<string, unknown>)
  ...
  it("creates and routes subagent runtime events into child threads", async () => {
    ...
    harness.emit({
      ...
      providerRefs: {
        ...
        parentNativeTurnId: "turn-parent",
      },
      payload: {},
    });
    ...
    const childThread = await waitForThread(
      ...
      2000,
      asThreadId("subagent:thread-1:child-engine-1"),
    );
  ...
  it("handles collab receiver and child engine refs on the same event without duplicate thread creation", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    harness.emit({
      ...
      payload: {
        ...
        data: {
          item: {
            type: "collabAgentToolCall",
            receiverThreadIds: ["child-engine-same-event"],
            receiverAgents: [
              {
                threadId: "child-engine-same-event",
    ...
    const childThread = await waitForThread(
      ...
      2000,
      asThreadId("subagent:thread-1:child-engine-same-event"),
    );
  ...
  it("materializes subagent child threads even when the collab payload only exposes receiverAgents", async () => {
    ...
    const childThread = await waitForThread(
      harness.engine,
      (entry) =>
        entry.id === "subagent:thread-1:child-engine-2" &&
        entry.subagentNickname === "Harper" &&
        entry.subagentRole === "reviewer",
      2000,
      asThreadId("subagent:thread-1:child-engine-2"),
    );
  ...
  it("keeps ingesting after a subagent child thread is deleted instead of re-creating it", async () => {
    const harness = await createHarness();
    const childThreadId = asThreadId("subagent:thread-1:child-engine-deleted");
    const collabEvent = {
      type: "item.updated",
      engine: "codex",
      threadId: asThreadId("thread-1"),
      ...
      payload: {
        ...
        data: {
          item: {
            type: "collabAgentToolCall",
            receiverThreadIds: ["child-engine-deleted"],
          },
        },
      },
    } as const;
    ...
    // A later engine event for the same child must not try to resurrect the
    ...
    const parent = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) => activity.id === "evt-after-deleted-child",
      ),
    );
  ...
  it("starts when an accepted open-turn row can no longer replay its stored command", async () => {
    ...
    const event: EngineRuntimeEvent = {
      ...
      payload: {
        ...
        data: {
          item: {
            type: "collabAgentToolCall",
            receiverThreadIds: ["child-engine-unreplayable"],
          },
        },
      },
    };
    ...
    // that reshaped engine command ids leaves receipts the next build can
    ...
    const rejected = await Effect.runPromise(
      Effect.result(
        harness.engine.dispatch({
          type: "thread.create",
          commandId: CommandId.makeUnsafe(
            `engine:${eventId}:subagent-thread-create:subagent:thread-1:child-engine-unreplayable`,
          ),
          threadId: asThreadId("thread-1"),
          projectId: asProjectId("project-1"),
          title: "Duplicate",
    ...
    const parent = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: EngineRuntimeTestActivity) => activity.id === "evt-after-unreplayable-open-turn",
      ),
    );
    expect(parent.id).toBe("thread-1");
  });
