          ...
          if (!startupRecoveryNotifiedThreads.has(blocker.threadId)) {
            ...
            if (skippedPromptCount > 0) {
              ...
              yield* appendProviderFailureActivity({
                ...
                detail: `OmniMind recovered an earlier provider failure, but ${skippedPromptCount} ${noun} skipped while the thread was blocked. Resend ${skippedPromptCount === 1 ? "it" : "them"} to continue.`,
                turnId: null,
                createdAt,
              });
