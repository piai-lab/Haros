...
describe("orchestration projector", () => {
  ...
  it("updates thread settings from turn start events", async () => {
    ...
    expect(next.threads[0]?.engineSelection).toEqual({
      engine: "pi",
      model: "openai/gpt-5.5",
    });
  ...
  it("lets empty threads adopt the requested first-turn engine", async () => {
    const createdAt = "2026-02-23T08:00:00.000Z";
    const turnRequestedAt = "2026-02-23T08:00:05.000Z";
    const model = createEmptyReadModel(createdAt);
      ...
      projectEvent(
        ...
        makeEvent({
          ...
          payload: {
            ...
            checkpointRef: "engine-diff:event-1",
            status: "missing",
            files: [],
      ...
      projectEvent(
        ...
        makeEvent({
          ...
          payload: {
            ...
            turnId: "turn-1",
            checkpointTurnCount: 1,
            checkpointRef: "refs/harnessos/checkpoints/thread-1/turn/1",
            status: "ready",
            files: [],
            assistantMessageId: null,
            completedAt: placeholderAt,
      ...
      projectEvent(
        ...
        makeEvent({
          ...
          payload: {
            ...
            turnId: "turn-0",
            checkpointTurnCount: 1,
            checkpointRef: "refs/harnessos/checkpoints/thread-1/turn/0",
            status: "ready",
            files: [],
      ...
      const afterSettled = await Effect.runPromise(
        projectEvent(
          ...
          makeSessionSetEvent({
            ...
            status,
            activeTurnId: null,
            lastError: status === "error" ? "engine crashed" : null,
            updatedAt: settledAt,
          }),
        ),
      );
  ...
  it("does not settle while an interrupted session still retains the active turn", async () => {
    ...
    // the engine's terminal event decides the real outcome.
  ...
  it("does not let a late engine-diff placeholder unsettle a session-settled turn", async () => {
    const createdAt = "2026-02-23T08:00:00.000Z";
    const startedAt = "2026-02-23T08:00:05.000Z";
    const settledAt = "2026-02-23T08:00:10.000Z";
      ...
      projectEvent(
        ...
        makeEvent({
          ...
          payload: {
            ...
            turnId: "turn-1",
            checkpointTurnCount: 1,
            checkpointRef: "engine-diff:event-late",
            status: "missing",
            files: [],
            assistantMessageId: null,
            completedAt: placeholderAt,
    ...
    const events: ReadonlyArray<OrchestrationEvent> = [
      ...
      makeEvent({
        ...
        payload: {
          ...
          checkpointRef: "refs/harnessos/checkpoints/thread-1/turn/2",
          status: "ready",
          files: [],
    ...
    const events: ReadonlyArray<OrchestrationEvent> = [
      makeEvent({
        ...
        payload: {
          ...
          turnId: "turn-1",
          checkpointTurnCount: 1,
          checkpointRef: "refs/harnessos/checkpoints/thread-revert/turn/1",
          status: "ready",
          files: [],
      ...
      makeEvent({
        ...
        payload: {
          ...
          turnId: "turn-2",
          checkpointTurnCount: 2,
          checkpointRef: "refs/harnessos/checkpoints/thread-revert/turn/2",
          status: "ready",
          files: [],
      ...
      (_, index) =>
        makeEvent({
          ...
          payload: {
            ...
            checkpointRef: `refs/harnessos/checkpoints/thread-capped/turn/${index + 1}`,
            status: "ready",
            files: [],
