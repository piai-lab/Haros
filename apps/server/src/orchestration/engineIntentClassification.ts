...
export const isEngineIntentEventType = (
  eventType: string,
): eventType is EngineIntentEvent["type"] =>
  ENGINE_INTENT_EVENT_TYPES.has(eventType as EngineIntentEvent["type"]);

export const isEngineIntentEvent = (event: OrchestrationEvent): event is EngineIntentEvent =>
  isEngineIntentEventType(event.type);

export const isReplaySafeClaimedEngineIntent = (event: EngineIntentEvent): boolean =>
...
export const isEngineSideEffectIntent = (event: EngineIntentEvent): boolean =>
  event.type !== "thread.created" &&
  event.type !== "thread.deleted" &&
  event.type !== "thread.session-set" &&
  event.type !== "thread.turn-queued";

export const isClaimedEngineIntent = (event: EngineIntentEvent): boolean =>
  isReplaySafeClaimedEngineIntent(event) || isEngineSideEffectIntent(event);
