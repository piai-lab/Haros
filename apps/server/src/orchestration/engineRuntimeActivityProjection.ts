 ...
 *
 * Activity payloads splice in raw engine values - `Schema.Unknown` payload
...
function reasoningActivityTone(
  ...
  // settles interrupted turns as `failed`, while engine-declined terminal
...
function buildToolProgressActivityPayload(
  event: Extract<EngineRuntimeEvent, { type: "tool.progress" }>,
): ActivityPayload {
...
function buildContextWindowActivityPayload(event: EngineRuntimeEvent): ActivityPayload | undefined {
  ...
  // Stamp the emitting engine so token stats can attribute usage to the
  // engine that actually processed the turn, not the thread's persisted
  // model selection (which can drift, e.g. across future per-turn engines).
  return toActivityPayload({ ...usage, engine: event.engine });
}
...
function requestedPermissionProfile(
  event: Extract<EngineRuntimeEvent, { type: "request.opened" }>,
): Record<string, unknown> | undefined {
...
export function projectProviderRuntimeActivities(
  ...
  if (
    ...
    return [
      {
        id: EventId.makeUnsafe(`engine-reasoning:${event.threadId}:${reasoningItemId}`),
  ...
  switch (event.type) {
      ...
      return [
        {
          ...
          summary: isBackgroundMove
            ...
            : isModelRequestRetry
              ...
              : (event.engine === "opencode" || event.engine === "kilo") &&
                  (nativeType === "session.next.retried" || nativeType === "session.status")
                ? event.engine === "opencode"
                  ? "OpenCode retrying"
                  : "Kilo retrying"
                : "Runtime warning",
    ...
    case "item.started": {
      ...
      // A engine that sends a blank title must not turn into a blank summary:
      ...
      const resetsAt =
        typeof resetsAtRaw === "number"
          ...
          : typeof resetsAtRaw === "string"
            ? resetsAtRaw
            : undefined;
      // Preserve per-window rate limit breakdown when the engine sends it.
...
export function providerActivityUpdateDedupeKey(
  ...
  const prefix = `${threadId}:${event.engine}:${activity.kind}`;
  if (activity.kind === "reasoning.updated" || activity.kind === "reasoning.completed") {
    return `${threadId}:${event.engine}:reasoning:${activity.id}`;
  }
  if (
