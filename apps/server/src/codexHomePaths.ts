...
export function resolveHarosCodexHomeOverlayPath(
  env: NodeJS.ProcessEnv,
 ...
 * writes under. Haros keeps its generated config isolated from the user's
...
export function resolveActiveCodexHomeWritePath(input: CodexHomePathsInput = {}): string {
  ...
  const overlay = resolveHarosCodexHomeOverlayPath(env, source);
