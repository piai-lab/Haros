// FILE: engineUsage/engines/opencode.test.ts
...
function makeHome(relativeAuthPath: string, auth: Record<string, unknown>) {
  const homeDir = mkdtempSync(nodePath.join(os.tmpdir(), "haros-opencode-usage-"));
...
describe("opencodeUsageFetcher", () => {
  it("returns needs-auth when no OpenCode auth.json exists", async () => {
    const homeDir = mkdtempSync(nodePath.join(os.tmpdir(), "haros-opencode-empty-"));
    tempDirs.push(homeDir);
