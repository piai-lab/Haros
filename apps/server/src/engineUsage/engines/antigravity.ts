...
import type { ServerEngineUsageLimit } from "@harnessos/contracts";
...
// Public Gemini CLI installed-app OAuth client (not an HarnessOS-issued secret).
...
export function parseAntigravityQuota(input: {
  ...
  const grouped = new Map<string, ServerEngineUsageLimit>();
