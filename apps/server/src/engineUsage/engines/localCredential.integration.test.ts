...
import { droidUsageFetcher, kiloUsageFetcher } from "./localCredential";

const NOW_MS = 1_780_000_000_000;
const tempDirs: string[] = [];

function makeHome(): string {
  const homeDir = mkdtempSync(nodePath.join(os.tmpdir(), "harnessos-local-usage-"));
...
describe("local credential usage fetchers", () => {
  ...
  it("surfaces signed-in Droid and Kilo without inventing quota bars", async () => {
    const homeDir = makeHome();
