...
interface CachedSnapshot {
  snapshot: ServerEngineUsageSnapshot;
  fetchedAtMs: number;
...
interface InFlightSnapshot {
  credentialKey: string;
  promise: Promise<ServerEngineUsageSnapshot | null>;
}

const snapshotCache = new Map<EngineKind, CachedSnapshot>();
const inFlightFetches = new Map<EngineKind, InFlightSnapshot>();

const snapshotCacheTtlMs = (snapshot: ServerEngineUsageSnapshot): number =>
  snapshot.stale === true
    ? 0
...
export function __resetEngineUsageCacheForTests(): void {
  snapshotCache.clear();
  inFlightFetches.clear();
}

async function getEngineUsageSnapshot(
  engine: EngineKind,
  ctx: EngineUsageContext,
...
export async function collectEngineUsageSnapshots(
  ctx: EngineUsageContext,
  ...
  return settled
    ...
    .filter((snapshot): snapshot is ServerEngineUsageSnapshot => snapshot !== null);
}
...
export const listEngineUsage = Effect.fn(function* (input: ServerListEngineUsageInput) {
  ...
  return yield* Effect.tryPromise({
    ...
    catch: () => [] as unknown as ServerListEngineUsageResult,
  });
});
