...
import { acquireClaudeAuthStatusLock } from "../../engine/claudeAuthStatusLock";
import { buildClaudeProcessEnv } from "../../engine/claudeProcessEnv";
import {
...
export function parseClaudeUsage(input: { json: unknown; nowMs: number; planName?: string }) {
  ...
  const limits: ServerEngineUsageLimit[] = [];
  const usageLines: ServerEngineUsageLine[] = [];
...
export const claudeUsageFetcher: EngineUsageFetcher = {
  ...
  async fetch(ctx) {
    ...
    let lastErrorSnapshot: ServerEngineUsageSnapshot | null = null;
