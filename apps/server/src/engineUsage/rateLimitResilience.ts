// FILE: engineUsage/rateLimitResilience.ts
...
interface ResilienceEntry {
  lastGoodSnapshot: ServerEngineUsageSnapshot | null;
...
export interface RateLimitResilience {
  ...
  serveDuringCooldown(key: string, nowMs: number): ServerEngineUsageSnapshot | null;
  /** Record a clean fetch and clear any cooldown for `key`. */
  rememberLastGood(key: string, snapshot: ServerEngineUsageSnapshot, nowMs: number): void;
