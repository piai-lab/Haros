...
function limit(snapshot: ServerEngineUsageSnapshot, window: string) {
  return snapshot.limits.find((entry) => entry.window === window);
}

function usageLine(snapshot: ServerEngineUsageSnapshot, label: string) {
...
describe("parseCodexAppServerRateLimits", () => {
  const json = {
    rateLimits: {
      planType: "plus",
      primary: { usedPercent: 6, resetsAt: 1_738_300_000, windowDurationMins: 300 },
      secondary: {
        usedPercent: 24,
  ...
  it("maps rate-limit windows, credits, and plan", () => {
    const snapshot = parseCodexAppServerRateLimits({ json, nowMs: NOW_MS });
