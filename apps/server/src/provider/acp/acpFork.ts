  ...
  return Effect.gen(function* () {
    ...
    if (!(yield* input.runtime.supportsSessionRecovery)) {
      return yield* new ProviderAdapterValidationError({
        provider: input.provider,
        operation: "forkThread",
        issue: `This ${input.provider} ACP version advertises session/fork but cannot reopen the forked session; OmniMind will rebuild the fork from its retained transcript.`,
      });
    }
