  ...
  const registry: typeof ProviderAdapterRegistry.Service = {
          ...
          : provider === "antigravity"
            ...
            : provider === "droid" && providers?.includeRestartRollbackDroid === true
              ...
              : provider === "pi" && providers?.includePi === true
                ...
                : provider === "oa" && providers?.includeOmniMind === true
    ...
    listProviders: () =>
      Effect.succeed([
        ...
        ...(providers?.includeOmniMind === true ? (["oa"] as const) : []),
      ] as const),
  };
...
staleSettlementRouting.layer("ProviderServiceLive exact stale-terminal settlement", (it) => {
  ...
  it.effect("settles an exact old turn after its lifecycle generation is retired", () =>
    Effect.gen(function* () {
      ...
      yield* provider.sendTurn({ threadId, input: "before retirement", attachments: [] });
  ...
  it.effect("drops stale terminals that cannot prove exact durable ownership", () =>
    Effect.gen(function* () {
      ...
      for (const event of [
        ...
        {
          type: "turn.aborted",
          eventId: asEventId("stale-terminal-wrong-provider"),
          provider: "claude",
          threadId,
          turnId: activeTurnId,
          lifecycleGeneration: String(generation),
          createdAt: "2026-08-16T08:02:01.000Z",
          payload: { reason: "wrong provider" },
        },
        {
          type: "turn.aborted",
  ...
  it.effect("never lets an old terminal clear a newer generation and turn", () =>
    Effect.gen(function* () {
      ...
      yield* provider.sendTurn({ threadId, input: "old", attachments: [] });
      ...
      yield* provider.sendTurn({ threadId, input: "new", attachments: [] });
  ...
  it.effect("re-adopts a stale live binding before routing a recovery-capable send", () =>
    Effect.gen(function* () {
      ...
      yield* provider.sendTurn({ threadId, input: "recover before send", attachments: [] });
  ...
  it.effect("reloads only the exact live OmniMind Agent session", () =>
    Effect.gen(function* () {
      ...
      yield* provider.startSession(threadId, {
        provider: "oa",
        threadId,
        modelSelection: { provider: "oa", model: "gateway/model-one" },
        runtimeMode: "full-access",
      });
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "omnimind-provider-service-stopall-"));
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntimeRepositoryLive.pipe(
        Layer.provide(persistenceLayer),
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "omnimind-provider-service-restart-"));
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntimeRepositoryLive.pipe(
        Layer.provide(persistenceLayer),
...
modelServiceAdmission.layer("ProviderServiceLive model-service admission fence", (it) => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* provider.startSession(threadId, {
          provider: "oa",
          threadId,
          modelSelection: { provider: "oa", model: "gateway-a/model-one" },
          runtimeMode: "full-access",
        });
        ...
        const replacementFiber = yield* provider
          .startSession(threadId, {
            provider: "oa",
            threadId,
            modelSelection: { provider: "oa", model: "gateway-b/model-two" },
            runtimeMode: "full-access",
          })
    ...
    Effect.gen(function* () {
      ...
      const sendFiber = yield* provider
        .sendTurn({
          ...
          attachments: [],
          modelSelection: { provider: "oa", model: "gateway-c/model-three" },
        })
        .pipe(Effect.forkChild);
      yield* sleep(25);
  ...
  it.effect("rechecks the current custom service after a queued recovery acquires its fence", () =>
    Effect.gen(function* () {
      ...
      assert.equal(modelServiceAdmission.oa.startSession.mock.calls.length, startCallCount + 1);
      assert.equal(
        modelServiceAdmission.oa.startSession.mock.calls.at(-1)?.[0].modelSelection?.model,
        "gateway-b/model-two",
      );
      yield* provider.stopSession({ threadId });
    }),
...
routing.layer("ProviderServiceLive routing", (it) => {
  ...
  it.effect("routes explicit claudeAgent provider session starts to the claude adapter", () =>
    Effect.gen(function* () {
      ...
      if (startInput && typeof startInput === "object") {
        ...
        assert.equal(startPayload.provider, "claude");
  ...
  it.effect("reuses persisted resume cursor when startSession is called after a restart", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "omnimind-provider-service-start-"));
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntimeRepositoryLive.pipe(
        Layer.provide(persistenceLayer),
      ...
      const firstRegistry: typeof ProviderAdapterRegistry.Service = {
        getByProvider: (provider) =>
          provider === "claude"
            ...
            : Effect.fail(new ProviderUnsupportedError({ provider })),
        listProviders: () => Effect.succeed(["claude"]),
      };
      const firstDirectoryLayer = ProviderSessionDirectoryLive.pipe(
        Layer.provide(runtimeRepositoryLayer),
      );
  ...
  it.effect("clears stale resume cursor while preserving provider options for fresh restart", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "omnimind-provider-service-clear-"));
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntimeRepositoryLive.pipe(
        Layer.provide(persistenceLayer),
  ...
  it.effect("stops the live runtime while preserving resume cursor and provider options", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "omnimind-provider-service-stop-runtime-"),
      );
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntimeRepositoryLive.pipe(
