...
const disabledProviderHealthLayer = EngineHealthLive.pipe(
...
it.layer(NodeServices.layer)("EngineHealth", (it) => {
  describe("engine update commands", () => {
    ...
    it.effect("stops a hung engine process and persists a failed update state", () =>
      Effect.gen(function* () {
        ...
        const layer = makeProviderHealthLive({ engineUpdateTimeoutMs: 20 }).pipe(
        ...
        const result = yield* Effect.gen(function* () {
          ...
          return yield* TestClock.withLive(providerHealth.updateEngine({ engine: "kilo" }));
  ...
  describe("disabled engine handling", () => {
    ...
    it.effect("rejects one-click updates for disabled engines", () =>
      Effect.gen(function* () {
        const providerHealth = yield* EngineHealth;
        const error = yield* Effect.flip(providerHealth.updateEngine({ engine: "kilo" }));
  ...
  // ── Custom model engine: checkCodexProviderStatus integration ───

  describe("checkCodexProviderStatus with custom model engine", () => {
  ...
  describe("checkCodexProviderStatus with openai model engine", () => {
