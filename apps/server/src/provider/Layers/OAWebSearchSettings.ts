...
} from "@harnessos/oa-web-access/config-service";
import {
...
} from "@harnessos/oa-web-access/settings-runtime";
...
function recovery(error: WebSearchConfigError): OAWebSearchRecoverySnapshot {
  return { state: "recovery", reason: error.kind, message: error.message };
}

export const OAWebSearchSettingsLive = Layer.effect(
  OAWebSearchSettings,
  Effect.gen(function* () {
    ...
    const read = (ensure: boolean, knownRevision?: string): OAWebSearchReadResult => {
      try {
    ...
    return {
      ...
      mutate: (input) =>
        runSync((): OAWebSearchMutationResult => {
          try {
    ...
    } satisfies OAWebSearchSettingsShape;
  }),
);
