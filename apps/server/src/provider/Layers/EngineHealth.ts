...
const providerCommandEnv = (engine: EngineKind): NodeJS.ProcessEnv =>
  buildProviderChildEnvironment({ engine: providerChildKind(engine) });
...
function mergeProviderStatusUpdates(
  ...
  return orderProviderStatuses([...statusByEngine.values()]);
}
...
export function projectProviderStatusesForSettings(
  ...
  for (const engine of PROVIDERS) {
    ...
    if (!isProviderEnabledForSettings(engine, settings)) {
      const disabledStatus = makeDisabledProviderStatus(engine, status?.checkedAt ?? checkedAt);
    ...
    if (status && !isDisabledProviderStatusOverlay(status)) {
      projected.push(
        settings.enableEngineUpdateChecks ? status : suppressProviderVersionAdvisory(status),
      );
    }
  }
...
export function makeProviderHealthLive(options?: { readonly engineUpdateTimeoutMs?: number }) {
  ...
  return Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const cachedStatuses: EngineStatuses = yield* Effect.forEach(
        PROVIDERS,
        (engine) =>
          readProviderStatusCache(cachePathByEngine.get(engine)!).pipe(
      ...
      const getProviderMaintenanceCapabilities = Effect.fn("getProviderMaintenanceCapabilities")(
        function* (engine: EngineKind) {
          ...
          return yield* resolveProviderMaintenanceCapabilitiesEffect(definition, {
            binaryPath: resolveProviderMaintenanceBinaryPath(engine, settings),
      ...
      const enrichStatuses = Effect.fn("enrichProviderStatuses")(function* (
        ...
        const enriched = yield* Effect.forEach(
          statuses,
          (status) =>
            getProviderMaintenanceCapabilities(status.engine).pipe(
            ...
            Effect.all(
              [
                ...
                checkProviderWhenEnabled(
                  settings,
                  CURSOR_PROVIDER,
                  makeCheckCursorProviderStatus(settings.engines.cursor.binaryPath),
                ),
                checkProviderWhenEnabled(
                  settings,
                  ANTIGRAVITY_PROVIDER,
                  checkAntigravityProviderStatus(settings.engines.antigravity.binaryPath),
                ),
                checkProviderWhenEnabled(
                  settings,
                  GROK_PROVIDER,
                  makeCheckGrokProviderStatus(settings.engines.grok.binaryPath),
                ),
                checkProviderWhenEnabled(
                  settings,
                  DROID_PROVIDER,
                  makeCheckDroidProviderStatus(settings.engines.droid.binaryPath),
                ),
                checkProviderWhenEnabled(
                  settings,
                  KILO_PROVIDER,
                  makeCheckKiloProviderStatus(settings.engines.kilo.binaryPath),
                ),
                checkProviderWhenEnabled(
                  settings,
                  OPENCODE_PROVIDER,
                  makeCheckOpenCodeProviderStatus(settings.engines.opencode.binaryPath),
                ),
...
export const EngineHealthLive = makeProviderHealthLive();
