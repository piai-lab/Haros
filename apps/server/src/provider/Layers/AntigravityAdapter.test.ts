...
describe("Antigravity CLI integration helpers", () => {
  ...
  it("runs packaged Electron as Node only for OmniMind-managed sessions", () => {
    ...
    expect(
      buildAntigravityCaptureCommand(
        String.raw`C:\Program Files\OmniMind\OmniMind.exe`,
        String.raw`C:\Users\test\.gemini\capture.cjs`,
        "pre-tool",
        "win32",
      ),
    ).toBe(
      String.raw`if not defined OMNIMIND_ANTIGRAVITY_EVENTS (more >nul 2>nul & echo {"decision":"ask"}) else (set "ELECTRON_RUN_AS_NODE=1" && "C:\Program Files\OmniMind\OmniMind.exe" "C:\Users\test\.gemini\capture.cjs" "pre-tool")`,
    );
  });
