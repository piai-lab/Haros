...
function harness(options: OAAgentPromptFilesLiveOptions = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "harnessos-prompts-"));
  ...
  const run = <A>(
    operation: (
      service: OAAgentPromptFilesShape,
      settings: ServerSettingsShape,
    ) => Effect.Effect<A, unknown, never>,
  ) =>
    Effect.runPromise(
      Effect.gen(function* () {
        return yield* operation(yield* OAAgentPromptFiles, yield* ServerSettingsService);
  ...
  it("keeps the largest legal escaped request and response below the existing WS ceiling", async () => {
    ...
    for (const payload of [
      ...
      const requestFrame = JSON.stringify({
        ...
        tag: WS_METHODS.oaAgentPromptsMutate,
        payload,
      });
    ...
    const maxLegalSnapshot = Schema.decodeUnknownSync(OAAgentPromptSnapshot)({
      ...snapshot,
      defaultPrompt: {
