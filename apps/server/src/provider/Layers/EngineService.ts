...
const configuredProviderRuntimeIdleStopMs = process.env.HARNESSOS_PROVIDER_RUNTIME_IDLE_STOP_MS;
const ENGINE_RUNTIME_IDLE_STOP_MS = Number.isFinite(Number(configuredProviderRuntimeIdleStopMs))
...
const ENGINE_REPLACEMENT_TARGET_PROVIDER_KEY = "replacementTargetProvider";
...
const makeProviderService = (options?: EngineServiceLiveOptions) =>
  Effect.gen(function* () {
    ...
    const providerInterruptionFences = new Map<ThreadId, EngineInterruptionFence>();
    ...
    const acceptRuntimeEvent = (
      ...
      if (event.providerRefs?.nativeParentThreadId !== undefined) {
        return isStaleGeneration
          ? rejectRuntimeEvent()
    ...
    const cleanupCandidatesForUncertainOwnership = Effect.fnUntraced(function* (
      threadId: ThreadId,
      requiredProviders: ReadonlyArray<EngineSession["engine"]>,
    ) {
      const inspected = yield* inspectUnboundProviderOwners(threadId);
      const requiredAdapters = requiredProviders.flatMap((engine) => {
    ...
    const persistUncertainProviderOwnership = (input: {
      ...
      readonly targetProvider?: EngineSession["engine"];
        ...
        directory.upsert({
          ...
          runtimePayload: {
            ...
            ...(input.targetProvider !== undefined
              ? {
                  [ENGINE_REPLACEMENT_TARGET_PROVIDER_KEY]: input.targetProvider,
                }
              : {}),
          },
        }),
    ...
    const runtimeEventPumpHealth = makeProviderRuntimeEventPumpHealthRegistry(engines);
                ...
                directory.upsert({
                  ...
                  runtimePayload: {
                    ...
                    ...(effectiveProviderOptions === undefined
                      ? {}
                      : { engineOptions: effectiveProviderOptions }),
...
export const EngineServiceLive = Layer.effect(EngineService, makeProviderService());

export function makeProviderServiceLive(options?: EngineServiceLiveOptions) {
  return Layer.effect(EngineService, makeProviderService(options));
}

/** Production engine service: journal each canonical event before live fan-out. */
export function makeDurableProviderServiceLive(options?: EngineServiceLiveOptions) {
  return Layer.effect(
    EngineService,
    Effect.gen(function* () {
