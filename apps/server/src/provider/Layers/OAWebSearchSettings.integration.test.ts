...
import { MAX_WEB_SEARCH_CONFIG_BYTES } from "@harnessos/oa-web-access/config-service";
...
import {
  OAWebSearchSettings,
  type OAWebSearchSettingsShape,
} from "../Services/OAWebSearchSettings.ts";
import { OAWebSearchSettingsLive } from "./OAWebSearchSettings.ts";
...
function harness() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "harnessos-web-search-settings-"));
  const baseDir = path.join(root, "state");
  fs.mkdirSync(baseDir, { recursive: true, mode: 0o700 });
  cleanups.push(root);
  const openInEditor = vi.fn(() => Effect.void);
  const layer = OAWebSearchSettingsLive.pipe(
  ...
  const run = <A>(operation: (service: OAWebSearchSettingsShape) => Effect.Effect<A, Error>) =>
    Effect.runPromise(
      Effect.gen(function* () {
        return yield* operation(yield* OAWebSearchSettings);
      }).pipe(Effect.provide(layer)),
    );
  return { root, baseDir, agentDir: path.join(baseDir, "agent"), openInEditor, run };
}

describe("OAWebSearchSettingsLive", () => {
