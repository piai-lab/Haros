...
interface OpenCodeSessionContext {
  ...
  /** Permission request ids resolved by OmniMind policy and never surfaced to the UI. */
  ...
  return yield* new OpenCodeRuntimeError({
    ...
    detail:
      status?.status === "failed"
        ? `${input.displayName} OmniMind MCP connection failed: ${status.error}`
        : `${input.displayName} OmniMind MCP connection did not become ready.`,
  });
});
  ...
  return Layer.effect(
    ...
    Effect.gen(function* () {
        ...
        if (!context.activeTurnToolCallIdleWatchdogStarted) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (context.activeTurnSawFinalAssistant) {
              ...
              if (
                ...
                (yield* recoverOpenCodeTurnFromMessageId(context, {
                  ...
                  raw: {
                    source: "omnimind.opencode.deferred-idle-completion",
                    event: raw,
                  },
                }))
              ) {
              ...
              if (
                ...
                yield* completeOpenCodeTurn(context, {
                  turnId,
                  raw: {
                    source: "omnimind.opencode.deferred-idle-local-part",
                    event: raw,
                  },
              ...
              if (
                ...
                (yield* recoverOpenCodeTurnFromMessageId(context, {
                  ...
                  raw: {
                    source: "omnimind.opencode.deferred-idle-completion-retry",
                    event: raw,
                  },
                }))
              ) {
              ...
              if (
                ...
                yield* completeOpenCodeTurn(context, {
                  turnId,
                  raw: {
                    source: "omnimind.opencode.deferred-idle-local-part-retry",
                    event: raw,
                  },
                  totalCostUsd: context.latestTurnCostUsd,
                });
            ...
            const completed = yield* completeOpenCodeTurn(context, {
              turnId,
              raw: {
                source: "omnimind.opencode.idle-after-tool-calls",
                event: raw,
              },
      ...
      const recoverOpenCodeTurnFromMessages = Effect.fn("recoverOpenCodeTurnFromMessages")(
        function* (
          ...
          return yield* recoverOpenCodeTurnFromAssistantMessage(context, {
            ...
            assistantEntry,
            raw: {
              source: "omnimind.opencode.prompt.recovery",
        ...
        yield* Effect.gen(function* () {
          ...
          Effect.flatMap(() =>
            Effect.gen(function* () {
              ...
              const completed = yield* completeOpenCodeTurn(context, {
                turnId: input.turnId,
                raw: { source: "omnimind.opencode.prompt.watchdog" },
                errorMessage: message,
              });
              if (!completed) return;
              yield* emit(context, {
        ...
        switch (event.type) {
          ...
          case "permission.replied": {
            if (context.policyResolvedPermissionIds.has(event.properties.requestID)) {
              // OmniMind policy resolved this request; nothing was surfaced to the UI.
              break;
            }
          ...
          // agent loop. Mirror them into OmniMind's canonical transcript stream.
                  ...
                  Effect.gen(function* () {
                      ...
                      gatewayControlAvailable = yield* installOpenCodeGatewayMcp({
                        ...
                        Effect.catchCause((cause) =>
                          Effect.sync(() => agentGatewaySessionLease?.release()).pipe(
                            Effect.andThen(
                              Effect.logWarning(
                                `${adapterConfig.displayName} could not install thread-scoped OmniMind MCP control`,
                                Cause.squash(cause),
                              ),
                            ),
                            Effect.as(false),
                    ...
                    const createSessionId = resumedSessionId
                      ? // A resumed engine may still be executing an interrupted Plan turn.
                        ...
                        // without ever emitting a permission request for OmniMind to reject.
      ...
      const sendTurnImpl = Effect.fn("sendTurn")(function* (
        ...
        const harnessPolicy = takeOmniMindHarnessPolicyForProviderSession(
          {
        ...
        // Always pin OmniMind's interaction mode to OpenCode's primary agent.
        // Otherwise a user config with default agent=plan (or a stale options.agent=plan
        // after leaving OmniMind plan mode) can trap default turns in plan mode.
        ...
        if (engine === "kilo") {
          ...
          // server rather than by one long-lived HTTP request. Keep OmniMind's event
