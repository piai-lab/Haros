...
const makeProviderSessionDirectory = Effect.gen(function* () {
    ...
    repository.getByThreadId({ threadId }).pipe(
      ...
      Effect.flatMap((runtime) =>
        Option.match(runtime, {
          ...
          onSome: (value) =>
            decodeProviderKind(value.providerName, "EngineSessionDirectory.getBinding").pipe(
              Effect.map((engine) =>
                Option.some({
  ...
  const listBindings: EngineSessionDirectoryShape["listBindings"] = () =>
    repository.list().pipe(
      ...
      Effect.flatMap(
        Effect.forEach((row) =>
          decodeProviderKind(row.providerName, "EngineSessionDirectory.listBindings").pipe(
            Effect.map((engine) =>
              Option.some({
...
export function makeProviderSessionDirectoryLive() {
  return Layer.effect(EngineSessionDirectory, makeProviderSessionDirectory);
}
