// FILE: debugMode.ts
// Purpose: Applies OmniMind's engine-independent evidence-first Debug instructions.
...
export function withProviderDebugModePrompt(input: {
  ...
  if (input.interactionMode !== "debug" || input.text.startsWith(ENGINE_DEBUG_MODE_PROMPT_PREFIX)) {
    return input.text;
  }
