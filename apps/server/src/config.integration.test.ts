...
const originalHarnessOSStaticDir = process.env.HARNESSOS_STATIC_DIR;
...
afterEach(() => {
  ...
  tempDirs.clear();
  if (originalHarnessOSStaticDir === undefined) {
    delete process.env.HARNESSOS_STATIC_DIR;
  } else {
    process.env.HARNESSOS_STATIC_DIR = originalHarnessOSStaticDir;
  }
});

describe("resolveStaticDir", () => {
  it("uses the desktop static snapshot exposed through the HarnessOS environment", async () => {
    const snapshotDir = makeTempDir("harnessos-static-snapshot-test-");
    fs.writeFileSync(path.join(snapshotDir, "index.html"), "<main>HarnessOS</main>");
...
describe("resolveDefaultChatWorkspaceRoot", () => {
  it("places the managed chat workspace under Documents/HarnessOS on macOS and Linux", () => {
    expect(
      resolveDefaultChatWorkspaceRoot({
        homeDir: "/Users/tester",
        platform: "darwin",
      }),
    ).toBe("/Users/tester/Documents/HarnessOS");
    expect(
      resolveDefaultChatWorkspaceRoot({
        homeDir: "/home/tester",
        platform: "linux",
      }),
    ).toBe("/home/tester/Documents/HarnessOS");
  });
  ...
  it("uses Windows separators when deriving the managed chat workspace on Windows", () => {
    ...
    ).toBe("C:\\Users\\tester\\Documents\\HarnessOS");
  });
  ...
  it("defaults to the current process platform when no platform is supplied", () => {
    ...
    try {
      expect(resolveDefaultChatWorkspaceRoot({ homeDir: "C:\\Users\\tester" })).toBe(
        "C:\\Users\\tester\\Documents\\HarnessOS",
      );
    } finally {
...
describe("resolveDefaultStudioWorkspaceRoot", () => {
  it("places the Studio workspace under Documents/HarnessOS/Studio on macOS and Linux", () => {
    expect(
      resolveDefaultStudioWorkspaceRoot({
        homeDir: "/Users/tester",
        platform: "darwin",
      }),
    ).toBe("/Users/tester/Documents/HarnessOS/Studio");
    expect(
      resolveDefaultStudioWorkspaceRoot({
        homeDir: "/home/tester",
        platform: "linux",
      }),
    ).toBe("/home/tester/Documents/HarnessOS/Studio");
  });
  ...
  it("uses Windows separators when deriving the Studio workspace on Windows", () => {
    ...
    ).toBe("C:\\Users\\tester\\Documents\\HarnessOS\\Studio");
  });
});
...
describe("resolveCanonicalWorkspaceRoots", () => {
  it("canonicalizes a symlinked home directory to match project row realpaths", async () => {
    ...
    expect(result.chatWorkspaceRoot).toBe(path.join(expectedHomeDir, "Documents", "HarnessOS"));
    expect(result.studioWorkspaceRoot).toBe(
      path.join(expectedHomeDir, "Documents", "HarnessOS", "Studio"),
    );
  });
  ...
  it("canonicalizes the nearest existing ancestor when the workspace root itself does not exist yet", async () => {
    ...
    // HarnessOS/ nor HarnessOS/Studio exist yet underneath it.
    ...
    expect(result.chatWorkspaceRoot).toBe(path.join(expectedDocuments, "HarnessOS"));
    expect(result.studioWorkspaceRoot).toBe(path.join(expectedDocuments, "HarnessOS", "Studio"));
