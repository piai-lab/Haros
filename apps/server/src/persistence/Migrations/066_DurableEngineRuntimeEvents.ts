...
import { ENGINE_RUNTIME_INGESTION_CONSUMER } from "../Services/EngineRuntimeEvents.ts";

// Engine output is journaled before live publication. The consumer cursor is
// intentionally independent from orchestration event sequences: it advances
// only after EngineRuntimeIngestion accepts the exact journal row.
export default Effect.gen(function* () {
  ...
  yield* sql`
    INSERT INTO provider_runtime_event_consumers (
      ...
      ${ENGINE_RUNTIME_INGESTION_CONSUMER}, 0, ${migratedAt}, ${migratedAt}
    )
