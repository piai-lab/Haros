// FILE: 035_NormalizeLegacyEngineSelectionOptions.ts
// Purpose: Repairs persisted engineSelection JSON whose legacy options are stored as option rows.
...
function normalizeEngineSelection(input: unknown): {
  readonly changed: boolean;
  readonly value: unknown;
} {
  const value = normalizePersistedEngineSelection(input);
  return { changed: JSON.stringify(value) !== JSON.stringify(input), value };
}

function normalizeEngineSelectionJson(json: string | null): {
  readonly changed: boolean;
  ...
  const normalized = normalizeEngineSelection(parsed);
  return {
...
function normalizeEventPayloadJson(json: string): {
  ...
  const defaultEngineSelection = normalizeEngineSelection(payload.defaultEngineSelection);
  if (defaultEngineSelection.changed) {
    mutablePayload().defaultEngineSelection = defaultEngineSelection.value;
  }

  const engineSelection = normalizeEngineSelection(payload.engineSelection);
  if (engineSelection.changed) {
    mutablePayload().engineSelection = engineSelection.value;
  }
...
export default Effect.gen(function* () {
  ...
  while (true) {
    ...
    for (const row of projectRows) {
      const normalized = normalizeEngineSelectionJson(row.defaultEngineSelection);
  ...
  while (true) {
    const threadRows: ReadonlyArray<{
      ...
      readonly engineSelection: string;
    }> = yield* sql<{
      readonly threadId: string;
      readonly engineSelection: string;
    }>`
    ...
    for (const row of threadRows) {
      const normalized = normalizeEngineSelectionJson(row.engineSelection);
      if (normalized.changed) {
        yield* sql`
