...
export default Effect.gen(function* () {
  ...
  yield* sql`
    ...
    SET default_model_selection_json = CASE
      ...
      ELSE json_object(
        'engine',
        CASE
          WHEN lower(default_model) LIKE '%claude%' THEN 'claude'
          ELSE 'codex'
        END,
        'model',
        default_model
  ...
  yield* sql`
    ...
    SET payload_json = CASE
      WHEN json_type(payload_json, '$.defaultModel') = 'null' THEN json_remove(
        json_set(payload_json, '$.defaultEngineSelection', json('null')),
        '$.defaultProvider',
        '$.defaultModel',
        '$.defaultModelOptions'
      )
            ...
            json_object(
              ...
              CASE
                ...
                WHEN json_type(payload_json, '$.defaultModelOptions.codex') IS NOT NULL
                  OR json_type(payload_json, '$.defaultModelOptions.claude') IS NOT NULL
                THEN CASE
                  WHEN (
                  CASE
                    WHEN json_extract(payload_json, '$.defaultProvider') IS NOT NULL
                  ...
                  THEN CASE
                    WHEN json_type(payload_json, '$.defaultModelOptions.claude') IS NOT NULL
                    THEN json_object(
                      'options',
                      json(json_extract(payload_json, '$.defaultModelOptions.claude'))
                    )
                    WHEN json_type(payload_json, '$.defaultModelOptions.codex') IS NOT NULL
                    THEN json_object(
                      'options',
    ...
    WHERE event_type IN ('project.created', 'project.meta-updated')
      AND json_type(payload_json, '$.defaultEngineSelection') IS NULL
  ...
  yield* sql`
      ...
      json_set(
        ...
        json_patch(
          json_object(
            'engine',
            CASE
              WHEN json_extract(payload_json, '$.engine') IS NOT NULL
              THEN json_extract(payload_json, '$.engine')
          ...
          CASE
            ...
            WHEN json_type(payload_json, '$.modelOptions.codex') IS NOT NULL
              OR json_type(payload_json, '$.modelOptions.claude') IS NOT NULL
            THEN CASE
              WHEN (
                CASE
                  WHEN json_extract(payload_json, '$.engine') IS NOT NULL
              ...
              THEN CASE
                WHEN json_type(payload_json, '$.modelOptions.claude') IS NOT NULL
                THEN json_object(
                  'options',
                  json(json_extract(payload_json, '$.modelOptions.claude'))
                )
                WHEN json_type(payload_json, '$.modelOptions.codex') IS NOT NULL
                THEN json_object(
                  'options',
    ...
    WHERE event_type IN ('thread.created', 'thread.meta-updated', 'thread.turn-start-requested')
      AND json_type(payload_json, '$.engineSelection') IS NULL
  ...
  yield* sql`
    ...
    SET payload_json = json_set(
      payload_json,
      '$.engineSelection',
      json(json_object('engine', 'codex', 'model', 'gpt-5.5'))
    )
