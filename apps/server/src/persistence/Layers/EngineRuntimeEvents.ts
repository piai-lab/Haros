import { NonNegativeInt, EngineRuntimeEvent } from "@harnessos/contracts";
 ...
 *
 * Deliberately matched to ENGINE_RUNTIME_EVENT_RETAIN_ACCEPTED: roughly one
 * scan per tail-length of accepted events falling out of the diagnostic tail.
 */
const ENGINE_RUNTIME_EVENT_RETENTION_SCAN_INTERVAL = ENGINE_RUNTIME_EVENT_RETAIN_ACCEPTED;

const EngineRuntimeEventJson = Schema.fromJsonString(EngineRuntimeEvent);
const encodeEvent = Schema.encodeEffect(EngineRuntimeEventJson);
const decodeEvent = Schema.decodeUnknownEffect(EngineRuntimeEventJson);
...
/**
 * Recursively bounds string leaves while preserving the payload's canonical
 * structure and every non-string value. Non-string floods still fail closed.
 */
...
const encodePersistableEvent = (event: EngineRuntimeEvent) =>
  Effect.gen(function* () {
    const eventJson = yield* encodeEvent(event).pipe(
      Effect.mapError(toPersistenceDecodeError("EngineRuntimeEvent.append.encode")),
    );
    const originalBytes = Buffer.byteLength(eventJson, "utf8");
    if (originalBytes <= ENGINE_RUNTIME_EVENT_MAX_BYTES) {
    ...
    const compactedJson = yield* encodeEvent(compactedEvent).pipe(
      Effect.mapError(toPersistenceDecodeError("EngineRuntimeEvent.append.compact")),
    );
    if (Buffer.byteLength(compactedJson, "utf8") <= ENGINE_RUNTIME_EVENT_MAX_BYTES) {
      return { event: compactedEvent, eventJson: compactedJson };
    }

    return yield* new PersistenceDecodeError({
      operation: "EngineRuntimeEvent.append",
      issue: `Engine runtime event exceeds ${ENGINE_RUNTIME_EVENT_MAX_BYTES} bytes after payload truncation and raw compaction.`,
    });
  });
...
const make = Effect.gen(function* () {
  ...
  const append: EngineRuntimeEventRepositoryShape["append"] = (event) =>
    Effect.gen(function* () {
      ...
      const appendResult = yield* sql
        ...
        .pipe(Effect.mapError(toPersistenceSqlError("EngineRuntimeEvent.append")));
      const persisted = appendResult.inserted
        ? yield* decodeSequenceRow(appendResult.row).pipe(
            ...
            Effect.mapError(toPersistenceDecodeError("EngineRuntimeEvent.append.row")),
          )
        : yield* decodeStoredRow(appendResult.row).pipe(
            Effect.mapError(toPersistenceDecodeError("EngineRuntimeEvent.append.conflictRow")),
          );
      if (persisted.eventJson !== eventJson) {
        return yield* new PersistenceDecodeError({
          operation: "EngineRuntimeEvent.append",
          issue: `Engine event '${event.eventId}' was reused with different content.`,
        });
      }
      return {
  ...
  const readAfter: EngineRuntimeEventRepositoryShape["readAfter"] = (input) => {
    ...
    return Effect.gen(function* () {
      const rows = yield* sql<Record<string, unknown>>`
        ...
        LIMIT ${limit}
      `.pipe(Effect.mapError(toPersistenceSqlError("EngineRuntimeEvent.readAfter")));
      return yield* Effect.forEach(
        rows,
        (unknownRow) =>
          Effect.gen(function* () {
            const row = yield* decodeStoredRow(unknownRow).pipe(
              Effect.mapError(toPersistenceDecodeError("EngineRuntimeEvent.readAfter.row")),
            );
  ...
  const getThreadCoverage: EngineRuntimeEventRepositoryShape["getThreadCoverage"] = (threadId) =>
    sql<{
    ...
    `.pipe(
      ...
      Effect.mapError(toPersistenceSqlError("EngineRuntimeEvent.getThreadCoverage")),
    );

  const readThreadEvents: EngineRuntimeEventRepositoryShape["readThreadEvents"] = (input) => {
    ...
    return Effect.gen(function* () {
      ...
      `.pipe(Effect.mapError(toPersistenceSqlError("EngineRuntimeEvent.readThreadEvents")));
      return yield* Effect.forEach(
        rows,
        (unknownRow) =>
          Effect.gen(function* () {
            const row = yield* decodeStoredRow(unknownRow).pipe(
              Effect.mapError(toPersistenceDecodeError("EngineRuntimeEvent.readThreadEvents.row")),
            );
            const event = yield* decodeEvent(row.eventJson).pipe(
              Effect.mapError(
                toPersistenceDecodeError(
                  `EngineRuntimeEvent.readThreadEvents(sequence=${row.sequence})`,
                ),
              ),
            );
            return { sequence: row.sequence, event } satisfies PersistedProviderRuntimeEvent;
  ...
  const readAcceptedOpenTurnEvents: EngineRuntimeEventRepositoryShape["readAcceptedOpenTurnEvents"] =
    (input) => {
      const limit = Math.max(1, Math.min(1_000, Math.floor(input.limit)));
      return Effect.gen(function* () {
        const rows = yield* sql<Record<string, unknown>>`
          ...
          LIMIT ${limit}
        `.pipe(
          Effect.mapError(toPersistenceSqlError("EngineRuntimeEvent.readAcceptedOpenTurnEvents")),
        );
        return yield* Effect.forEach(
          rows,
          (unknownRow) =>
            Effect.gen(function* () {
              const row = yield* decodeStoredRow(unknownRow).pipe(
                Effect.mapError(
                  toPersistenceDecodeError("EngineRuntimeEvent.readAcceptedOpenTurnEvents.row"),
                ),
              );
              const event = yield* decodeEvent(row.eventJson).pipe(
                Effect.mapError(
                  toPersistenceDecodeError(
                    `EngineRuntimeEvent.readAcceptedOpenTurnEvents(sequence=${row.sequence})`,
                  ),
                ),
              );
              return { sequence: row.sequence, event } satisfies PersistedProviderRuntimeEvent;
  ...
  const pruneSettledOpenTurns: EngineRuntimeEventRepositoryShape["pruneSettledOpenTurns"] = sql`
    ...
    `.pipe(
    Effect.asVoid,
    Effect.mapError(toPersistenceSqlError("EngineRuntimeEvent.pruneSettledOpenTurns")),
  );

  const getConsumerCursor: EngineRuntimeEventRepositoryShape["getConsumerCursor"] = (
    consumerName,
  ) =>
    sql<{ readonly lastAckedSequence: number }>`
        ...
        rows[0] === undefined
          ? Effect.fail(
              new PersistenceDecodeError({
                operation: "EngineRuntimeEvent.getConsumerCursor",
      ...
      Effect.mapError((error) =>
        error instanceof PersistenceDecodeError
          ? error
          : toPersistenceSqlError("EngineRuntimeEvent.getConsumerCursor")(error),
      ),
    );

  const hasPendingEventsForThreads: EngineRuntimeEventRepositoryShape["hasPendingEventsForThreads"] =
    (input) => {
      ...
      return Effect.gen(function* () {
        ...
        const rows = yield* sql<{ readonly present: number }>`
          ...
          LIMIT 1
        `.pipe(
          Effect.mapError(toPersistenceSqlError("EngineRuntimeEvent.hasPendingEventsForThreads")),
        );
  ...
  const advanceConsumerCursor: EngineRuntimeEventRepositoryShape["advanceConsumerCursor"] = (
    input,
  ) => {
    ...
    return sql
        ...
        Effect.gen(function* () {
          ...
          if (
            ...
            input.eventSequence - lastRetentionScanSequence <
              ENGINE_RUNTIME_EVENT_RETENTION_SCAN_INTERVAL
          ) {
            return true;
          }
      ...
      .pipe(
        ...
        Effect.mapError(toPersistenceSqlError("EngineRuntimeEvent.advanceConsumerCursor")),
      );
  };
  ...
  } satisfies EngineRuntimeEventRepositoryShape;
});

export const EngineRuntimeEventRepositoryLive = Layer.effect(EngineRuntimeEventRepository, make);
