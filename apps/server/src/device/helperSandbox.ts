 ...
 *
 * Fails closed when the profile is missing. The helper drives private simulator
 * APIs and model-controlled HID input; a packaging mistake must not silently
 * widen its filesystem or network authority. Development can opt out explicitly
 * with {@link SANDBOX_OPT_OUT_ENV} while diagnosing a profile rule.
 */
export async function sandboxedHelperCommand(
  ...
  if (!readable) {
    throw new Error(
      `Device helper sandbox profile is missing or unreadable at ${profilePath}. ` +
        `Set ${SANDBOX_OPT_OUT_ENV}=1 only for an explicit unconfined development run.`,
    );
  }
