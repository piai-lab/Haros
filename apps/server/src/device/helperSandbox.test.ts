...
describe("sandboxedHelperCommand", () => {
  ...
  it.runIf(darwin)("fails closed when the profile is missing", async () => {
    await expect(
      sandboxedHelperCommand([context.binaryPath], {
        ...context,
        helperSourceDir: "/nonexistent/device-helper",
      }),
    ).rejects.toThrow(/sandbox profile is missing or unreadable/iu);
  });
});
