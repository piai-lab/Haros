...
// Purpose: Compute Profile-page stats from Haros's local projection DB only.
...
// Builds profile skill rows from every stored Haros user message, plus the
...
const makeProfileStatsQuery = Effect.gen(function* () {
  ...
  // Activity = days/hours the user actually sent a Haros prompt. One day-hour
  ...
  // Token usage for EVERY engine, straight from Haros's own DB (no external
