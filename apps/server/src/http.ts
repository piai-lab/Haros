        ...
        return HttpServerResponse.jsonUnsafe(
          { error: authorization.reason === "unavailable" ? "Not Found" : "Unauthorized" },
          {
            ...
            ...(authorization.status === 401
              ? { headers: { "WWW-Authenticate": 'Bearer realm="harnessos-desktop-shutdown"' } }
              : {}),
          },
        );
      }
      ...
      return HttpServerResponse.jsonUnsafe(
        {
          ...
          // This route is unauthenticated. lastFailure may embed raw decoded
          // event values, so expose only shape-level recovery diagnostics.
          projection: {
...
const threadExportEffectRouteLayer = HttpRouter.add(
  ...
  Effect.gen(function* () {
    ...
    // app fetches cross-origin (harnessos://app), and without them the browser masks
