...
export function getHarosModelRuntimeMutationRevision(agentDir: string): number {
  return revisions.get(key(agentDir)) ?? 0;
}

export function publishHarosModelRuntimeMutation(agentDir: string): number {
