...
import {
  parseEngineEventFixture,
  EngineEventFixtureError,
  replayEngineEventFixture,
  sanitizeEngineEventFixtureEvents,
  serializeEngineEventFixture,
} from "./engineEventFixture";

describe("engine event fixtures", () => {
  it("redacts content and preserves deterministic identifier relationships", () => {
    const sanitized = sanitizeEngineEventFixtureEvents([
      {
  ...
  it("only preserves explicitly classified root protocol discriminators", () => {
    for (const event of [
      ...
      expect(() => serializeEngineEventFixture([event])).toThrow(/unclassified string field/u);
    }
  });

  it("does not trust discriminator-looking strings inside engine payloads", () => {
    expect(() =>
      serializeEngineEventFixture([{ type: "custom.event", status: "private-project-codename" }]),
    ).toThrow(/unclassified string field/u);
  ...
  it("supports classified Claude and OpenCode payload shapes", () => {
    const [claude, openCode] = sanitizeEngineEventFixtureEvents([
      {
        type: "stream_event",
        payload: {
          type: "result",
  ...
  it("rejects non-object events and non-finite numbers before serialization", () => {
    for (const event of [null, 1, []]) {
      expect(() => serializeEngineEventFixture([event])).toThrow(/events must be objects/u);
    }
    expect(() =>
      serializeEngineEventFixture([{ type: "custom.event", attempt: Number.NaN }]),
    ).toThrow(/numbers must be finite/u);
    expect(() =>
      serializeEngineEventFixture([{ type: "custom.event", usage: [Infinity] }]),
    ).toThrow(/numbers must be finite/u);
  });
  ...
  it("round-trips a sanitized line-oriented fixture", () => {
    const serialized = serializeEngineEventFixture([
      {
    ...
    const records = parseEngineEventFixture(serialized);
  ...
  it("rejects malformed, version-mismatched, out-of-order, and non-object events", () => {
    expect(() => parseEngineEventFixture("not-json")).toThrow(EngineEventFixtureError);
    expect(() =>
      parseEngineEventFixture(JSON.stringify({ version: 2, index: 0, event: {} })),
    ).toThrow(/unsupported version/u);
    expect(() =>
      parseEngineEventFixture(JSON.stringify({ version: 1, index: 1, event: {} })),
    ).toThrow(/out-of-order index/u);
    for (const event of [null, "event", 1, []]) {
      expect(() =>
        parseEngineEventFixture(JSON.stringify({ version: 1, index: 0, event })),
  ...
  it("enforces byte and event-count bounds", () => {
    ...
    expect(() => parseEngineEventFixture(one, { maxBytes: 1 })).toThrow(/exceeds 1 bytes/u);
    expect(() => parseEngineEventFixture(two, { maxEvents: 1 })).toThrow(/exceeds 1 events/u);
    expect(() => serializeEngineEventFixture(Array.from({ length: 2_001 }, () => ({})))).toThrow(
  ...
  it("replays records sequentially in fixture order", async () => {
    const records = parseEngineEventFixture(
      [
    ...
    await replayEngineEventFixture(records, consume);
