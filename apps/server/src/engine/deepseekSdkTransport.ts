...
export const DEEPSEEK_SDK_MAX_FRAME_BYTES = 16 * 1024 * 1024;
export const DEEPSEEK_SDK_MAX_QUEUED_STDIN_BYTES = 32 * 1024 * 1024;

export type DeepSeekSdkTransportErrorReason =
  | "frame-too-large"
  | "invalid-utf8"
  | "unterminated-frame"
  | "read-closed"
...
export class DeepSeekSdkTransportError extends Error {
  readonly reason: DeepSeekSdkTransportErrorReason;
  ...
  constructor(input: {
    ...
    this.name = "DeepSeekSdkTransportError";
...
export class DeepSeekJsonlFramer {
  private readonly chunks: Buffer[] = [];
  private readonly decoder = new TextDecoder("utf-8", { fatal: true });
  private frameBytes = 0;
  private ended = false;

  constructor(readonly maxFrameBytes = DEEPSEEK_SDK_MAX_FRAME_BYTES) {
    if (!Number.isSafeInteger(maxFrameBytes) || maxFrameBytes <= 0) {
      throw new RangeError("DeepSeek JSONL frame budget must be a positive safe integer");
    }
  }

  push(chunk: Buffer | Uint8Array | string): ReadonlyArray<string> {
    if (this.ended) {
      throw new DeepSeekSdkTransportError({
...
export class DeepSeekJsonlWriter {
  ...
  constructor(
    ...
    readonly maxFrameBytes = DEEPSEEK_SDK_MAX_FRAME_BYTES,
    readonly maxQueuedBytes = DEEPSEEK_SDK_MAX_QUEUED_STDIN_BYTES,
  ) {
    if (
      ...
      throw new RangeError("DeepSeek stdin budgets must be positive and queue >= frame");
    }
  }
  ...
  write(message: unknown): Promise<void> {
    ...
    if (encoded === undefined) {
      return Promise.reject(new TypeError("DeepSeek SDK message is not JSON serializable"));
    }
  ...
  private closedError(observedBytes: number, cause?: unknown): DeepSeekSdkTransportError {
    return new DeepSeekSdkTransportError({
      reason: "write-closed",
      maxBytes: this.maxQueuedBytes,
      observedBytes,
...
function writeWithDrain(writable: Writable, frame: Buffer, signal: AbortSignal): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    ...
    const onClose = () => fail(new Error("DeepSeek SDK stdin closed during write"));
    const onAbort = () => fail(signal.reason ?? new Error("DeepSeek SDK stdin write aborted"));
    const onDrain = () => {
      drainComplete = true;
      settle();
    };
...
function transportErrorMessage(input: {
  ...
  switch (input.reason) {
    case "invalid-utf8":
      return `DeepSeek SDK emitted invalid UTF-8 (${input.observedBytes} bytes).`;
    case "read-closed":
      return "DeepSeek SDK stdout closed before process shutdown.";
    case "unterminated-frame":
      return `DeepSeek SDK stdout ended with an unterminated JSONL frame (${input.observedBytes}/${input.maxBytes} bytes).`;
    case "frame-too-large":
      return `DeepSeek SDK JSONL frame exceeded its byte limit (${input.observedBytes}/${input.maxBytes}).`;
    case "write-overloaded":
      return `DeepSeek SDK stdin queue exceeded its byte limit (${input.observedBytes}/${input.maxBytes}).`;
    case "write-closed":
      return "DeepSeek SDK stdin closed before the frame was written.";
  }
}
