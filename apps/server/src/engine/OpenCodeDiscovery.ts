...
type OpenCodeInventoryProvider = OpenCodeModelInventory["engineList"]["all"][number];
...
export function resolvePreferredOpenCodeModelProviders(input: {
  ...
  const connected = new Set(inventory.engineList.connected);
  const connectedProviders = inventory.engineList.all.filter((engine) => connected.has(engine.id));
...
export function mergeOpenCodeCliModelDescriptors(input: {
  ...
  const providerById = new Map(
    input.inventory.engineList.all.map((engine) => [engine.id, engine] as const),
  );
...
export function buildOpenCodeModelContextLimitMap(
  ...
  for (const engine of inventory.engineList.all) {
