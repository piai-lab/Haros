...
describe("engine attachment paths", () => {
  it.each([
    ...
    expect(resolved && resolveEngineAttachmentPath({ attachmentsDir, attachment: resolved })).toBe(
      storagePath,
    );
  });
  ...
  it("carries only the server-resolved path through engine schema decoding", async () => {
    ...
    const [carried] = carryEngineAttachmentPaths(rawInput, decoded.attachments ?? []);

    expect(carried && resolveEngineAttachmentPath({ attachmentsDir, attachment: carried })).toBe(
      storagePath,
    );
  });
});
