...
function completedEvent(eventId: string): EngineRuntimeEvent {
  return {
    type: "turn.completed",
...
describe("engineRuntimeEventPump", () => {
  it("retries the current event before consuming the next queue item", async () => {
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          const queue = yield* Queue.unbounded<EngineRuntimeEvent>();
          const completed = yield* Deferred.make<void>();
          const health = makeEngineRuntimeEventPumpHealthRegistry(["codex"]);
          const processed: string[] = [];
          let attempts = 0;

          const fiber = yield* runEngineRuntimeEventPump({
            engine: "codex",
