import { EventId, RuntimeTaskId, ThreadId, type EngineRuntimeEvent } from "@harnessos/contracts";
...
describe("assignDerivedEngineRuntimeEventIds", () => {
  it("keeps singleton native ids unchanged", () => {
    ...
    expect(assignDerivedEngineRuntimeEventIds([event])).toEqual([event]);
  });
  ...
  it("assigns stable distinct ids when one native event expands", () => {
    ...
    ] satisfies ReadonlyArray<EngineRuntimeEvent>;

    const assigned = assignDerivedEngineRuntimeEventIds(events);
