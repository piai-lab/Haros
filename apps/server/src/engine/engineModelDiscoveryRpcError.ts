// FILE: engineModelDiscoveryRpcError.ts
...
function statusForProvider(
  ...
  engine: EngineKind,
): ServerEngineStatus | undefined {
  return statuses.find((status) => status.engine === engine);
}

export function toEngineModelDiscoveryRpcError(input: {
  readonly cause: unknown;
  readonly engine: EngineKind;
  readonly statuses: ReadonlyArray<ServerEngineStatus>;
}): WsRpcError {
  if (
