...
const CODEX_DEFINITION = {
  ...
  nativeUpdate: null,
} as const satisfies PackageManagedEngineMaintenanceDefinition;
...
describe("engineMaintenance", () => {
  ...
  it("resolves npm global update commands for unqualified binaries", () => {
    const capabilities = resolvePackageManagedEngineMaintenance(CODEX_DEFINITION, {
      binaryPath: "codex",
  ...
  it("pins the npm global prefix that owns the detected binary", () => {
    ...
    // second node install (e.g. nvm) would receive the update while Haros
  ...
  it("keeps native engine update truth with the engine when explicitly configured", () => {
    const capabilities = resolvePackageManagedEngineMaintenance(CLAUDE_DEFINITION, {
      binaryPath: "claude",
  ...
  it("uses engine-native update commands with detected install method", () => {
    const capabilities = resolvePackageManagedEngineMaintenance(OPENCODE_DEFINITION, {
      binaryPath: "opencode",
  ...
  describe("resolveEngineMaintenanceCapabilitiesEffect", () => {
    function runWithVirtualFileSystem(
      presentPaths: ReadonlySet<string>,
      options: Parameters<typeof resolveEngineMaintenanceCapabilitiesEffect>[1],
    ) {
      const probed: string[] = [];
      ...
      return Effect.runPromise(
        resolveEngineMaintenanceCapabilitiesEffect(CODEX_DEFINITION, options).pipe(
    ...
    it("resolves explicit Homebrew symlinks before selecting a cask variant", async () => {
      ...
      const capabilities = await Effect.runPromise(
        resolveEngineMaintenanceCapabilitiesEffect(CLAUDE_DEFINITION, {
          binaryPath: "/opt/homebrew/bin/claude",
          platform: "darwin",
    ...
    it("does not offer one-click update when the native CLI cannot be located", async () => {
      ...
      const capabilities = await Effect.runPromise(
        resolveEngineMaintenanceCapabilitiesEffect(DROID_DEFINITION, {
          platform: "darwin",
  ...
  it("marks older semver versions as behind latest", () => {
    const advisory = createEngineVersionAdvisory({
      engine: "codex",
  ...
  it("reports an unknowable latest version for self-updating engines", () => {
    ...
    const advisory = createEngineVersionAdvisory({
      ...
      maintenanceCapabilities: makeEngineMaintenanceCapabilities({
        engine: "cursor",
        packageName: null,
