...
function makeFakeCodexAdapter(
  ...
  const emitEffect = (event: LegacyEngineRuntimeEvent): Effect.Effect<void> =>
...
function makeEngineServiceLayer(
  options?: Parameters<typeof makeEngineServiceLive>[0],
  engines?: {
  ...
  const engineAdapterLayer = Layer.succeed(EngineAdapterRegistry, registry);
...
const routing = makeEngineServiceLayer(undefined, { includePi: true });
...
const rotationRetry = makeEngineServiceLayer({
...
const restartRollbackRouting = makeEngineServiceLayer(undefined, {
  includeRestartRollbackDroid: true,
});
const piInteractionRouting = makeEngineServiceLayer(undefined, {
  includePi: true,
});
const staleSettlementPersistedEvents = new Map<string, EngineRuntimeEvent>();
const staleSettlementRouting = makeEngineServiceLayer({
  persistRuntimeEvent: (event) =>
    Effect.sync(() => {
...
const lockCorrectionRouting = makeEngineServiceLayer(
  {
    persistRuntimeEvent: (event) =>
      Effect.sync(() => {
...
const cursorClearRaceRouting = makeEngineServiceLayer({
  persistRuntimeEvent: (event) =>
    Effect.suspend(() => {
...
const bindingRetryRouting = makeEngineServiceLayer({
  persistRuntimeEvent: (event) =>
    Effect.sync(() => {
      const eventId = String(event.eventId);
...
staleSettlementRouting.layer("EngineServiceLive exact stale-terminal settlement", (it) => {
  ...
  it.effect("drops stale terminals that cannot prove exact durable ownership", () =>
    Effect.gen(function* () {
      ...
      ] satisfies LegacyEngineRuntimeEvent[]) {
...
it.effect("EngineServiceLive keeps persisted resumable sessions on startup", () =>
  Effect.gen(function* () {
    ...
    const engineLayer = makeEngineServiceLive().pipe(
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const firstEngineLayer = makeEngineServiceLive().pipe(
      ...
      const secondEngineLayer = makeEngineServiceLive().pipe(
...
routing.layer("EngineServiceLive routing", (it) => {
  ...
  it.effect("publishes the restored same-engine generation before the adapter can emit", () =>
    Effect.gen(function* () {
      ...
      assert.equal(starting?.lifecycleGeneration, restoreInput.lifecycleGeneration);
      assert.equal(
        asRuntimePayloadRecord(starting?.runtimePayload)[HOST_GATEWAY_CREDENTIAL_ROTATION_REQUIRED],
        false,
      );
  ...
  it.effect("keeps ownership unknown when a half-started replacement cannot be retired", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        asRuntimePayloadRecord(uncertainBinding?.runtimePayload).replacementTargetEngine,
        "claude",
      );
    ...
    Effect.gen(function* () {
      ...
      yield* directory.upsert({
        ...
        runtimePayload: {
          ...
          replacementTargetEngine: "claude",
        },
      });
      yield* routing.codex.waitForRuntimeSubscribers();
      routing.codex.emit({
  ...
  it.effect("routes engine operations and rollback conversation", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        asRuntimePayloadRecord(fencedBinding?.runtimePayload).hostGatewayCredentialRotationRequired,
        true,
      );
      ...
      assert.equal(
        asRuntimePayloadRecord(recoveredBinding?.runtimePayload)
          .hostGatewayCredentialRotationRequired,
        false,
      );
  ...
  it.effect("ignores subagent-scoped runtime events for the parent binding", () =>
    Effect.gen(function* () {
      ...
      // identity in engineRefs. Neither may clear the parent's active turn.
      const subagentRefs = {
  ...
  it.effect("marks persisted runtime bindings errored on runtime errors", () =>
    Effect.gen(function* () {
      ...
      routing.codex.emit({
        ...
        threadId: session.threadId,
        turnId: turn.turnId,
        payload: { message: "Engine crashed", class: "engine_error" },
      });
      yield* sleep(50);
...
const idleCleanup = makeEngineServiceLayer({ runtimeIdleStopMs: 100 });
...
const fanout = makeEngineServiceLayer();
fanout.layer("EngineServiceLive fanout", (it) => {
  it.effect("fans out adapter turn completion events", () =>
    Effect.gen(function* () {
      ...
      const completedEvent: LegacyEngineRuntimeEvent = {
        type: "turn.completed",
  ...
  it.effect("keeps subscriber delivery ordered and isolates failing subscribers", () =>
    Effect.gen(function* () {
      ...
      const events: ReadonlyArray<LegacyEngineRuntimeEvent> = [
        {
          type: "tool.completed",
...
const persistedFanout = makeEngineServiceLayer({
  persistRuntimeEvent: (event) =>
    Effect.sync(() => ({
...
const validation = makeEngineServiceLayer();
...
const boundedFanout = makeEngineServiceLayer({
  runtimeEventBufferCapacity: 1,
});
...
const liveFallback = makeEngineServiceLayer();
