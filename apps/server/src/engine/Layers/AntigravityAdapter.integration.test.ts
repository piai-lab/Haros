...
} from "../../hostGateway/Services/HostGatewayCredentials";
...
describe("Antigravity CLI integration helpers", () => {
  it("rotates the gateway lease per print turn and rejects a retained prior bootstrap", async () => {
    ...
    const credentials: HostGatewayCredentialsShape = {
    ...
    try {
      await Effect.runPromise(
        Effect.gen(function* () {
          ...
          const bootstrapA = spawnedEnvironments[0]?.HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN;
          ...
          const bootstrapB = spawnedEnvironments[1]?.HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN;
          ...
          Effect.provide(
            makeAntigravityAdapterLive({
              ...
              Layer.provide(Layer.succeed(HostGatewayCredentials, credentials)),
              Layer.provideMerge(
      ...
      expect(
        ...
        mcpServers: {
          harnessos: {
            ...
            env: {
              HARNESSOS_HOST_GATEWAY_URL: "$HARNESSOS_HOST_GATEWAY_URL",
              HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN: "$HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN",
  ...
  it("gives an Antigravity turn only its thread-scoped gateway credential", () => {
    const env = buildAntigravityTurnProcessEnvironment({
      ...
      baseEnv: {
        ...
        HARNESSOS_HOST_GATEWAY_URL: "http://127.0.0.1:9999/stale",
        HARNESSOS_HOST_GATEWAY_TOKEN: "stale-token",
    ...
    expect(env).toEqual({
      ...
      HOME: "/home/test",
      GEMINI_API_KEY: "gemini-key",
      HARNESSOS_HOST_GATEWAY_URL: "http://127.0.0.1:3773/mcp",
      HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN: "thread-a-bootstrap",
  ...
  it("advertises canonical browser tools only while the session owns a gateway lease", () => {
    ...
    const envWithoutLease = buildAntigravityTurnProcessEnvironment({
      ...
      baseEnv: {
        HARNESSOS_HOST_GATEWAY_URL: "http://127.0.0.1:9999/stale",
        HARNESSOS_HOST_GATEWAY_TOKEN: "stale-token",
        HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN: "stale-bootstrap",
      },
    });
    expect(envWithoutLease.HARNESSOS_HOST_GATEWAY_URL).toBeUndefined();
    expect(envWithoutLease.HARNESSOS_HOST_GATEWAY_TOKEN).toBeUndefined();
    expect(envWithoutLease.HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN).toBeUndefined();
  });
        ...
        Effect.gen(function* () {
          ...
          const eventsFiber = yield* adapter.streamEvents.pipe(
            Stream.takeUntil(
              (event) =>
                ...
                event.engineRefs?.nativeThreadId === "conv-parent-1",
            ),
            Stream.runCollect,
            Effect.forkChild,
          );
          ...
          const childThreadStarted = events.find(
            (event) =>
              ...
              event.engineRefs?.nativeThreadId === "conv-child-1",
          );
          expect(childThreadStarted?.engineRefs).toEqual(childRefs);
          const childTurnStarted = events.find(
            (event) =>
              event.type === "turn.started" && event.engineRefs?.nativeThreadId === "conv-child-1",
          );
          ...
          const childItemStarted = events.find(
            (event) =>
              event.type === "item.started" && event.engineRefs?.nativeThreadId === "conv-child-1",
          );
          expect(childItemStarted?.engineRefs).toEqual(childRefs);
          ...
          const parentItem = events.find(
            (event) =>
              event.type === "item.started" && event.engineRefs?.nativeThreadId === "conv-parent-1",
          );
          expect(parentItem?.engineRefs).toEqual({ nativeThreadId: "conv-parent-1" });
        ...
        Effect.gen(function* () {
          ...
          const eventsFiber = yield* adapter.streamEvents.pipe(
            Stream.takeUntil(
              (event) =>
                event.type === "turn.completed" &&
                event.engineRefs?.nativeParentThreadId === undefined,
            ),
            Stream.runCollect,
            Effect.forkChild,
          );
          ...
          const childTerminalIndex = events.findIndex(
            (event) =>
              event.type === "turn.completed" &&
              event.engineRefs?.nativeThreadId === "conv-child-failure",
          );
