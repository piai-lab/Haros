...
import {
  ...
  startHostGatewaySessionLeaseExitWatcher,
...
const ENGINE = "grok" as const;

export const takeGrokHarosHarnessPolicyTextPart = (
...
const GROK_PLAN_MODE_PROMPT_PREFIX = [
  "Haros requested Grok's native plan mode.",
...
export function buildGrokPromptMeta(interactionMode: EngineInteractionMode): {
  ...
  // the engine state when Haros sends the desired mode again.
...
export function resolveGrokPlanHookResponse(
  ...
  return {
    decision: "deny",
    systemMessage: `Haros Plan mode blocks the mutating or unknown Grok tool "${toolName || "unknown"}".`,
  };
}
...
export function scopeGrokRuntimeItemIdForTurn(turnId: TurnId, itemId: string): string {
  return scopeAcpRuntimeItemIdForTurn(ENGINE, turnId, itemId);
}
...
export function scopeGrokToolCallStateForTurn(
  ...
  return scopeAcpToolCallStateForTurn(ENGINE, turnId, toolCall);
}
...
export function makeGrokAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const requireSession = (
      ...
      if (!ctx || ctx.stopped) {
        return Effect.fail(new EngineAdapterSessionNotFoundError({ engine: ENGINE, threadId }));
      }
    ...
    const stopSessionInternal = (ctx: GrokSessionContext) =>
      Effect.gen(function* () {
        ...
        ctx.stopped = true;
        yield* cancelHostGatewayTurn(ctx.gatewaySessionLease, ctx.activeTurnId);
        ...
        Effect.gen(function* () {
          ...
          const acp = yield* makeGrokAcpRuntime({
            ...
            Effect.mapError((cause) =>
              mapAcpToAdapterError(ENGINE, input.threadId, "session/start", cause),
            ),
          );
          ...
          const started = yield* Effect.gen(function* () {
                ...
                Effect.gen(function* () {
                  ...
                  if (planMarkdown !== undefined) {
                    ...
                    if (
                      ...
                      ctx.lastPlanFingerprint = planMarkdown;
                      // The extension response must reach Grok before Haros cancels the
            ...
            Effect.mapError((error) =>
              mapAcpToAdapterError(ENGINE, input.threadId, "session/start", error),
            ),
          );
          yield* startHostGatewaySessionLeaseExitWatcher(gatewaySessionLease, acp.awaitExit);
          ...
          yield* Effect.gen(function* () {
            yield* applyRequestedEngineSelection({
              ...
              mapError: ({ cause, method }) =>
                mapAcpToAdapterError(ENGINE, input.threadId, method, cause),
            });
    ...
    const failGrokTurnAsTimedOut = (ctx: GrokSessionContext, turnId: TurnId, idleMs: number) =>
      Effect.gen(function* () {
        ...
        yield* cancelHostGatewayTurn(ctx.gatewaySessionLease, turnId);
        if (!clearAcpActiveTurn(ctx, turnId)) {
          return;
        }
        const completedCost = finalizeAcpActiveTurnCost(ctx);
    ...
    const startGrokTurn = (
      ...
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeGrokHarosHarnessPolicyTextPart(
          ctx,
        ...
        const runPrompt = Effect.suspend(() =>
          ...
          Effect.mapError((error) =>
            mapAcpToAdapterError(ENGINE, input.threadId, "session/prompt", error),
          ),
          Effect.matchEffect({
            onFailure: (error) =>
              Effect.gen(function* () {
        ...
        yield* withHostGatewayTurnCancellation(
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.ignore(
              ctx.acp.cancel.pipe(
                Effect.mapError((error) =>
                  mapAcpToAdapterError(ENGINE, threadId, "session/cancel", error),
                ),
              ),
            );
            if (activePromptFiber) {
    ...
    const runGrokCompaction = (ctx: GrokSessionContext) =>
      Effect.gen(function* () {
        ...
        const compactResult = yield* runGrokAcpCompactionCommand(ctx.acp).pipe(
          Effect.mapError((error) =>
            mapAcpToAdapterError(ENGINE, ctx.threadId, "session/prompt", error),
          ),
      ...
      Effect.gen(function* () {
        ...
        const forkRuntime = (runtime: AcpSessionRuntimeShape) =>
          forkViaAcpRuntime({
            ...
            targetCwd,
            unsupportedIssue:
              "This Grok ACP version does not advertise session/fork; Haros will rebuild the fork from its retained transcript.",
        ...
        if (activeSource?.activeTurnId !== undefined) {
          return yield* new EngineAdapterValidationError({
            ...
            operation: "forkThread",
            issue:
              "The source Grok session has a turn in flight; Haros will rebuild the fork from its retained transcript.",
          });
        }
        ...
        Effect.mapError((cause) =>
          ...
          cause instanceof EngineAdapterValidationError
            ? cause
            : mapAcpToAdapterError(ENGINE, input.sourceThreadId, "session/fork", cause),
        ),
      );
