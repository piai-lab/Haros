...
function makeMultiQueryHarness(config?: {
  ...
  readonly gatewayCredentials?: HostGatewayCredentialsShape;
}) {
  ...
  if (config?.gatewayCredentials) {
    layer = layer.pipe(
      Layer.provideMerge(Layer.succeed(HostGatewayCredentials, config.gatewayCredentials)),
    );
  }
...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("routes subagent-tagged messages to a child engine thread", () => {
    ...
    return Effect.gen(function* () {
      ...
      const runtimeEventsFiber = yield* adapter.streamEvents.pipe(
        Stream.takeUntil(
          (event) =>
            event.type === "turn.completed" && event.engineRefs?.nativeThreadId === undefined,
        ),
        Stream.runCollect,
        Effect.forkChild,
      );
      ...
      const childEvents = runtimeEvents.filter(
        (event) => event.engineRefs?.nativeThreadId === "tool-task-1",
      );
      assert.equal(
        childEvents.every((event) => event.engineRefs?.nativeParentThreadId === THREAD_ID),
        true,
      );
      assert.equal(
      ...
      assert.equal(
        textDeltas.every((event) => event.engineRefs?.nativeThreadId === "tool-task-1"),
        true,
      );
      ...
      assert.equal(
        usageEvents.every((event) => event.engineRefs?.nativeThreadId === "tool-task-1"),
        true,
      );
  ...
  it.effect("keeps async Bash progress on the parent thread", () => {
    ...
    return Effect.gen(function* () {
      ...
      const runtimeEvents = Array.from(yield* Fiber.join(runtimeEventsFiber));
      assert.equal(
        runtimeEvents.some((event) => event.engineRefs?.nativeThreadId !== undefined),
        false,
      );
      ...
      assert.equal(progress?.engineRefs?.nativeThreadId, undefined);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect("projects a complete-message subagent conversation onto the child thread", () => {
    ...
    return Effect.gen(function* () {
      ...
      const toolStarted = childEvents.find(
        (event) =>
          event.type === "item.started" && event.engineRefs?.nativeItemId === "tool-grep-1",
      );
      assert.equal(toolStarted?.type, "item.started");
      if (toolStarted?.type === "item.started") {
      ...
      const toolCompleted = childEvents.find(
        (event) =>
          event.type === "item.completed" && event.engineRefs?.nativeItemId === "tool-grep-1",
      );
      ...
      assert.equal(
        runtimeEvents.some(
          (event) =>
            event.engineRefs?.nativeThreadId === undefined &&
            event.engineRefs?.nativeItemId === "tool-grep-1",
        ),
        false,
      );
    }).pipe(
  ...
  it.effect("drops zombie-tagged messages after a subagent task settles", () => {
    ...
    return Effect.gen(function* () {
      ...
      for (const toolUseId of ["tool-task-zombie", "tool-task-zombie2"]) {
        const childEvents = runtimeEvents.filter(
          (event) => event.engineRefs?.nativeThreadId === toolUseId,
        );
      ...
      const stoppedItemCompleted = runtimeEvents.find(
        (event) =>
          event.type === "item.completed" && event.engineRefs?.nativeItemId === "tool-task-zombie",
      );
  ...
  it.effect("delivers queued subagent steers through the PreToolUse hook", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (steered?.type === "turn.steered") {
        ...
        assert.equal(steered.engineRefs?.nativeThreadId, "tool-task-steer-1");
        assert.equal(steered.engineRefs?.nativeParentThreadId, THREAD_ID);
      }
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
...
`;
  ...
  it.effect("maps task_updated status patches onto the subagent thread", () => {
    ...
    return Effect.gen(function* () {
      ...
      const runtimeEventsFiber = yield* adapter.streamEvents.pipe(
        Stream.takeUntil(
          (event) =>
            ...
            event.engineRefs?.nativeThreadId === "tool-task-2",
        ),
        Stream.runCollect,
        Effect.forkChild,
      );
      ...
      if (stateChanged?.type === "session.state.changed") {
        ...
        assert.equal(stateChanged.engineRefs?.nativeParentThreadId, THREAD_ID);
      }
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
  ...
  it.effect("restarts the Claude process after an authentication failure", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (runtimeError?.type === "runtime.error") {
        assert.equal(runtimeError.payload.class, "engine_error");
  ...
  it.effect("invalidates a missing resumed conversation reported by the async stream", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (turnCompleted?.type === "turn.completed") {
        assert.equal(String(turnCompleted.turnId), String(turn.turnId));
        assert.equal(turnCompleted.payload.state, "failed");
        assert.equal(turnCompleted.engineRefs?.nativeThreadId, undefined);
      }
      assert.equal(yield* adapter.hasSession(THREAD_ID), false);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
  ...
  it.effect("keeps Auto reviewer-gated after accepting one request for the session", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepEqual(requested.value.engineRefs, {
      ...
      assert.deepEqual(resolved.value.engineRefs, {
        nativeItemId: EngineItemId.makeUnsafe("tool-use-1"),
      });
  ...
  it.effect("captures ExitPlanMode as a proposed plan and denies auto-exit", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepEqual(proposedEvent.value.engineRefs, {
  ...
  it.effect("handles AskUserQuestion via user-input.requested/resolved lifecycle", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepEqual(requestedEvent.value.engineRefs, {
      ...
      assert.deepEqual(resolvedEvent.value.engineRefs, {
        nativeItemId: EngineItemId.makeUnsafe("tool-ask-1"),
      });
