...
// Purpose: Characterizes Cursor's private Haros host-policy delivery.
// Layer: Engine adapter tests

import { HARNESSOS_HARNESS_POLICY_MARKER } from "../../hostGateway/harnessPolicy.ts";
import { describe, expect, it } from "vitest";

import { takeCursorHarosHarnessPolicyTextPart } from "./CursorAdapter.ts";

describe("Cursor Haros harness policy", () => {
  it("delivers scoped MCP host context exactly once per fresh/load/fork session", () => {
    for (const lifecycle of ["fresh", "load", "fork"] as const) {
      const state: { harnessPolicyDelivered?: boolean } = {};
      const first = takeCursorHarosHarnessPolicyTextPart(state, true);
      ...
      expect(takeCursorHarosHarnessPolicyTextPart(state, true), lifecycle).toBeNull();
    }
  });

  it("stays truthful without a scoped gateway connection", () => {
    expect(takeCursorHarosHarnessPolicyTextPart({}, false)?.text).toContain(
      "Haros MCP control is unavailable",
    );
  });
});
