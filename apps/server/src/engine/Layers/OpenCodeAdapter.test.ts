    ...
    mcp: {
      add: async (input: Record<string, unknown>) => {
        ...
        return options?.mcpAdd
          ...
          : { data: { harnessos: { status: "connected" } } };
      },
    },
  };
  ...
  const runtime: OpenCodeRuntimeShape = {
    ...
    loadOpenCodeInventory: () =>
      options?.inventoryError
        ...
        : Effect.succeed(
            options?.inventory ?? {
              engineList: { connected: [], all: [], default: {} },
              agents: [],
              consoleState: null,
            },
          ),
...
describe("OpenCodeAdapter runtime lifecycle", () => {
  ...
  it("isolates same-cwd managed sessions, injects distinct gateway tokens, and revokes them", async () => {
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        Effect.provide(
          makeOpenCodeAdapterLive({ runtime: runtime.runtime }).pipe(
            Layer.provide(Layer.succeed(HostGatewayCredentials, gateway.credentials)),
            Layer.provideMerge(
              ServerConfig.layerTest(process.cwd(), { prefix: "opencode-adapter-test-" }),
            ),
            Layer.provideMerge(NodeServices.layer),
    ...
    const runtime = createMockOpenCodeRuntime({
      mcpAdd: async (input) => {
        ...
        if (config?.enabled === false) {
          return { data: { harnessos: { status: "disabled" } } };
        }
        activeSetupAttempts += 1;
        return activeSetupAttempts === 1
          ? { data: { harnessos: { status: "failed", error: "gateway unavailable" } } }
          : { data: { harnessos: { status: "connected" } } };
      },
    });
    const gateway = makeGatewayCredentials();
  ...
  it("keeps managed sessions identity-only and revokes credentials when MCP setup is not connected", async () => {
    const runtime = createMockOpenCodeRuntime({
      mcpAdd: async () => ({ data: { harnessos: { status: "failed", error: "offline" } } }),
    });
    const gateway = makeGatewayCredentials();
  ...
  it("normalizes OpenCode running titles without blocking completed child tool activity", async () => {
    ...
    const decodeEngineRuntimeEvent = Schema.decodeUnknownSync(EngineRuntimeEvent);
    expect(() => {
      for (const event of result.slice(3)) {
        decodeEngineRuntimeEvent(event);
      }
    }).not.toThrow();
  });
