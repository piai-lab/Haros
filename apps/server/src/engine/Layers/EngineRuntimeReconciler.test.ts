...
import { EngineRuntimeReconciler } from "../Services/EngineRuntimeReconciler.ts";
...
import { makeProviderRuntimeReconcilerLive } from "./EngineRuntimeReconciler.ts";
...
function readyEngineSession(): EngineSession {
  return {
    engine: "codex",
    status: "ready",
    runtimeMode: "full-access",
...
describe("EngineRuntimeReconcilerLive", () => {
  it("retries a terminal-session turn repair without reopening the session", async () => {
    ...
    let engineSession = readyEngineSession();
    ...
    } as unknown as EngineSessionDirectoryShape;
    const engineService = {
      listSessions: () => Effect.succeed([engineSession]),
    ...
    } as unknown as EngineRuntimeEventRepositoryShape;
    ...
    const layer = makeProviderRuntimeReconcilerLive({ staleAfterMs: 1 }).pipe(
      ...
      Layer.provide(Layer.succeed(EngineSessionDirectory, directory)),
      Layer.provide(Layer.succeed(EngineService, engineService)),
      Layer.provide(Layer.succeed(EngineRuntimeEventRepository, runtimeEvents)),
    );

    await Effect.gen(function* () {
      const reconciler = yield* EngineRuntimeReconciler;
    ...
    if (activityCommand?.type === "thread.activity.append") {
      expect(activityCommand.activity.kind).toBe("engine.runtime.reconciled");
