...
const validationLayer = it.layer(
  makeCodexAdapterLive({ manager: validationManager }).pipe(
    ...
    Layer.provideMerge(engineSessionDirectoryTestLayer),
...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("prefers manager-assigned turn ids for Codex task events", () =>
    Effect.gen(function* () {
      ...
      assert.equal(firstEvent.value.engineRefs?.nativeTurnId, "turn-parent");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(firstEvent.value.engineRefs?.nativeItemId, "agent_message_9");
      }),
  );
