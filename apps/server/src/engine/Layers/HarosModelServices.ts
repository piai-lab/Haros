...
import type {
  ...
  HarosModelServiceAuthEvent,
  HarosModelServiceAuthPrompt,
  HarosModelServiceAuthResult,
  HarosModelServiceAuthMethodType,
  HarosModelServiceOAuthPromptMode,
  HarosModelServiceAuthSource,
  HarosModelServiceDescriptor,
  HarosModelServiceModel,
  HarosModelServicesExtensionProjectionState,
  HarosModelServicesListResult,
  HarosModelServicesProjectionIntent,
...
import {
  createHarosModelsConfigReader,
  loadModelServicesSdk,
  readModelServicesPrivateTextFile,
  resolveModelServicesAgentDir,
  type ModelServicesSdk,
  type ModelServicesPrivateRuntimeFilename,
...
export interface HarosModelServicesLiveOptions {
...
function clearOmittedCompatAfterApiChange(
  ...
  previous: ReturnType<ModelServicesSdk["ModelRuntime"]["prototype"]["getModelConfigProvider"]>,
): HarosCustomModelServiceConfigInput {
...
): ReadonlyArray<import("@earendil-works/pi-coding-agent").ModelConfigHeaderReferenceMutation> {
...
function projectCustomConfig(
  serviceId: string,
  provider: ReturnType<ModelServicesSdk["ModelRuntime"]["prototype"]["getModelConfigProvider"]>,
  headerMetadata: ReturnType<
    ModelServicesSdk["ModelRuntime"]["prototype"]["getModelConfigProviderHeaderMetadata"]
  >,
...
function projectAuthPrompt(prompt: AuthPrompt): HarosModelServiceAuthPrompt {
...
function projectAuthEvent(event: AuthEvent): HarosModelServiceAuthEvent | null {
...
interface ModelServiceAuthRequest {
  ...
  readonly authType: HarosModelServiceAuthMethodType;
  ...
  readonly events: HarosModelServiceAuthEvent[];
  providerDefaultPromptConsumed: boolean;
  deadlineExpired: boolean;
  pendingPrompt?: PendingAuthPrompt;
  outcome?: HarosModelServiceAuthResult;
...
class StaticCredentialStore implements CredentialStore {
  ...
  info(providerId: string): {
    readonly type: HarosModelServiceAuthMethodType;
...
function normalizeAuthSource(source: string | undefined): HarosModelServiceAuthSource | null {
  switch (source) {
    case "stored":
    case "runtime":
    case "environment":
...
type HarosModelRuntime = Awaited<ReturnType<ModelServicesSdk["ModelRuntime"]["create"]>>;
type HarosExtensionServices = Awaited<ReturnType<ModelServicesSdk["createAgentSessionServices"]>>;

async function loadIntentScopedExtensionServices(input: {
  readonly sdk: ModelServicesSdk;
  readonly runtime: HarosModelRuntime;
...
async function projectModelServices(input: {
  ...
  readonly connectable: ReadonlyArray<HarosModelServiceDescriptor>;
  ...
  readonly extensionProjectionState?: HarosModelServicesExtensionProjectionState;
}> {
  input.signal.throwIfAborted();
  ...
  const readTextFile =
    ...
    ((filePath, signal) => {
      ...
      const filename = path.basename(filePath) as ModelServicesPrivateRuntimeFilename;
      ...
      return readModelServicesPrivateTextFile({
        agentDir,
        filename,
        ...(signal ? { signal } : {}),
      });
  ...
  const modelsConfigReader = input.readTextFile
    ...
    : createHarosModelsConfigReader(agentDir);
  ...
  const modelsByServiceId = new Map<string, ReadonlyArray<HarosModelServiceModel>>();
  const customConfigsByServiceId = new Map<string, HarosCustomModelServiceConfig>();

  const descriptors = runtime.getProviders().map<HarosModelServiceDescriptor>((provider) => {
    ...
    const origin = registeredProviderIds.has(provider.id)
      ...
      : modelConfigProviderIds.has(provider.id)
        ...
        : ("builtin" as const);
    const authMethods: Array<HarosModelServiceDescriptor["authMethods"][number]> = [];
...
function unavailableListResult(): HarosModelServicesListResult {
  return {
    state: "error",
    services: [],
    connectableServices: [],
...
export function makeHarosModelServicesLive(options: HarosModelServicesLiveOptions = {}) {
  return Layer.effect(
    HarosModelServices,
    Effect.gen(function* () {
      ...
      const directoryScope = new AsyncLocalStorage<string>();
      const currentAgentDir = () => {
        const directory = directoryScope.getStore();
        if (!directory) throw new Error("Model-service operation has no configuration scope");
        return directory;
      };
      const scoped = <A>(effect: Effect.Effect<A>): Effect.Effect<A> =>
        Effect.gen(function* () {
          const settings = yield* serverSettings.getSettings.pipe(Effect.orDie);
          const connectionSignal = yield* CurrentWsConnectionSignal;
          const requestedDir =
            settings.engines.pi.agentDir.trim() ||
            (yield* Effect.promise(loadModelServicesSdk)).getAgentDir();
          const agentDir = resolveModelServicesAgentDir(requestedDir, config.baseDir);
          return yield* Effect.promise((signal) =>
            directoryScope.run(agentDir, () =>
              Effect.runPromise(
                effect.pipe(Effect.provideService(CurrentWsConnectionSignal, connectionSignal)),
                { signal },
              ),
            ),
          );
      ...
      const createMutationRuntime = async (signal: AbortSignal) => {
        ...
        const sdk = await (options.loadModule ?? loadModelServicesSdk)();
        signal.throwIfAborted();
        const runtime = await sdk.ModelRuntime.create({
          authPath: path.join(agentDir, "auth.json"),
          modelsPath: null,
          modelsConfigReader: createHarosModelsConfigReader(agentDir),
          ...
          input.credential.type === "preserve"
            ? await StaticCredentialStore.create({
                ...
                readTextFile: (filePath, readSignal) =>
                  readModelServicesPrivateTextFile({
                    agentDir,
                    filename: path.basename(filePath) as ModelServicesPrivateRuntimeFilename,
      ...
      const runLogin = async (request: ModelServiceAuthRequest) => {
        await serializeMutation(async () => {
          ...
          let previousService: HarosModelServiceDescriptor | undefined;
          try {
      ...
      const service = {
              ...
              return first
                ? ({
                    ...
                    ...(projection.extensionProjectionState
                      ...
                      : {}),
                    errorCode: null,
                  } satisfies HarosModelServicesListResult)
                : ({
                    state: "empty",
                    services: [],
                    connectableServices: projection.connectable,
        ...
        discoverCustom: (input) =>
          Effect.promise(async (requestSignal) => {
            ...
            let sdk: ModelServicesSdk | undefined;
            try {
        ...
        testCustom: (input) =>
          Effect.promise(async (requestSignal) => {
            ...
            try {
              ...
              const testedModels = input.config.models.flatMap<HarosModelServiceModel>(
                ({ modelId }) => {
        ...
        saveCustom: (input) =>
          Effect.promise((signal) =>
            serializeMutation(async () => {
              ...
              publishHarosModelRuntimeMutation(agentDir);
              let service: HarosModelServiceDescriptor | null = null;
              try {
                service = await getProjectedService(serviceId, new AbortController().signal);
              } catch {
                synchronizationFailed = true;
      ...
      } satisfies HarosModelServicesShape;
      return {
        ...
        pollLogin: (...args: Parameters<HarosModelServicesShape["pollLogin"]>) =>
          service.pollLogin(...args),
        answerLogin: (...args: Parameters<HarosModelServicesShape["answerLogin"]>) =>
          service.answerLogin(...args),
        cancelLogin: (...args: Parameters<HarosModelServicesShape["cancelLogin"]>) =>
          service.cancelLogin(...args),
        logout: (...args: Parameters<HarosModelServicesShape["logout"]>) =>
          scoped(service.logout(...args)),
        revealApiKey: (...args: Parameters<HarosModelServicesShape["revealApiKey"]>) =>
        ...
        discoverCustom: (...args: Parameters<HarosModelServicesShape["discoverCustom"]>) =>
          scoped(service.discoverCustom(...args)),
        saveCustom: (...args: Parameters<HarosModelServicesShape["saveCustom"]>) =>
          scoped(service.saveCustom(...args)),
        removeCustom: (...args: Parameters<HarosModelServicesShape["removeCustom"]>) =>
          scoped(service.removeCustom(...args)),
      } satisfies HarosModelServicesShape;
    }),
  );
}

export const HarosModelServicesLive = makeHarosModelServicesLive();
