...
import {
  ...
  HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN_ENV,
...
export function buildAntigravityTurnProcessEnvironment(input: {
  ...
  readonly gatewayConnection?: Pick<HostGatewayMcpConnection, "url">;
  ...
  const gatewayKeys = hasGatewayBootstrap
    ? [HARNESSOS_HOST_GATEWAY_URL_ENV, HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN_ENV]
    : [];
  const gatewayEnvironment = hasGatewayBootstrap
    ? {
        [HARNESSOS_HOST_GATEWAY_URL_ENV]: input.gatewayConnection!.url,
        [HARNESSOS_HOST_GATEWAY_BOOTSTRAP_TOKEN_ENV]: input.gatewayBootstrapToken!,
      }
    : {};
...
const makeAntigravityAdapter = (dependencies: AntigravityAdapterDependencies = {}) =>
  Effect.gen(function* () {
    ...
    const base = (
      ...
      ...(context.conversationId ? { engineRefs: { nativeThreadId: context.conversationId } } : {}),
    });
    ...
    const requireSession = (
      ...
      return context
        ...
        : Effect.fail(new EngineAdapterSessionNotFoundError({ engine: ENGINE, threadId }));
    };

    const releaseTurnGatewayLease = (
      context: AntigravitySessionContext,
      lease: HostGatewaySessionLease | undefined = context.gatewaySessionLease,
    ): void => {
      lease?.release();
    ...
    const startSession: AntigravityAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        yield* Effect.tryPromise({
          try: () =>
            (dependencies.ensurePlugin ?? ensureCapturePlugin)(
              binaryPath,
              hostGatewayCredentials?.stdioProxy,
            ),
          catch: (cause) =>
            new EngineAdapterRequestError({
              engine: ENGINE,
        ...
        if (existing) {
          ...
          yield* cancelHostGatewayTurn(existing.gatewaySessionLease, existing.activeTurnId);
    ...
    const sendTurn: AntigravityAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const canBootstrapGateway = hostGatewayCredentials !== undefined;
        ...
        child.once("close", (code, signal) => {
          ...
          void (async () => {
            ...
            await Effect.runPromise(cancelHostGatewayTurn(gatewaySessionLease, completedTurnId));
            if (!ownsTurn()) {
              releaseTurnGatewayLease(context, gatewaySessionLease);
              await cleanupTurnRunDirectory(context, runDir);
              return;
            ...
            if (failed && stderr.trim()) {
              offer({
                ...base(context, { includeTurn: false }),
                type: "runtime.error",
                payload: { message: stderr.trim(), class: "engine_error" },
