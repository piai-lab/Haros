...
import { acquireHostGatewaySessionLease } from "../../hostGateway/sessionLease.ts";
...
import { assignDerivedEngineRuntimeEventIds } from "../engineRuntimeEventIdentity.ts";
...
function codexEventBase(
  ...
  const engineRefs = base.engineRefs
    ? {
        ...base.engineRefs,
  ...
  return {
    ...
    ...(Object.keys(engineRefs).length > 0 ? { engineRefs } : {}),
  };
}
...
function engineRefsFromEvent(event: EngineEvent): EngineRuntimeEvent["engineRefs"] | undefined {
  ...
  return Object.keys(refs).length > 0 ? (refs as EngineRuntimeEvent["engineRefs"]) : undefined;
}
...
function runtimeEventBase(
  ...
  const refs = engineRefsFromEvent(event);
  return {
    eventId: event.id,
    engine: event.engine,
    ...
    return [
      {
        ...
        payload: {
          ...
          ...(!treatAsWarning ? { class: "engine_error" as const } : {}),
      ...
      Effect.gen(function* () {
        ...
        const ingress = yield* makeBoundedCallbackIngress<CodexRuntimeIngressItem, never, never>(
            ...
            }),
          {
            ...
            isTerminal: (item) => item.runtimeEvents.some(isTerminalEngineRuntimeEvent),
        ...
        const listener = (event: EngineEvent) => {
          ...
          try {
            mappedRuntimeEvents = assignDerivedEngineRuntimeEventIds(
          ...
          for (const runtimeEvent of mappedRuntimeEvents) {
            ...
            const settleUnavailableAfterProjection = () => {
              ...
              const runtimeEvent = compactEngineRuntimeEventForIngress(unavailableTerminal);
              ingress.offer({
          ...
          const sizedRuntimeEvents = mappedRuntimeEvents
            ...
            .map(compactEngineRuntimeEventForIngress);
