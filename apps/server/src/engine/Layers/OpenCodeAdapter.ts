...
const installOpenCodeGatewayMcp = Effect.fn("installOpenCodeGatewayMcp")(function* (input: {
  ...
  readonly directory: string;
  readonly displayName: string;
  readonly connection: HostGatewaySessionLease["connection"];
}) {
    ...
    Effect.gen(function* () {
        ...
        if (!context.activeTurnToolCallIdleWatchdogStarted) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (context.activeTurnSawFinalAssistant) {
              ...
              if (
                ...
                (yield* recoverOpenCodeTurnFromMessageId(context, {
                  ...
                  raw: {
                    source: "harnessos.opencode.deferred-idle-completion",
                    event: raw,
                  },
                }))
              ) {
              ...
              if (
                ...
                yield* completeOpenCodeTurn(context, {
                  turnId,
                  raw: {
                    source: "harnessos.opencode.deferred-idle-local-part",
                    event: raw,
                  },
              ...
              if (
                ...
                (yield* recoverOpenCodeTurnFromMessageId(context, {
                  ...
                  raw: {
                    source: "harnessos.opencode.deferred-idle-completion-retry",
                    event: raw,
                  },
                }))
              ) {
              ...
              if (
                ...
                yield* completeOpenCodeTurn(context, {
                  turnId,
                  raw: {
                    source: "harnessos.opencode.deferred-idle-local-part-retry",
                    event: raw,
                  },
                  totalCostUsd: context.latestTurnCostUsd,
                });
            ...
            const completed = yield* completeOpenCodeTurn(context, {
              turnId,
              raw: {
                source: "harnessos.opencode.idle-after-tool-calls",
                event: raw,
              },
      ...
      const recoverOpenCodeTurnFromMessages = Effect.fn("recoverOpenCodeTurnFromMessages")(
        function* (
          ...
          return yield* recoverOpenCodeTurnFromAssistantMessage(context, {
            ...
            assistantEntry,
            raw: {
              source: "harnessos.opencode.prompt.recovery",
      ...
      const startSession: OpenCodeAdapterShape["startSession"] = Effect.fn("startSession")(
        function* (input) {
          ...
          const poolIsolationKey = hostGatewayConnection ? randomUUID() : undefined;
            ...
            (sessionScope) =>
              Effect.gen(function* () {
                ...
                const context: OpenCodeSessionContext = {
                  ...
                  session,
                  gatewayControlAvailable: started.gatewayControlAvailable,
                  ...(started.gatewayControlAvailable && hostGatewaySessionLease
                    ? {
                        gatewaySessionLease: hostGatewaySessionLease,
                      }
                    : {}),
            ...
            (sessionScope) =>
              sessionScopeTransferred
                ...
                : Scope.close(sessionScope, Exit.void).pipe(
                    Effect.ignore,
                    Effect.ensuring(Effect.sync(() => hostGatewaySessionLease?.release())),
                  ),
          );
      ...
      const sendTurnImpl = Effect.fn("sendTurn")(function* (
        ...
        const baseText = withEnginePlanModePrompt({
          ...
          Effect.onError(() => {
            ...
            if (context.activeTurnId === turnId) {
              return clearActiveTurnState(context).pipe(
                Effect.tap(() =>
                  Effect.sync(() =>
                    updateEngineSession(context, { status: "ready" }, { clearActiveTurnId: true }),
                  ),
                ),
              );
            }
        ...
        return Effect.gen(function* () {
          ...
          if (Exit.isSuccess(inventoryExit)) {
            ...
            yield* Effect.logDebug(`${adapterConfig.displayName} model discovery resolved`, {
              binaryPath,
              connectedProviders: inventory.engineList.connected,
