import { createPiModelRuntime } from "./PiAdapter";
import { ServerSettingsService } from "../../serverSettings";
import { resolveModelServicesAgentDir } from "../modelServicesRuntime";
...
import type { ModelServicesSdk } from "../modelServicesRuntime.ts";
...
import { makeHarosModelServicesLive } from "./HarosModelServices.ts";
...
async function makeRoot(): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "harnessos-model-services-"));
  roots.push(root);
  return root;
}
...
function makeTestLayer(input: {
  ...
  return makeHarosModelServicesLive({
    ...
    Layer.provideMerge(
      ServerSettingsService.layerTest({
        engines: { pi: { agentDir: path.join(input.root, "pi-agent") } },
      }),
    ),
...
describe("HarosModelServicesLive", () => {
  it("keeps pending authentication bound to the original Pi directory after Settings changes", async () => {
    const root = await makeRoot();
    await isolateProviderEnvironment(root);
    const originalDir = path.join(root, "pi-agent");
    const nextDir = path.join(root, "next-pi-agent");
    ...
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const begin = yield* service.beginLogin(55, { serviceId: "deepseek", authType: "api_key" });
        if (begin.state !== "prompt") throw new Error("Expected API key prompt");
        yield* settings.updateSettings({ engines: { pi: { agentDir: nextDir } } });
        const answer = yield* service.answerLogin(55, {
          requestId: begin.requestId,
          promptId: begin.prompt.promptId,
          value: "test-only-bound-key",
        });
        const next = yield* service.list();
        return { answer, next };
      }).pipe(Effect.provide(makeTestLayer({ root }))),
    );
    expect(result.answer.state).toBe("complete");
    expect(result.next.state).toBe("empty");
    expect(
      JSON.parse(await readFile(path.join(originalDir, "auth.json"), "utf8")).deepseek.key,
    ).toBe("test-only-bound-key");
    await expect(stat(path.join(nextDir, "auth.json"))).rejects.toMatchObject({ code: "ENOENT" });
  });

  it("rejects retired OA state before configuration can be opened", async () => {
    const root = await makeRoot();
    const retiredDir = path.join(root, "agent");
    await mkdir(retiredDir, { recursive: true });
    const link = path.join(root, "pi-agent");
    await symlink(retiredDir, link, process.platform === "win32" ? "junction" : "dir");
    expect(() => resolveModelServicesAgentDir(link, root)).toThrow("retired OA");
  });

  it("projects configured Pi model-service facts without commands, network, or mutation", async () => {
    const root = await makeRoot();
    ...
    const canonicalAgentDir = path.join(await realpath(root), "pi-agent");
  ...
  it("does not execute Extension resources for passive model-service projection", async () => {
    ...
    const result = await loadService({
      root,
      loadModule: async () => ({ ...sdk, createAgentSessionServices }) as ModelServicesSdk,
    });
  ...
  it("uses the selected Extension overlay instead of the colliding built-in provider", async () => {
    ...
    const loadModule = async () => ({ ...sdk, createAgentSessionServices }) as ModelServicesSdk;
  ...
  it("binds pending auth prompts to the originating client and supports cancellation", async () => {
    ...
    expect(JSON.parse(await readFile(path.join(root, "pi-agent", "auth.json"), "utf8"))).toEqual(
      {},
    );
  });
  ...
  it("expires a pending auth request honestly and releases the serialized mutation queue", async () => {
    ...
    await isolateProviderEnvironment(root);
    await mkdir(path.join(root, "pi-agent"), { recursive: true });
    const layer = makeTestLayer({ root, authRequestTimeoutMs: 1_000 });
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        const service = yield* HarosModelServices;
  ...
  it("rejects preserved credentials for non-custom service identities before loading or network", async () => {
    ...
    const layer = makeTestLayer({
      root,
      loadModule: async () =>
        ({ ...sdk, createAgentSessionServices }) as unknown as ModelServicesSdk,
    });
  ...
  it("saves, reopens, edits, and removes one custom service through Pi-owned state", async () => {
    ...
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const sdk = yield* Effect.promise(() => import("@earendil-works/pi-coding-agent"));
        const piRuntime = yield* Effect.promise(() =>
          createPiModelRuntime(path.join(root, "pi-agent"), sdk),
        );
        expect(piRuntime.getAvailableSnapshot()).toContainEqual(
          expect.objectContaining({ provider: serviceId, id: "model-one" }),
        );
