...
import { makePiHostSystemPrompt, promptRequiredHostGatewayToolNames } from "../piFamilyPrompt.ts";
...
describe("Pi native resource projection", () => {
  ...
  it("derives only the bounded canonical prompt-required Gateway closure", () => {
    expect(promptRequiredHostGatewayToolNames(undefined)).toEqual([]);
    expect(
      promptRequiredHostGatewayToolNames({ turnKind: "user", dispatchOrigin: "user" }),
    ).toEqual([]);
    expect(
    ...
    expect(
      promptRequiredHostGatewayToolNames({ turnKind: "user", dispatchOrigin: "automation" }),
...
describe("Pi native Haros gateway tools", () => {
  it("uses canonical MCP schemas and keeps same-cwd thread tokens distinct", async () => {
    ...
    const first = await buildPiHostGatewayCustomTools({
      connection: firstConnection,
      defineTool,
      fetch,
    });
    const second = await buildPiHostGatewayCustomTools({
  ...
  it("forwards Pi tool cancellation to the in-flight MCP request", async () => {
    ...
    const tools = await buildPiHostGatewayCustomTools({
  ...
  it("keeps stock Pi on the direct eager Gateway projection", async () => {
    ...
    try {
      const layer = makePiAdapterLive({ hostGatewayFetch: gateway.fetch }).pipe(
...
function makeGatewayCredentialsLayer(): Layer.Layer<HostGatewayCredentials> {
  ...
  return Layer.succeed(HostGatewayCredentials, credentials);
}
