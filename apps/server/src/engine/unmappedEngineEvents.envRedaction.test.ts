...
import { registerEngineCredentialKey } from "./engineChildEnvironment.ts";
import { sanitizeUnmappedProviderData } from "./unmappedEngineEvents.ts";

describe("unmapped engine environment credential redaction", () => {
  ...
  it("redacts delimited custom engine credential keys", () => {
    const assignment = JSON.stringify(
  ...
  it("redacts exact configured engine credential keys in every supported shape", () => {
