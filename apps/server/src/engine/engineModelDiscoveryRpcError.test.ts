...
import { toEngineModelDiscoveryRpcError } from "./engineModelDiscoveryRpcError";
...
describe("toEngineModelDiscoveryRpcError", () => {
  it.each([
    ...
    expect(
      toEngineModelDiscoveryRpcError({ cause, engine, statuses: [readyStatus] }),
    ).toMatchObject({
  ...
  it("keeps authentication failure deterministic", () => {
    const result = toEngineModelDiscoveryRpcError({
