...
export interface OpenCodeInventory {
  readonly engineList: ProviderListResponse;
  ...
  Effect.gen(function* () {
        ...
        yield* Scope.addFinalizer(
          ...
          Effect.tryPromise({
            try: () =>
              teardownEffectProcessTree(
                child,
                options?.teardownProcessTree ?? teardownEngineProcessTree,
              ),
            catch: (cause) =>
              new OpenCodeRuntimeError({
    ...
    const loadOpenCodeInventory: OpenCodeRuntimeShape["loadOpenCodeInventory"] = (client) =>
      Effect.all([loadProviders(client), loadOptionalAgents(client), loadConsoleState(client)], {
        ...
        Effect.map(([engineList, agents, consoleState]) => ({
          engineList,
          agents,
          consoleState,
        })),
...
export class OpenCodeRuntime extends ServiceMap.Service<OpenCodeRuntime, OpenCodeRuntimeShape>()(
  "harnessos/engine/opencodeRuntime",
) {}
