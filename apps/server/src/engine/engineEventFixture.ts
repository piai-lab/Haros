...
export function sanitizeEngineEventFixtureEvents(events: readonly unknown[]): unknown[] {
...
export function serializeEngineEventFixture(events: readonly unknown[]): string {
  if (events.length > DEFAULT_MAX_FIXTURE_EVENTS) {
    throw new EngineEventFixtureError(
      `Fixture exceeds ${String(DEFAULT_MAX_FIXTURE_EVENTS)} events.`,
    );
  }
  const serialized = sanitizeEngineEventFixtureEvents(events)
    .map((event, index) =>
      JSON.stringify({
...
export function parseEngineEventFixture(
  text: string,
...
export async function replayEngineEventFixture(
