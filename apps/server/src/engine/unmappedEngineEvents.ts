import type { EngineEvent } from "@harnessos/contracts";

import { isEngineCredentialKey } from "./engineChildEnvironment.ts";
...
export function sanitizeUnmappedProviderEvent(event: EngineEvent): EngineEvent {
  return {
    ...event,
...
export function makeUnmappedProviderEventGate(maxTrackedBursts = 128) {
  ...
  return (event: EngineEvent): boolean => {
