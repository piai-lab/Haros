...
import {
  assertEngineAdapterConformance,
  engineAdapterConformanceIssues,
} from "./engineAdapterConformance.ts";
...
describe("engine adapter conformance", () => {
  it("requires turn steering when the capability is advertised", () => {
    ...
    expect(engineAdapterConformanceIssues(adapter)).toEqual([
      {
  ...
  it("accepts adapters whose advertised discovery and steering methods exist", () => {
    ...
    expect(engineAdapterConformanceIssues(adapter)).toEqual([]);
    expect(() => assertEngineAdapterConformance(adapter)).not.toThrow();
  });
  ...
  it("reports all invalid capability declarations in one error", () => {
    ...
    expect(() => assertEngineAdapterConformance(adapter)).toThrow(
