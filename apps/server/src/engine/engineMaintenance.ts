...
export const HOMEBREW_ENGINE_UPDATE_TIMEOUT_MS = 60 * 60_000;
...
export interface PackageManagedEngineMaintenanceDefinition {
  readonly engine: EngineKind;
 ...
 * Homebrew-prefix install checked by Haros while nvm's node makes npm install
...
export function makeEngineMaintenanceCapabilities(input: {
  readonly engine: EngineKind;
  readonly packageName: string | null;
  readonly latestVersionSource?: EngineLatestVersionSource | null;
...
function makeManualOnlyEngineMaintenanceCapabilities(input: {
  readonly engine: EngineKind;
  readonly packageName: string | null;
}): EngineMaintenanceCapabilities {
...
function makeNpmGlobalEngineMaintenanceCapabilities(
  definition: PackageManagedEngineMaintenanceDefinition,
  pathPrepend?: string | null,
  commandPath?: string | null,
): EngineMaintenanceCapabilities {
  if (!definition.npmPackageName) {
    return makeManualOnlyEngineMaintenanceCapabilities({
...
function makeBunGlobalEngineMaintenanceCapabilities(
  definition: PackageManagedEngineMaintenanceDefinition,
  pathPrepend?: string | null,
): EngineMaintenanceCapabilities {
  if (!definition.npmPackageName) {
...
function makePnpmGlobalEngineMaintenanceCapabilities(
  definition: PackageManagedEngineMaintenanceDefinition,
  pathPrepend?: string | null,
): EngineMaintenanceCapabilities {
  if (!definition.npmPackageName) {
...
function makeHomebrewEngineMaintenanceCapabilities(
  definition: PackageManagedEngineMaintenanceDefinition,
  ...
  return makeEngineMaintenanceCapabilities({
    ...
    updateTimeoutMs: HOMEBREW_ENGINE_UPDATE_TIMEOUT_MS,
    ...(pathPrepend === undefined ? {} : { updatePathPrepend: pathPrepend }),
  });
}
...
function makeNativeEngineMaintenanceCapabilities(
  definition: PackageManagedEngineMaintenanceDefinition,
  installSource: EngineInstallSource,
...
function makeEngineMaintenanceForInstallSource(input: {
  readonly definition: PackageManagedEngineMaintenanceDefinition;
  ...
  if (
    ...
    return (
      makeNativeEngineMaintenanceCapabilities(definition, installSource, executable, pathPrepend) ??
      makeManualOnlyEngineMaintenanceCapabilities({
        engine: definition.engine,
        packageName: definition.npmPackageName,
      })
    );
  ...
  if (installSource === "bun") {
    return makeBunGlobalEngineMaintenanceCapabilities(definition, pathPrepend);
  }
  if (installSource === "pnpm") {
    return makePnpmGlobalEngineMaintenanceCapabilities(definition, pathPrepend);
  }
  if (installSource === "npm") {
    return makeNpmGlobalEngineMaintenanceCapabilities(definition, pathPrepend, commandPath);
  }
  if (installSource === "homebrew") {
    return makeHomebrewEngineMaintenanceCapabilities(
      definition,
...
export function resolvePackageManagedEngineMaintenance(
  definition: PackageManagedEngineMaintenanceDefinition,
  ...
  for (const commandPath of commandPaths) {
    ...
    if (installSource !== "unknown") {
      return makeEngineMaintenanceForInstallSource({
        definition,
        installSource,
...
export const resolveEngineMaintenanceCapabilitiesEffect = Effect.fn(
  ...
  if (hasPathSeparator(binaryPath)) {
    ...
    return resolvePackageManagedEngineMaintenance(definition, {
      ...options,
      binaryPath,
      realCommandPath,
    });
  ...
  // The production caller passes `buildEngineChildEnvironment(...)`, which always carries PATH.
...
}): Pick<ServerEngineVersionAdvisory, "status" | "message"> {
...
export function createEngineVersionAdvisory(input: {
  readonly engine: EngineKind;
  readonly currentVersion: string | null;
  readonly latestVersion?: string | null;
  ...
  const capabilities =
    ...
    makeManualOnlyEngineMaintenanceCapabilities({ engine: input.engine, packageName: null });
...
export const resolveLatestEngineVersion = Effect.fn("resolveLatestEngineVersion")(function* (
...
export const enrichEngineStatusWithVersionAdvisory = Effect.fn(
  "enrichEngineStatusWithVersionAdvisory",
)(function* (status: ServerEngineStatus, maintenanceCapabilities: EngineMaintenanceCapabilities) {
  if (!status.available || !status.version) {
    return {
      ...status,
      versionAdvisory: createEngineVersionAdvisory({
  ...
  const latestVersion = yield* resolveLatestEngineVersion(maintenanceCapabilities);
  return {
    ...status,
    versionAdvisory: createEngineVersionAdvisory({
      engine: status.engine,
