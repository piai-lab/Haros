...
function binding(
  ...
  engine: EngineRuntimeBinding["engine"] = "codex",
): EngineRuntimeBinding {
  return {
    threadId: THREAD_ID,
    engine,
...
function liveSession(input: {
  ...
  readonly engine?: EngineSession["engine"];
...
describe("planProviderRuntimeReconciliation", () => {
  ...
  it("uses the same stale-turn recovery for Claude sessions", () => {
    expect(
      planProviderRuntimeReconciliation({
        ...
        liveSessions: [liveSession({ engine: "claude", status: "ready" })],
        pumpHealth: [
          {
            engine: "claude",
            status: "healthy",
  ...
  it("trusts a terminal live error over stale active-turn metadata", () => {
    expect(
      planProviderRuntimeReconciliation({
        ...
        liveSessions: [
          liveSession({
            status: "error",
            activeTurnId: OLD_TURN_ID,
            lastError: "Engine process exited.",
          }),
        ],
        pumpHealth: [],
        nowMs: NOW,
      ...
      expect.objectContaining({
        ...
        projectedTurnId: OLD_TURN_ID,
        runtimeTurnId: null,
        errorMessage: "Engine process exited.",
      }),
    ]);
  });
  ...
  it("aligns a queued startup when the live engine has acquired a turn", () => {
    const plans = planProviderRuntimeReconciliation({
      threads: [
        threadShell({
          latestTurn: {
  ...
  it("does not let stale readiness complete a turn after a live engine error", () => {
    const plans = planProviderRuntimeReconciliation({
      threads: [
        threadShell({
          session: {
      ...
      liveSessions: [
        liveSession({
          status: "error",
          lastError: "Engine stream failed.",
        }),
      ],
      pumpHealth: [],
      nowMs: NOW,
  ...
  it("does not attribute a live engine error to a different projected turn", () => {
    const plans = planProviderRuntimeReconciliation({
      threads: [threadShell()],
      bindings: [binding(LIVE_TURN_ID)],
      liveSessions: [
        liveSession({
          ...
          lastError: "Newer engine turn failed.",
        }),
      ],
      pumpHealth: [],
      nowMs: NOW,
      ...
      bindings: [
        {
          ...
          runtimePayload: {
            activeTurnId: null,
            lastError: "Previous engine failure.",
          },
        },
      ],
      liveSessions: [liveSession({ status: "ready" })],
   ...
   * `thread.activity.append` from it (EngineRuntimeReconciler.applyPlan). Those
  ...
  const commandsFromPlan = (
    plan: EngineRuntimeReconciliationPlan,
  ...
  const expectSchemaValidPlans = (
    plans: ReadonlyArray<EngineRuntimeReconciliationPlan>,
    thread: OrchestrationThreadShell,
  ): void => {
  ...
  it("does not settle while the engine's runtime-event pump is unhealthy", () => {
