...
const healthyOrchestrationEngine = {
  getProjectionCatchUpStatus: Effect.succeed({
    ...
    inFlight: false,
    retryAttempts: 0,
    lastFailure: "sensitive projected payload detail",
    highWaterSequence: 0,
    lagByProjector: {},
    missingProjectors: [],
  }),
...
describe("production Effect HTTP routes", () => {
  ...
  it("serves readiness through the deployed health route", async () => {
    await withEffectServer(makeConfig(), { kind: "health", readiness }, async (origin) => {
      ...
      const body = (await response.json()) as { readonly projection: Record<string, unknown> };
      expect(body).toMatchObject({
        status: "ok",
        startupReady: false,
        pushBusReady: true,
        projection: { state: "healthy", hasFailure: true },
      });
      expect(body.projection).not.toHaveProperty("lastFailure");
      expect(JSON.stringify(body)).not.toContain("sensitive projected payload detail");
    });
  });
