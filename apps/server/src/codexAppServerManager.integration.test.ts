...
describe("handleStdoutLine", () => {
  it("keeps clean JSON intact and removes ANSI before protocol parsing", () => {
    const json = '{"method":"item/started","params":{}}';
    expect(normalizeCodexProcessLine(json)).toBe(json);
    expect(normalizeCodexProcessLine(`\u001b[32m${json}\u001b[0m`)).toBe(json);
  });
  ...
  it("ignores multiline and standalone JSON leaked from command output", () => {
    ...
    for (const line of ["{", "[", '{"scripts": {', "{}", "[]", '{"name":"omnimind"}']) {
      handleStdoutLine(context, line);
    }
...
describe("CodexAppServerManager discovery", () => {
  it("restarts the idle grace period after a discovery request settles", async () => {
    vi.useFakeTimers();
    try {
      const manager = new CodexAppServerManager(undefined, { discoverySessionIdleMs: 15_000 });
      const context = {
        discovery: true,
        session: { cwd: "/repo" },
        pending: new Map(),
      ...
      (manager as unknown as { discoverySessions: Map<string, unknown> }).discoverySessions.set(
        "/repo",
        context,
      );
      vi.spyOn(
      ...
      const stopDiscoverySession = vi
        .spyOn(
          manager as unknown as { stopDiscoverySession: (cwd: string) => Promise<void> },
          "stopDiscoverySession",
        )
        .mockResolvedValue(undefined);

      (
        manager as unknown as { scheduleDiscoverySessionIdleStop: (cwd: string) => void }
  ...
  it("reads account limits through the initialized app-server context", async () => {
    const manager = new CodexAppServerManager();
    const context = {
      session: {
        provider: "codex",
    ...
    const response = { rateLimits: { planType: "plus" } };
    const sendRequest = vi
      .spyOn(
        manager as unknown as {
    ...
    await expect(manager.readAccountRateLimits()).resolves.toEqual(response);
    expect(sendRequest).toHaveBeenCalledWith(context, "account/rateLimits/read", undefined);
  });
