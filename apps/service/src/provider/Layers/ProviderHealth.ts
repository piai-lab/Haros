...
export const checkAntigravityProviderStatus = (
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      version: parsedVersion,
      checkedAt,
      message: "Antigravity CLI is installed, but OmniMind could not verify login by listing models.",
    } satisfies ServerProviderStatus;
  });
      ...
      const updateProvider: ProviderHealthShape["updateProvider"] = Effect.fn(
        ...
        if (!isProviderEnabledForSettings(provider, settings)) {
          return yield* new ServerProviderUpdateError({
            provider,
            reason: "Provider is disabled in OmniMind settings.",
          });
        }
          ...
          const finalProviders = yield* setProviderUpdateState(
            ...
            makeUpdateState({
              ...
              message: stillOutdated
                ? `Update command completed, but OmniMind still detects an outdated provider version${stillOutdatedVersions}.`
                : "Provider updated.",
              output: output ? output.slice(0, UPDATE_OUTPUT_MAX_BYTES) : null,
            }),
          );
