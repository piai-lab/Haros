...
import { DEFAULT_SERVER_SETTINGS, ServerProviderUpdateError } from "@omnimind/contracts";
  ...
  describe("disabled provider handling", () => {
    it("builds an inert status for disabled providers", () => {
      assert.deepStrictEqual(makeDisabledProviderStatus("kilo", "2026-06-16T12:00:00.000Z"), {
        ...
        authStatus: "unknown",
        checkedAt: "2026-06-16T12:00:00.000Z",
        message: "Provider is disabled in OmniMind settings.",
      });
    });
    ...
    it.effect("projects cached ready status when a disabled provider is re-enabled", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.strictEqual(disabledCodex?.message, "Provider is disabled in OmniMind settings.");
          ...
          assert.notStrictEqual(currentCodex?.message, "Provider is disabled in OmniMind settings.");
    ...
    it.effect("rejects one-click updates for disabled providers", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(error.reason, "Provider is disabled in OmniMind settings.");
      }).pipe(Effect.provide(disabledProviderHealthLayer)),
    );
  });
