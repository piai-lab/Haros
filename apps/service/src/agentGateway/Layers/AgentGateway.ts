...
const AGENT_GATEWAY_INSTRUCTIONS =
  "OmniMind tools are thread-scoped. Use browser_* only for OmniMind's visible WebView; follow the provider-delivered <omnimind_host_context> for full policy.";
...
export const makeAgentGateway = Effect.gen(function* () {
  ...
  const createThread: ToolEntry = {
    ...
    definition: {
      ...
      description:
        "Create exactly one standalone OmniMind thread. Worktree threads start at a detached HEAD. For two or more threads use one omnimind_create_threads call instead.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: {
        title: "Create a OmniMind thread",
        readOnlyHint: false,
        destructiveHint: true,
        idempotentHint: true,
        openWorldHint: true,
