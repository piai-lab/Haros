...
Prefer OmniMind's built-in visible browser for browser work. In code mode, call its MCP methods directly inside \`functions.exec\` with the exact \`tools.mcp__omnimind__browser_*\` prefix (for example, \`await tools.mcp__omnimind__browser_open({ url })\` and \`await tools.mcp__omnimind__browser_snapshot({})\`). The available suffixes are ${BROWSER_TOOL_NAMES.map((name) => `\`${name.slice("browser_".length)}\``).join(", ")}.
