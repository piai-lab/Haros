...
import servicePackageJson from "../package.json" with { type: "json" };
...
const applyPublishIconOverrides = Effect.fn("applyPublishIconOverrides")(function* (
  ...
  yield* Effect.log("[service] Applied publish icon overrides inside the isolated package stage");
});
...
const applyDevelopmentIconOverrides = Effect.fn("applyDevelopmentIconOverrides")(function* (
  ...
  for (const override of DEVELOPMENT_ICON_OVERRIDES) {
    const sourcePath = path.join(repoRoot, override.sourceRelativePath);
    const targetPath = path.join(serviceDir, override.targetRelativePath);
  ...
  yield* Effect.log("[service] Applied development icon overrides to dist/client");
});
...
const buildCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      yield* Effect.log("[service] Running tsdown...");
      yield* runCommand(
        ChildProcess.make({
          cwd: serviceDir,
      ...
      const clientTarget = path.join(serviceDir, "dist/client");

      if (yield* fs.exists(webDist)) {
        yield* fs.copy(webDist, clientTarget);
        yield* applyDevelopmentIconOverrides(repoRoot, serviceDir);
        yield* Effect.log("[service] Bundled web app into dist/client");
      } else {
        yield* Effect.logWarning("[service] Web dist not found — skipping client bundle.");
      }
    }),
).pipe(Command.withDescription("Build the service package (tsdown + bundle web client)."));
...
const publishCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const version = Option.getOrElse(config.appVersion, () => servicePackageJson.version);
      const pkg = {
        name: servicePackageJson.name,
        license: servicePackageJson.license,
        bin: servicePackageJson.bin,
        type: servicePackageJson.type,
        version,
        engines: servicePackageJson.engines,
        files: servicePackageJson.files,
        dependencies: resolveCatalogDependencies(
          servicePackageJson.dependencies as Record<string, unknown>,
          resolveRootWorkspaceCatalog(),
          "apps/service dependencies",
        ),
      };
      ...
      yield* fs.copy(path.join(serviceDir, "dist"), path.join(stagedPackageDir, "dist"));
      ...
      yield* Effect.log(`[service] Running from isolated stage: npm ${args.join(" ")}`);
      yield* runCommand(
...
).pipe(Command.withDescription("Publish the service package to npm."));
