...
- snapshot字段结构只由`@harnessos/contracts`拥有，fork的presenter type直接引用该合同；Server在bundled profile缺snapshot时fail closed，防止pre-effect启动落回固定黑白；
...
exact pushed implementation SHA `9c4740da87a894913f30f4b3648340a8d2ef3cdc`取得了第一阶段代表性运行证据：独立clean clone构建的DMG SHA-256为`8da30689b1393df6ec462ae19b3656739b17851b8a18d7a039b316fbf08d1451`，DMG内与安装版`app.asar` SHA-256均为`c30750bd7727480865ca13aa2fee583def5e6aef09d1b444779b55cbfb05d220`。隔离profile中的明显暖色light/dark palette贯穿真实Shell、Composer、Appearance Settings、Browser chrome/local state、annotation overlay和Terminal；第三方网页保留青/珊瑚/黄原色，Terminal在换肤与renderer reload前后保持同一PTY，简中/英文、1100×760窄窗、键盘focus与App关闭重开成立。正式Curator Server和实际页面在390×740、reduced-motion下证明中文observer与英文review消费完整暖色snapshot、结果键盘折叠、无水平溢出且Provider资产不被filter。Diff/code由同一SHA的warm browser tests与production build保护；安装App只到达暖色Diff框架，没有真实彩色hunk，因此此项保持为source/browser证据，不冒充packaged hunk。该阶段证明A/B/C/D分类、唯一owner与第二palette/schema收口方向成立；它已被下方最终task/main证据继续扩展，不能单独代表当前最新候选。

后续同一owner cut由implementation SHA `7424d07d900c8126e5227760c454a99d4ec3f3b9`、authority/status SHA `61b4836c3ef6649ea5b8023258b92ef3577a5828`与tested task SHA `e30963c1291e8becbe5d799881ff3c032dbb4a3d`闭合：主题preset的字体、对比度与窗口材质应用语义已从唯一catalog descriptor投影，43个bundled variant的正常正文、链接与小字号状态文字对`surface/surfaceUnder/panel`均达到4.5:1，同时保留raw accent、Diff身份色、第三方内容和品牌资产原色。focused门为theme 30/30、Browser主题表面5/5、Desktop exact-reopen 27/27、正式Curator presentation 6/6；Web 4193/4193、Contracts 264/264、Desktop 606通过/5跳过、Shared 575通过/1跳过、Scripts 81/81、root typecheck 7/7、lint 0 error（601个既有warning）与production build 5/5通过。root并发门与作者package独立门各有一次560/561，唯一失败是untouched `pi-web-access` GitHub SIGTERM-resistant process-tree用例未及时生成临时`processes.json`；同一支持Node 24环境的精确用例单独复跑1/1通过，未修改作者测试，也未把偶发竞态写成主题回归。

从exact pushed task SHA `e30963c1291e8becbe5d799881ff3c032dbb4a3d`的clean clone构建并安装arm64 DMG，SHA-256为`2a75671a07784319e43b802ecf6a0d84c6a301eca12a678aab141d9c3adb1b6c`，DMG内与安装版`app.asar`均为`51dcabcb6c317659d9db7198dd85c9bb236bf77a1cbc61c46a61002881f93941`。fresh任务profile隔离了HOME、CFFIXED_USER_HOME、OMNIMIND_HOME、PI_CODING_AGENT_DIR、XDG、TMPDIR与Electron userData，Main、Renderer、Helper和bundled Server均绑定该profile；启动、操作与重开前后的lsof未发现真实`.pi`、`.omnimind`或真实OmniMind Application Support。安装版Solarized light的Browser surface为`#fdf6e3`，raw accent保持`#b58900`，正文/链接派生为`#60747c`/`#8f6c00`并实测4.542:1/4.514:1；Solarized dark raw accent保持`#d30102`，正文/链接为`#839496`/`#e56969`并实测4.921:1/4.858:1。关闭重开后preset与派生角色保持，Browser无假恢复。

任务tip `aae6699b2cdeb836427faea12a70eb5cb10b5ed4`通过no-ff merge `8f59ccad2089ecaaa2b501358a2dd328ef494ee5`进入并推送`origin/main`。从该精确pushed main merge SHA的clean clone构建并安装arm64 DMG，SHA-256为`ef33d1011071a3761e54585ba726a20c253051aef481912316438926f4edb990`，DMG内与安装版`app.asar`均为`064e543925db77bb04ca6edcd959f8797ccdcd017e6ed53aaadc900c22646d8b`；fresh main任务profile再次复证四进程隔离、Solarized light的Browser surface/raw accent/可读角色及关闭重开持久化，且未打开真实profile文件。记录该post-merge证据的尾随commit `98f66b4ec59d9bc04002977ee1aa627e69e206b5`不属于上述已测试Desktop bytes。当前状态准确为`global-theme-system-candidate / packaged-not-released`，不是签名、公证、Release、update feed或整个产品已发行。
