...
- [pi-native-host-tool-loading-review.md](pi-native-host-tool-loading-review.md)：绑定 OmniMind `8066f23…` 与 bundled Pi `0.84.2`/upstream `914cf147…` 的 AgentGateway→Pi-family 工具投影复核；区分 Host capability、非 Pi native MCP、Pi inline Extension、Package、Skill 与 Tool Search 的唯一 owner，给出 named hidden Extension、owned additive activation、prompt/cache、collision/provenance、10/100/1000 规模、未来变化落点与扩展成本、契约测试、删除路径、冰山验证、回滚与 stop-loss 的 Gate A 结论及详细 Gate B 参考。维护者已于 2026-08-18 接受其中的稳定 owner/Engine-native 投影原则并由 `architecture/README.md`、`architecture/execution.md` 唯一拥有；inline dynamic 实现仍未获 Gate B 授权。本文不取代当前 brief 或 Mission。
