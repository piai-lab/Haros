...
- [omnimind-agent-capability-surface.md](omnimind-agent-capability-surface.md)：基于当前 OmniMind 真实侧栏、header、Composer stack、Timeline、288px Workbench Environment card 和 right dock 的 Agent 能力投影规格；逐项定义目标、Todo、子智能体、Engine-native workflow、自动记忆/知识、恢复、Computer Use、Skills/Plugins 何时无 UI、何时只显示实质结果、何时才允许人介入；区分能力/集合 icon、每个 child 的稳定 identity glyph 与状态语义，并规定 Todo 单行摘要、Environment 目录、right-dock workflow 图的渐进披露。包含 React Flow/X6/Cytoscape/Mermaid 的网络 renderer intake、exact-version challenger proof 与同一真实外壳下的 deterministic HTML storyboard；不取代 `architecture/workbench.md`，也不拥有实施顺序。
