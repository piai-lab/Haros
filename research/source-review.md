...
### 14.3 Non-adopted and no-code map — 51/51 disposed
...
| `c8e15cb0784a2a91415d2128bf70265f8b920495` | Deferred: a generic raw “unmapped provider event” surface would bypass typed Product projection and localized presentation; current fail-closed typed mapping is stronger.            | A reproducible shipped Provider event is materially lost and can be expressed through one stable typed projection without raw diagnostic leakage. |
| `78a311969a0d05594b6f1cdd9fc08876d936e768` | Current-stronger/declined exact patch: OmniMind already owns runtime icon preference/hydration; donor dark assets and automatic Dock identity are not adopted.                        | An authorized OmniMind dark icon asset and a packaged macOS appearance-switch journey expose a current owner gap.                                 |
...
| `40cb5e1a3e3ef7af460f71d2256cd7f94d65983f` | Deferred: the broad debug mode spans AgentGateway, Provider commands, drafts and user UI, creating a new cross-owner control surface while C1 is not admitted.                        | An admitted diagnostic requirement has one privacy/receipt authority and cannot be met by existing diagnostics.                                   |
...
| `c7131c650145cd88c99e29633a4beb45c78fec34` | Current-stronger: the picker follows exact runtime catalog/readiness and must still expose truthful setup routes; filtering by “installed” alone would hide recoverable services.     | A real picker journey exposes a false selectable Provider or a missing availability gate in the current owner.                                    |
| `f3eff079db7f52895cf4647fa7dc8ebb83adcb4c` | Declined exact UI: width presets duplicate the responsive Workbench geometry/preference owner and add persistent settings for an unproven need.                                       | A reproducible `480/960/1440` reading-width defect survives the current responsive owner and requires a user preference.                          |
...
| `33ac3fd30b8f16df746d9189fd2c8352af2597ca` | Deferred Goal control-plane persistence; C1/C3 Goal truth is not admitted and this would add competing durable authority.                                                             | C1 then C3 are admitted with one Goal/Task owner and complete recovery/economics proof.                                                           |
| `82487f12959b228853e0b5522c7b4eccc3de0b99` | Deferred Goal command/indicator for the same missing durable authority.                                                                                                               | Same C1/C3 admission trigger.                                                                                                                     |
| `dd5f92fd6eac2bf242e6671a17c05533a2417900` | Deferred Goal MCP projection; exporting an unadmitted Goal fact would widen public authority.                                                                                         | Same C1/C3 admission plus explicit MCP exposure contract.                                                                                         |
...
| `d265e95888a6163acd6d06f79454e75105ba5d69` | Deferred Goal panel/pause/timer and migrations; it is a second lifecycle before Agent Core truth.                                                                                     | Same C1/C3 admission trigger with one projection and timer owner.                                                                                 |
...
| `838ea204066c7687a26e9da9156555e44cb1017c` | Deferred Goal achievements/footer badge; persistent chrome and a new outcome fact are unadmitted.                                                                                     | C3 admits Goal outcome truth and the existing low-noise footer wins a focused UI falsifier.                                                       |
| `21a64cf70c13d93cdf6c0201eba8d5d920d08843` | Deferred Goal timing snapshot fix because the underlying Goal schema is not adopted.                                                                                                  | Same C1/C3 admission trigger.                                                                                                                     |
| `b069b40b004acf573459342ba5c1b3b8a695b1f3` | No-code relative to current product: test-only follow-up for the unadopted Goal schema.                                                                                               | The Goal schema is admitted.                                                                                                                      |
...
| `28a3ed5b0d1a3539e8ab014052b909beb373e6db` | Deferred Goal icon; no Goal command is admitted and extra iconography would be false affordance.                                                                                      | Same C1/C3 admission trigger.                                                                                                                     |
| `f34c3f51b9e2ebb4425e77743cbb9b975ffbf33a` | Deferred staged Goals/slash chips; adds Composer syntax and Timeline semantics before one Goal owner exists.                                                                          | Same C1/C3 admission trigger plus keyboard/IME/round-trip proof.                                                                                  |
...
| `84c3436a6eed6cfbc20e25f9916919279ae7be5f` | Deferred automatic Goal continuation; it would create ambient work and a second retry authority.                                                                                      | C3 admits continuation with bounded receipts, stop semantics and economics truth.                                                                 |
| `8d52c68885030b1f448c9c32513c93b4fb9b4247` | Deferred formatting follow-up for unadopted Goal continuation.                                                                                                                        | Automatic Goal continuation is admitted.                                                                                                          |
| `c1649c9ccb8948d537def5d8e47819bf4e14bfdc` | No-code relative to current product: type-only follow-up for the unadopted Goal UI.                                                                                                   | The corresponding Goal UI is admitted.                                                                                                            |
| `e187172845ce14cc1f04b8609254430b99b099be` | Deferred payload correction for unadopted automatic Goal continuation.                                                                                                                | Automatic Goal continuation is admitted.                                                                                                          |
| `0f473550b98d4ce4d4fbec659cf8306a0cac83bf` | Deferred review fixes for the same unadopted Goal lifecycle.                                                                                                                          | Same C3 admission trigger.                                                                                                                        |
| `0dfb3c03c24058ea0a4f0f632c72f1b46ed0b4a0` | Deferred passive Goal edits; still depends on the unadopted Goal fact.                                                                                                                | Same C1/C3 admission trigger.                                                                                                                     |
| `f7164d3037e0af21e650cd4398d7132f0064041a` | Deferred Goal continuation concurrency owner.                                                                                                                                         | Same C3 admission trigger with race/reopen proof.                                                                                                 |
| `b10bd4a9698d85a3c48363ff3bd8057269e846ac` | No-code relative to current product: test typing for the unadopted Goal lifecycle.                                                                                                    | Goal continuation is admitted.                                                                                                                    |
| `b6890e5be8f9fa9d7f20469458c3c100d8a32ced` | No-code: formatting-only Goal test follow-up.                                                                                                                                         | Goal continuation is admitted.                                                                                                                    |
| `3c2a138a5abb97d6f32b37292ad0eb8b2459d00a` | Deferred Goal retry from Terminal sessions; would couple Terminal settlement to an unadmitted ambient loop.                                                                           | C3 admits retries with exact receipt, stop and outcome semantics.                                                                                 |
...
| `ed6006456a2ff8ab10445ae84947548efa3035a1` | Declined exact icon swap: current sidebar icons/low-noise controls are the Workbench owner; no usability regression justifies donor-wide glyph replacement.                           | Focused user/AX evidence shows current panel controls are ambiguous.                                                                              |
