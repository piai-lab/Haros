...
- Git history continuously includes the original T3Code lineage and later contributors.
- A temporary downstream copyright change was later reverted before the fixed revision.
- Icons, screenshots, product names and other branded assets remain baseline evidence only. They require replacement or item-level rights and product-need review before a production candidate.
...
The Pi adapter alone is 2,944 lines. `plans/006-make-synara-the-agent-harness.md` explicitly records an intention to make the source product an authoritative Agent Harness. That design ambition is useful evidence, but it cannot become OmniMind authority merely because the code is mature.
...
At the fixed revision, Pi packages resolve to `0.81.1` through caret declarations. The separately inspected local Pi research tree is newer (`0.83.0`). This is evidence of version-tracking pressure and supports exact pins plus a conformance matrix; it is not itself evidence that either version is production compatible.
