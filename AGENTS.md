...
- 安全、隐私和正确性是服务真实用户结果的约束，不是独立追求最大化的产品目标。OmniMind 默认是本机个人产品；没有远程、多租户或不可信操作者的真实入口与证据时，不得套用对应威胁模型制造确认、认证、隔离、状态或验证成本。
- 新增保护前必须说清具体资产、可信的出错或攻击路径、可能损失、可恢复性、现有保护，以及新增方案给用户和维护者造成的摩擦。只有“最佳实践”“更安全”或想象中的未来风险，不构成施工依据。
- 用户已经明确发起且可恢复的本地动作默认直接完成，以撤销、回收站、版本历史、重试和清楚状态代替二次确认；显式按钮本身可以表达意图，不能机械地再弹一次确认。只读、搜索、浏览和 `Full Access` 范围内已启用、当前可用、属于任务意图的普通操作不得重复审批。
- 有明确边界的本地敏感能力优先按 exact scope 或稳定 fingerprint 授权一次，只在对象、范围或内容实质变化后重新确认。不可恢复删除、用户未表达的外部发送或发布、身份与付款、OAuth/2FA、原生系统权限及其他真实不可逆副作用，才默认要求额外确认或人在场。
- 文件权限、secret 脱敏、原子写入、幂等、一次性 claim、路径 containment、冲突检测和防重复执行等低摩擦保护，只要防的是可信故障且不打断正常 journey，应保持在 owner 内，不得转化成用户审批仪式。后台和普通投影不得泄露 secret；用户主动查看自己已保存凭据是否允许回显是独立产品决定，不能用“secret 永不回显”替代裁决，也不能借回显把凭据写入日志、事件、缓存或诊断。
- 不把 `fail closed`、`no fallback` 或“精确绑定”当口号。防止重复外部副作用、错误目录或静默更换 Provider、模型、Thinking、权限时可以拒绝自动继续，但必须提供真实、可理解、尽量一步可达的重试或用户选择替代项。局部能力的保护条件失败时默认只降级该能力；除非共享不变量已被破坏，不得让整个 App 失败。
- 任何安全方案若新增持久状态、owner、进程、公共合同、后台生命周期、重复确认或显著验证矩阵，都是需要证据和维护者授权的产品分叉。优先删除风险来源、缩小 authority、复用现有 owner 和使用可恢复设计，不得用第二 store、permission broker、ledger 或安全平台补偿边界不清。
- 验证强度按真实损失和 claim 选择：本地可恢复行为做 focused proof；凭据、不可逆外部结果、迁移、公开发行和供应链才提高门槛。不得把正式公开发行的完整门禁例行施加到不进入 shipped bytes 的普通开发，也不得以安全名义建立复制生产语义的永久测试系统。
- 本节只约束风险判断和 Agent 行为，不拥有具体产品事实。已有 architecture sole owner 或维护者明确裁决决定某项功能如何工作时，更新并遵循该 owner；不得在这里扩写第二份权限矩阵、确认清单或恢复协议。
