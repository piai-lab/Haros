...
- 能由现有 test、Browser 或 packaged Electron harness 程序化证明的路径不得再调用 Computer Use；它只用于无法合理程序化或人工验证的 OS 原生表面。确需人工视觉复核时，只在 ignored 的 `apps/web/test-results/manual-verification/<sha>/` 生成本次候选清单，并引用 canonical 测试与产品 owner；不得把清单扩张成第二 case registry。纯测试、CI、文档或其他不改变 shipped bytes 的候选不重复打包，也不伪造视觉 case。
