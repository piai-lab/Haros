...
OmniMind Agent 当前技术 lineage/生态兼容基准是 Pi stable `v0.84.2`，之后使用自己的 runtime version；它不以 Pi version 作为长期产品身份。它使用独立构建或等价的 instance-level 配置，使全局与 project-local private state 都进入 `.omnimind`。stock Pi 仅在被用户显式选择时使用自己的 `.pi` native state；产品 reset、OmniMind Agent 和后台 discovery 都不得读取、迁移、同步或改写它。stock Pi 的实际 session runtime version 与可选本机 CLI version 必须分别呈现，不能互相冒充。production-adopted Synara head 是 exact `8f9f60045ea652db7d4a6822e2f723dde073f40a`；当前安装的 OmniMind product bytes 来自 exact pushed `3077bf253`，其祖先 `58f76446d3d7061b7fe18e140fe56676269497c3` 完成母体语义吸收、作者等价测试与完整 macOS fresh-profile journey，`3077bf253` 进一步闭合 Engine picker 身份与 readiness 反馈并完成独立 packaged/reopen 复验。持久 ThreadGoal 与逐回合 Todo 是两条独立责任，且都位于同一个 inherited Thread/Product Orchestration authority 内。
...
当前本机安装与 macOS arm64 ad-hoc candidate 均来自 exact pushed product `3077bf253`，DMG SHA-256 为 `fae7e7a6b42907f36aa0f1e235eddef7f14d3d0ddbdb1a9cecf47f3d4d7950ea`。fresh task profile 已真实验证 Engine picker 的十个 Engine 图标、中英文 readiness、可用数量汇总、未安装 Engine 的设置可达性以及关闭重开；进程参数同时证明 main、Helper 与 bundled Server 均使用任务专用 HOME、XDG、OmniMind home 和 userData。其祖先 `58f76446d` 的独立 fresh task profile 已真实验证 DeepSeek 首轮、`/debug`、Goal 设置与自动 continuation、手动暂停/恢复、运行中中断 auto-pause、可恢复 Provider 失败 auto-pause、关闭重开、Todo 独立投影、宽屏 `72rem`、深色图标、Profile PNG copy 与本地 Thread Group；migrations 1–100 成功。更早精确候选验证了同一未再改动的 `/default` 与 Profile PNG native-save 链，作者等价测试保护 Automation `1`/`3`/`5`/持续运行策略与导出失败反馈。该产物是本地 ad-hoc candidate，不冒充 official signed/notarized release，也没有创建 GitHub Release 或修改 update feed；后续纯文档 SHA 不冒充 installed product bytes。
