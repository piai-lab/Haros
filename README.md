...
当前 `/Applications/OmniMind.app` 来自 exact pushed product commit `db25a5b91343a4ddbf70fedd98ea3583bd020317`；DMG SHA-256 为 `9a62e2384bb5a917a460679928e67fea2ce0733de61eb67fba06d59d1e28aaef`，ZIP 为 `43d446891944749d2647a2bec5bc5fabd5d8c02c6932a6bf067f899d90d5966b`，安装副本 app.asar 为 `95937e7f95237917556151afb6324392704d9f44af3d7cd9e506643dd0389aea`；asar 内嵌 commit 与该 pushed SHA 一致，240 项 staged legal identities闭合。任务专用 HOME、OmniMind home、显式 Electron userData、Provider private home 与 workspace 下的 fresh-profile journey 证明：MiMo 全新 Chat 首轮读取 preconfigured V1 + custom rules，保存V2但不reload仍保持V1，显式reload后读取V2；DeepSeek全新Agent首轮读取V2 + custom rules；页面查看/取消不改变settings或rules的hash/mtime；同profile关闭重开保留当前值；恢复factory并删除rules后回到空文件状态。一次 `--version` 探测被Electron当作默认启动，未进入UI、未纳入journey；实例立即停止，真实 `~/.omnimind/agent` 的7个Prompt leaf仍全部absent。该产物是本地未签名、未公证 candidate，不冒充 official release，也没有创建GitHub Release或修改update feed；F-22保持`candidate`，后续纯文档SHA不冒充installed product bytes。
