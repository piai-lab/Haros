...
```source-adoptions
{
  "adopted": [
    ...
    },
    {
      ...
      "status": "candidate-pending-packaged",
      "paths": ["packages/om-web-access", "packages/contracts/src/omnimindWebSearch.ts", "apps/server/src/provider/Layers/OmniMindWebSearchSettings.ts", "apps/server/src/provider/Services/OmniMindWebSearchSettings.ts", "apps/web/src/components/settings/WebSearchSettingsPanel.tsx"],
      "sourcePaths": ["."],
      ...
      "rights": "The fixed pi-web-access source is MIT-licensed under the retained exact legal text by Nico Bailon. OmniMind keeps the upstream LICENSE and author tests. Research-only Provider logo candidates and marks without production admission are not adopted or shipped.",
      "mode": "fork-narrowly",
      "changes": "The complete mature upstream source remains one private workspace package for the bundled OmniMind Agent. OmniMind adds only the six bounded seams for per-Session and per-call lifecycle isolation, one package-owned canonical config service, the OmniMind runtime profile, typed bilingual Curator presentation, route evidence to the Pi active set, and honest Unicode source_check evidence semantics. It is not a Host capability, AgentGateway backend, npm release or stock Pi compatibility profile; all unadmitted Provider identities use the neutral local provider mark.",
...
当前 production-adopted baseline 是 exact reviewed Synara head `57f48ef…` 的单一物理 substrate：`apps/desktop + apps/web + apps/server + packages/contracts + packages/shared`。产品关系是 Synara upstream platform → OmniMind downstream distribution，不是 selective donor 拼装；上方 adoption record 通常描述已完成 evidence invariant 的 revision 与固定 divergence，唯一显式标为`candidate-pending-packaged`的`bundled-omnimind-web-access`只记录已冻结的source candidate，必须在pushed-SHA packaged evidence闭合后才能提升为production-adopted。`apps/service`、`apps/native-host` 与旧 Product Control Plane 不属于生产拓扑；维护者已确认不存在前代用户，Synara/DP Code legacy profile/storage migration 因 first-public identity safety 永久排除，不读取或改写这些无关身份路径。未进入清单的Pi与其他对照项目仍只是研究来源；exact revision/tree、rights/lineage/assets、构建/测试/运行观察和兼容限制只以对应research owner为准，法定文本保存在`LICENSES/`。
...
本 README 只拥有稳定产品身份、战略不变量与 production adoption，不记录瞬时 product bytes、施工状态、claim 状态或历史 journey。

- 进行中的工作目标、已装 product bytes、真实阻塞与下一动作只见 [`execution-brief.md`](execution-brief.md)；
- Campaign claim 状态与 evidence pointer 只见 [`missions/independent-omnimind-v1.md`](missions/independent-omnimind-v1.md)；
- fixed source、历史 artifact、journey、失败与反例只见 [`research/`](research/README.md)，其中 Synara exact source 与 disposition 由 [`research/source-review.md`](research/source-review.md) 拥有。
