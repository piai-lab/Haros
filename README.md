...
当前 `/Applications/OmniMind.app` 来自 exact pushed product commit `1aac8c1e2a690f17ed0356eab90a01a39c3b65f6`；macOS arm64 DMG SHA-256 为 `f377f4439597a1d281c723c33bdd80b4f04846fe72459abb0bb5e43c64752fab`，DMG 内与安装副本 app.asar 均为 `76700eb30ede3b7081713640b9c01ac0fe527fba6d3ce947939c9b7cc86ffeac`，240 项 staged legal identities闭合。本轮未生成与该 SHA 对应的 ZIP，目录中的旧 ZIP 不属于当前证据。任务专用 HOME、OMNIMIND_HOME、XDG、显式 Electron userData 与 Provider private home 下的 fresh-profile journey 证明：页面只有默认提示词与自定义规则两张卡，不再提供当前对话资源或 reload 控件；factory customize/no-op/restore、custom rules create/Desktop Open/cancel mtime稳定、同profile关闭重开与remove回空态通过。祖先 product `db25a5b91343a4ddbf70fedd98ea3583bd020317` 的 deterministic request capture、MiMo/DeepSeek、Session snapshot、底层 native reload 与 cache-stable digest 证据仍覆盖本轮未改动的 runtime 链。该产物是本地未签名、未公证 candidate，不冒充 official release，也没有创建 GitHub Release 或修改 update feed；F-22 保持 `candidate`，后续纯文档 SHA 不冒充 installed product bytes。
