...
当前 `/Applications/OmniMind.app` 来自 exact pushed product commit `d120b394d50248472ed294068cd1791198e943e6`；macOS arm64 DMG SHA-256 为 `dea7b9bbf752b39e904e3ed4ecface1c4ac4a1d7c155d002a2fa0cb6787de69e`，安装副本 app.asar SHA-256 为 `2f8612c3b0a637f031e4bf2edad6c8e5914dfbab21f455ce421bd6db24bbddbc`，内嵌 commit 与 source 一致，240 项 staged legal identities 闭合。任务专用 HOME、OMNIMIND_HOME、XDG、显式 Electron userData 与 Provider private home 证明 Main、Helper、Renderer 和 bundled Server 全部隔离；凭据只经进程环境注入，隔离 `auth.json` 保持空对象。安装版从 authoritative runtime catalog 显示 DeepSeek 2 个、Xiaomi 6 个模型，精确选择 `DeepSeek V4 Pro` 完成首轮与 continuation，关闭重开后同一 Chat、两轮结果和模型选择恢复。人工植入旧 OmniMind `customModels` 的第二隔离 profile 证明启动读取不会改盘且旧条目、转换入口和发送候选均不可见；一次无关 streaming 设置保存只让 revision `4→5` 并自然丢弃该字段，其他哨兵设置逐项保持，重开结果稳定。全部实例正常退出后进程树归零；旧安装副本和任务 profile 已移入废纸篓，可恢复。该产物是未签名、未公证的本地 ad-hoc candidate，没有创建 Release、生成 current-SHA ZIP 或修改 update feed；后续 docs-only SHA 不冒充 installed product bytes。
