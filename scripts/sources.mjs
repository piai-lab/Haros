...
export function validateSourceAdoptions(adoptions, trackedFiles, { toolRoots = [] } = {}) {
  ...
  for (const [index, adoption] of adoptions.entries()) {
    ...
    if (adoption?.provenance !== undefined) {
      ...
      if (!provenance.trees || typeof provenance.trees !== "object" || Array.isArray(provenance.trees)) {
        errors.push(`${label}: provenance trees must be an object`);
        continue;
      }
...
export function validateSourceRepository({
  ...
  const errors = validateSourceAdoptions(adoptions, trackedFiles, { toolRoots });
