...
async function archiveFixture(extraPackage = false): Promise<string> {
  ...
  for (const name of extraPackage ? [...packages, "undisclosed"] : packages) {
    write(
      ...
      JSON.stringify({ name, version: name === "undisclosed" ? "1.0.0" : "0.81.1" }),
    );
  }
  const components = packages.map((name) => ({ id: `${name}@0.81.1` }));
  write(
    join(source, "apps/service/dist/client/licenses/release-dependencies.json"),
    JSON.stringify({
      schemaVersion: 2,
  ...
  write(join(source, "apps/service/dist/client/licenses/sbom.cdx.json"), "{}\n");
  write(join(source, "apps/service/dist/client/licenses/THIRD-PARTY-NOTICES.txt"), "notices\n");
