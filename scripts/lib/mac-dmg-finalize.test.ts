...
describe("macOS DMG finalization", () => {
  it("requires exactly one DMG artifact", () => {
    expect(
      resolveSingleMacDmgFileName(["HarnessOS-0.6.0-arm64.zip", "HarnessOS-0.6.0-arm64.dmg"]),
    ).toBe("HarnessOS-0.6.0-arm64.dmg");
  ...
  it("sign-checks, notarizes, staples, and validates the final DMG in order", () => {
    const dmgPath = "/tmp/HarnessOS-0.6.0-arm64.dmg";
  ...
  it("fails closed when Apple notarization credentials are unavailable", () => {
    expect(() =>
      buildMacDmgFinalizationCommands("/tmp/HarnessOS.dmg", {
