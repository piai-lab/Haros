...
const FORMAT = "product-truth-complexity-v5";
...
const configPath = resolve(scriptDirectory, "complexity-universe-v5.json");
...
assertNoDuplicateJsonKeys(configText, "complexity-universe-v5.json");
...
if (
  ...
  throw new Error("Usage: measure-complexity-v5.mjs [--negative-fixture <name>] --ref <lowercase-full-40-hex-commit>");
}
...
const loadFixture = (name, ancestors = new Set()) => {
  ...
  const fixture = JSON.parse(readFileSync(resolve(scriptDirectory, "fixtures/complexity-v5", `${name}.json`), "utf8"));
...
for (const [path, expected] of [
  ["scripts/product-truth/measure-complexity-v5.mjs", scriptBytes],
  ["scripts/product-truth/complexity-universe-v5.json", configBytes],
]) {
...
const analyzeDirectToolClassifierCopy = () => {
  ...
  const collectCallables = (node) => {
    const name = localCallableName(node);
    if (name) named.set(name, [...(named.get(name) ?? []), node]);
    ts.forEachChild(node, collectCallables);
  };
  collectCallables(file);
  ...
  const creatorText = bodyText(creator);
  if (!/mkdtemp(?:Sync)?\s*\(/.test(creatorText) || !/(?:0o700|448)/.test(creatorText) || !/tmpdir\s*\(/.test(creatorText)) {
    violations.push({ kind: "scratch-not-private-fresh" });
  }
  const copierText = bodyText(copier);
  for (const token of ["O_NOFOLLOW", "O_RDONLY", "O_CREAT", "O_EXCL", "0o600", "sourceStatBefore", "sourceStatAfter", "copyStat", "sourceBytes", "copyBytes", "sourceSha256", "copySha256", "sourceManifestBefore", "sourceManifestAfter"]) {
    if (!copierText.includes(token)) violations.push({ kind: "copy-identity-token", token });
  }
  const openerText = bodyText(opener);
  if (!/new\s+DatabaseSync\s*\(/.test(openerText) || !/(?:readOnly|readonly)\s*:\s*true/.test(openerText) || /(?:create|readwrite)\s*:\s*true/.test(openerText)) {
    violations.push({ kind: "sqlite-not-readonly-no-create" });
  }
  ...
  if (!/\b(?:close|Symbol\.dispose)\b/.test(entryText)) violations.push({ kind: "database-close-missing" });
...
const analyzeRuntimeOwner = (definition) => {
  ...
  const executeStatement = (statement, states, presentLiterals) => {
    ...
    if (ts.isThrowStatement(statement)) {
      const next = processExpression(statement.expression, entered).map((state) => ({ ...state, completion: "throw" }));
      return dedupeStates([...carried, ...next]);
    }
    ...
    if (ts.isForOfStatement(statement)) {
      ...
      return dedupeStates([...carried, ...afterIterable, ...once]);
    }
    if (ts.isWhileStatement(statement) || ts.isForStatement(statement)) {
      ...
      return dedupeStates([...carried, ...(value.kind === "boolean" && value.value === true ? once : [...afterCondition, ...once])]);
    }
    if (ts.isDoStatement(statement)) {
      const once = executeStatement(statement.statement, entered, presentLiterals).map((state) =>
        ["break", "continue"].includes(state.completion) ? { ...state, completion: "normal" } : state,
      );
      return dedupeStates([...carried, ...processExpression(statement.expression, once)]);
    }
    if (ts.isTryStatement(statement)) {
      const tried = executeStatement(statement.tryBlock, entered, presentLiterals);
      const thrown = tried.filter(({ completion }) => completion === "throw").map((state) => ({ ...state, completion: "normal" }));
      const notThrown = tried.filter(({ completion }) => completion !== "throw");
  ...
  const presentCfgResults = legacyRequirements.map(({ literal }) => {
    const before = new Map([...lockPredecessorsAtSink].map(([position, states]) => [position, states.length]));
    const completion = runBoundedCfg(new Set([literal]));
    const reachedSinks = sinks.filter((sink) => (lockPredecessorsAtSink.get(sink.position)?.length ?? 0) > (before.get(sink.position) ?? 0));
    return { literal, reachedSinks: reachedSinks.map(({ position: _position, ...sink }) => sink), completion: completion.map(({ completion: kind }) => kind) };
  });
  ...
  for (const sink of sinks.sort((left, right) => left.position - right.position)) {
    ...
    report.lockStatesAtSinks.push({
      sink,
      state: matchingRelease ? "released" : uncertainRelease || detachedReleaseSites.length || mutuallyExclusiveAcquire ? "unknown" : "held",
