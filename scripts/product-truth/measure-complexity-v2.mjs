...
const FORMAT = "product-truth-complexity-v2";
const scriptPath = fileURLToPath(import.meta.url);
const scriptDirectory = dirname(scriptPath);
const repositoryRoot = resolve(scriptDirectory, "../..");
const configPath = resolve(scriptDirectory, "complexity-universe-v2.json");
...
if (
  ...
  throw new Error("Usage: measure-complexity-v2.mjs [--negative-fixture <name>] --ref <lowercase-full-40-hex-commit>");
}
const commit = commitArgument;
const negativeFixture = fixtureMode
  ? JSON.parse(readFileSync(resolve(scriptDirectory, "fixtures/complexity-v2", `${fixtureName}.json`), "utf8"))
  : null;
if (negativeFixture?.workCoverageAdd) {
  for (const [work, paths] of Object.entries(negativeFixture.workCoverageAdd)) {
    if (!config.workCoverage[work] || !Array.isArray(paths)) throw new Error("Malformed negative coverage fixture.");
    config.workCoverage[work].paths.push(...paths);
  }
}
...
for (const [path, expected] of [
  ["scripts/product-truth/measure-complexity-v2.mjs", scriptBytes],
  ["scripts/product-truth/complexity-universe-v2.json", configBytes],
]) {
...
const globPattern = (pattern) => new RegExp(`^${pattern
  ...
  .replace(/\*/g, "[^/]*")
  .replace(/\0/g, ".*")}$`);
const workMatches = (path, boundary) =>
  boundary.paths.includes(path) || boundary.globs.some((pattern) => globPattern(pattern).test(path));
...
if (JSON.stringify(Object.keys(config.workCoverage).sort()) !== JSON.stringify([...requiredWorkIds].sort())) {
  throw new Error("The frozen work coverage manifest must contain exactly the five product Works.");
}
...
const materializedWorkCoverage = requiredWorkIds.map((work) => {
  const boundary = config.workCoverage[work];
  const materializedPaths = [...tree.keys()]
    .filter((path) => config.extensions.includes(extname(path)) && measuredCategories.has(classification(path)) && workMatches(path, boundary))
    .sort();
  const newlyMaterializedBoundedPaths = materializedPaths.filter(
    (path) => !boundary.paths.includes(path) && !config.allowedMaterializedGlobPaths.includes(path),
  );
  return {
    work,
    declaredPaths: boundary.paths,
    declaredGlobs: boundary.globs,
    materializedPaths,
    missingDeclaredPaths: boundary.paths.filter((path) => !tree.has(path)).sort(),
    globMatches: boundary.globs.map((pattern) => ({
      pattern,
      paths: materializedPaths.filter((path) => globPattern(pattern).test(path)),
    })),
...
const historicalSeeds = [...tree.keys()].filter(
  (path) =>
    measuredCategories.has(classification(path)) &&
    (config.historicalProductionSeeds.paths.includes(path) ||
      config.historicalProductionSeeds.globs.some((pattern) => globPattern(pattern).test(path))),
);
const seedPaths = new Set([
  ...historicalSeeds,
  ...materializedWorkCoverage.flatMap((entry) => entry.materializedPaths),
].filter((path) => isSource(path) || path.endsWith(".json")));
const resolveBidirectionalClosure = (initialPaths) => {
  const closure = new Set(initialPaths);
  let changed = true;
  while (changed) {
    changed = false;
    for (const edge of uniqueInternalEdges) {
      if (closure.has(edge.source) && !closure.has(edge.target)) {
...
const closurePaths = resolveBidirectionalClosure(seedPaths);
for (const path of config.universeRequiredPaths) if (tree.has(path)) closurePaths.add(path);
for (const path of negativeFixture?.universeOmit ?? []) closurePaths.delete(path);
const inUniverse = (path) => closurePaths.has(path);
const universePaths = [...closurePaths].filter((path) => tree.has(path) && config.extensions.includes(extname(path))).sort();
...
const workCoverage = materializedWorkCoverage.map((report) => {
  const workClosure = resolveBidirectionalClosure(report.materializedPaths.filter((path) => isSource(path)));
  const closurePathsForWork = [...workClosure].sort();
  return {
    ...report,
    uncoveredPaths: report.materializedPaths.filter((path) => !inUniverse(path)),
    resolvedInternalProductionImportClosure: {
      count: closurePathsForWork.length,
      sha256: sha256(Buffer.from(`${closurePathsForWork.join("\n")}\n`)),
      omittedFromGlobalClosure: universePaths.filter((path) => !workClosure.has(path)),
      addedOutsideGlobalClosure: closurePathsForWork.filter((path) => !inUniverse(path)),
    },
  };
});
...
const canonicalProductDatabaseConsumers = [];
const noncanonicalProductDatabaseResolutionSites = [];
for (const [path] of productionText) {
  const file = sourceFile(path);
  const visit = (node) => {
    if (ts.isCallExpression(node)) {
      const symbol = terminalSemanticSymbol(semanticIdentity(node.expression, path, node));
      if (config.productDatabaseComposition.canonicalResolverSymbols.includes(symbol)) {
        canonicalProductDatabaseConsumers.push({
          path,
      ...
      if (ts.isPropertyAccessExpression(node.expression) && node.expression.name.text === "join") {
        ...
        const containsDatabaseIdentity = config.productDatabaseComposition.databaseIdentityTokens.some((token) => source.includes(token));
        const owner = declarationOwner(node);
        if (containsDatabaseIdentity && !config.productDatabaseComposition.canonicalResolverSymbols.includes(owner)) {
          noncanonicalProductDatabaseResolutionSites.push({
            path,
            directory: posix.dirname(path),
            line: file.getLineAndCharacterOfPosition(node.getStart(file)).line + 1,
            owner,
...
canonicalProductDatabaseConsumers.sort((left, right) => `${left.path}\0${left.line}`.localeCompare(`${right.path}\0${right.line}`));
noncanonicalProductDatabaseResolutionSites.sort((left, right) => `${left.path}\0${left.line}`.localeCompare(`${right.path}\0${right.line}`));
...
const requiredLegacyPresenceSentinels = config.legacyClassification.requiredLegacyPresenceSentinels.map((sentinel) => {
  ...
  if (!content) return { ...sentinel, status: "absent", literalOccurrences: 0, operationOccurrences: 0 };
    ...
    const owner =
      (ts.isFunctionDeclaration(node) || ts.isMethodDeclaration(node)) && node.name
        ...
        : ts.isVariableDeclaration(node) && ts.isIdentifier(node.name) && node.initializer &&
          ...
          ? node.name.text
          : null;
    if (owner === sentinel.owner) ownerText = node.getText(file);
    if (!ownerText) ts.forEachChild(node, visit);
  };
  visit(file);
  const literalOccurrences = occurrenceCount(ownerText, sentinel.literal);
  const operationOccurrences = occurrenceCount(ownerText, sentinel.operation);
  return {
    ...sentinel,
    status: literalOccurrences === 1 && operationOccurrences === sentinel.count ? "exact" : "mismatch",
    literalOccurrences,
...
const legacyClassification = {
  ...
  disjoint: config.legacyClassification.toolOnlyDestructiveIdentities.every(
    (identity) => !config.legacyClassification.forbiddenCompatibility.includes(identity),
  ),
};
...
const report = {
  ...
  coverage: {
    source: "frozen-historical-and-five-work-seeds-with-bidirectional-resolved-import-closure",
    candidateSelectedPathsUsed: false,
    workingTreeUsed: false,
    dependencyManifestSeedsUsed: false,
    excludedLockfiles: ["bun.lock"],
    historicalProductionSeeds: historicalSeeds.sort(),
    productionSeeds: [...seedPaths].sort(),
    resolvedProductionClosure: universePaths,
