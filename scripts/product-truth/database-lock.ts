...
interface DatabaseLockInstrumentationPort {
  readonly operation?: (
    operationId: string,
    site: "before" | "after",
...
function lockOperation<Result>(
  instrumentation: DatabaseLockInstrumentationPort | undefined,
  operationId: string,
  ordinal: number | "single",
  effect: () => Result,
): Result {
...
function readProfileLockOwner(
  path: string,
  instrumentation?: DatabaseLockInstrumentationPort,
  ordinal?: number,
  ...
  const descriptor = phase !== undefined && ordinal !== undefined
    ? openLockDescriptor(instrumentation, openId, ordinal, () => FS.openSync(path, flags))
    : FS.openSync(path, flags);
  let decoded: unknown;
  try {
    const text = phase !== undefined && ordinal !== undefined
      ? lockOperation(instrumentation, readId, ordinal, () => FS.readFileSync(descriptor, "utf8"))
  ...
  if (phase !== undefined && ordinal !== undefined)
    closeLockDescriptor(descriptor, instrumentation, closeId, ordinal);
...
function acquireProductTruthProfileLockInternal(
  ...
  if (lockOperation(instrumentation, "db-lock.lstat-existing", ordinal, () => pathExists(path))) {
    const first = readProfileLockOwner(path, instrumentation, 0, "existing");
    instrumentation?.barrier?.("db-lock-record-to-process-probe", 0, () =>
    ...
    if (
      ...
      lockOperation(instrumentation, "db-lock.probe-owner", 0, () =>
        observedProcessState(first.pid, instrumentation)) !== "dead" ||
      observedProcessState(second.pid, instrumentation) !== "dead"
    ) throw new Error("OWNER_NOT_STOPPED");
    instrumentation?.barrier?.("db-lock-dead-proof-to-stale-remove", 0, () =>
      FS.writeFileSync(path, `${JSON.stringify({ pid: process.pid, token: randomUUID() })}\n`));
    ...
    if (
      ...
      observedProcessState(current.pid, instrumentation) !== "dead"
    ) throw new Error("OWNER_NOT_STOPPED");
    const stale = `${path}.stale.${first.token}.${randomUUID()}`;
    lockOperation(instrumentation, "db-lock.remove-stale", 0, () => {
  ...
  try {
    lockOperation(instrumentation, "db-lock.write-publish", ordinal, () =>
      FS.writeFileSync(descriptor, `${JSON.stringify({ pid: process.pid, token })}\n`));
    lockOperation(instrumentation, "db-lock.fsync-publish", ordinal, () => FS.fsyncSync(descriptor));
  } finally {
    closeLockDescriptor(descriptor, instrumentation, "db-lock.close-publish", ordinal);
  }
  FS.renameSync(staging, path);
  lockOperation(instrumentation, "db-lock.fsync-parent", ordinal, () => syncDirectory(root));
  instrumentation?.barrier?.("db-lock-publish-to-verify", ordinal, () =>
    FS.writeFileSync(path, `${JSON.stringify({ pid: process.pid, token: randomUUID() })}\n`));
  const stat = FS.lstatSync(path);
  const verified = readProfileLockOwner(path, instrumentation, ordinal, "verify");
  if (verified.pid !== process.pid || verified.token !== token)
    throw new Error("OWNER_NOT_STOPPED");
  return { path, token, dev: stat.dev, ino: stat.ino };
}

function releaseProductTruthProfileLockInternal(
  ...
  if (parsed.pid !== process.pid || parsed.token !== lock.token)
    throw new Error("DESTRUCTION_INCOMPLETE");
  lockOperation(instrumentation, "db-lock.remove-owned", ordinal, () => {
  ...
  lockOperation(instrumentation, "db-lock.verify-absent", ordinal, () => {
...
function readOwner(
  ...
  const closeId = phase === "existing" ? "db-lock.close-existing" : "db-lock.close-verify";
  const descriptor = phase !== undefined && ordinal !== undefined
    ? openLockDescriptor(instrumentation, openId, ordinal, () => FS.openSync(ownerPath, flags))
...
function observedProcessState(
  ...
  return instrumentation?.processState?.(pid, actual) ?? actual;
}
...
function publish(
  ...
  try {
    ...
    lockOperation(instrumentation, "db-lock.fsync-parent", ordinal, () =>
...
function acquireProductTruthDatabaseLockInternal(
  databasePath: string,
  instrumentation?: DatabaseLockInstrumentationPort,
  ordinal = 0,
  ...
  lockOperation(instrumentation, "db-lock.lstat-existing", ordinal, () => pathExists(lockPath));
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const token = randomUUID();
    if (publish(lockPath, token, instrumentation, ordinal)) {
      const lockStat = FS.lstatSync(lockPath);
      ...
      const verified = readOwner(lockPath, instrumentation, ordinal, "verify");
      if (verified.pid !== process.pid || verified.token !== token)
        throw new Error("OWNER_NOT_STOPPED");
    ...
    const owner = attempt === 0
      ? readOwner(lockPath, instrumentation, 0, "existing")
      : readOwner(lockPath);
    if (attempt === 0)
      instrumentation?.barrier?.("db-lock-record-to-process-probe", 0, () =>
        FS.writeFileSync(
    ...
    if (
      lockOperation(instrumentation, "db-lock.probe-owner", 0, () =>
        observedProcessState(owner.pid, instrumentation)) !== "dead" ||
      attempt > 0
    ) throw new Error("OWNER_NOT_STOPPED");
    const current = readOwner(lockPath);
    if (current.pid !== owner.pid || current.token !== owner.token || observedProcessState(current.pid, instrumentation) !== "dead") throw new Error("OWNER_NOT_STOPPED");
    if (attempt === 0)
      instrumentation?.barrier?.("db-lock-dead-proof-to-stale-remove", 0, () =>
        FS.writeFileSync(
          Path.join(lockPath, "owner.json"),
    ...
    if (
      ...
      observedProcessState(finalCurrent.pid, instrumentation) !== "dead"
    ) throw new Error("OWNER_NOT_STOPPED");
...
function releaseProductTruthDatabaseLockInternal(
...
export function acquireProductTruthProfileLock(
  ...
  return acquireProductTruthProfileLockInternal(identity);
}

export function releaseProductTruthProfileLock(lock: ProductTruthProfileLock): void {
  releaseProductTruthProfileLockInternal(lock);
}

export function acquireProductTruthDatabaseLock(
  databasePath: string,
): ProductTruthDatabaseLock {
  return acquireProductTruthDatabaseLockInternal(databasePath);
}

export function releaseProductTruthDatabaseLock(lock: ProductTruthDatabaseLock): void {
  releaseProductTruthDatabaseLockInternal(lock);
}
...
export async function withProductTruthDatabaseLocks<Result>(
  ...
  try {
    for (const [ordinal, identity] of PROFILE_IDENTITIES.entries()) {
      const lock = acquireProductTruthProfileLockInternal(identity, instrumentation, ordinal);
      if (lock !== null) {
    ...
    for (const lane of LANES) {
      ...
      for (const filename of [LEGACY_PRODUCT_DATABASE, LEGACY_SERVICE_DATABASE]) {
        databaseLocks.push(
          acquireProductTruthDatabaseLockInternal(
    ...
    for (const [reverseIndex, lock] of [...databaseLocks].reverse().entries()) {
      try {
        ...
        instrumentation?.barrier?.("db-lock-owned-to-release", ordinal, () => {
          FS.writeFileSync(
            Path.join(lock.lockPath, "owner.json"),
            `${JSON.stringify({ pid: process.pid, token: randomUUID(), createdAt: new Date().toISOString() })}\n`,
          );
        });
        releaseProductTruthDatabaseLockInternal(lock, instrumentation, ordinal);
      } catch (cause) {
    ...
    for (const [reverseIndex, lock] of [...profileLocks].reverse().entries()) {
      try {
        ...
        releaseProductTruthProfileLockInternal(lock, instrumentation, ordinal);
      } catch (cause) {
        releaseFailure ??= cause;
      }
    }
