...
- canonical Web slice 已删除 200ms 单选自动前进和主 Composer answer fallback；Host projection 自动追加 `自定义` sentinel，custom inline autofocus，note 仅随 preset 渐进出现，Review 结构化区分主答案/note，并用 root-scoped keyboard 与 radio/checkbox/aria/focus 语义覆盖交互。
