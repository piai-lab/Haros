...
R11-7 的首个 source candidate 已在 exact pushed `47613073877c733d2585865e9ad65413d7a0a125` 进入 `main`。其窄 safety follow-up 进一步把 bootstrap completion 从 public `thread.meta.update` 移到既有 server-only internal command authority，public schema显式拒绝客户端伪造；Reactor只在Provider接受exact bootstrap后提交同一cutoff/version。skill alias normalization只作用于latest user segment，随后才与byte-exact imported prefix组装；history-only Claude的native retry、stale cursor clear与fresh replay始终复用同一exact selection，不进入ordinary transcript summary/truncate路径。Web在既有message action single-flight内保留exact command/thread receipt：一次同ID replay与两次shell hydration有界收敛，dispatch已接受但shell暂不可读时只报告“仍在加载”，再次操作继续同一identity，不新建第二Thread。
...
packaged journey在任何窗口操作前证明Main、Helper/Renderer与bundled Server均使用任务专用HOME、userData、`OMNIMIND_HOME`、XDG、TMP及Provider private homes；fresh启动、正常退出、同profile重开与再次退出均闭合且进程归零。DeepSeek与Xiaomi MiMo分别完成history-only middle-assistant fork：source prefix marker A保留、cutoff后的marker B不进入target首turn，同profile重开continuation仍只继承A且没有重复bootstrap。fresh隔离Provider homes中Claude/Codex未认证，OpenCode无可用private credential，因此native-capable scoped-vs-full fork journey准确记为`external-unavailable`；Windows Explorer/taskbar packaged evidence继续pending，不以macOS注入测试冒充。含credential的任务profile已清理，artifact与可恢复备份保留。
