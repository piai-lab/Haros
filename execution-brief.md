...
S1 当前仅准入 R5 text↔tool causal interleave：assistant text boundary 由 canonical Provider runtime journal sequence 排序，只有真实 row-making event 才开启下一段；buffered delivery 保留边界，spill/truncation fail-safe 不伪造段。Migration `092` 在现有 `090`/`091` lineage 后增加 derived `message_text_segments`，fresh、through-91 upgrade、replay、rebuild、reopen、edit、revert 与 rollback 必须同义；SQLite/in-memory/snapshot/repository 共用 optional ordered segments。Web 只扩现有 normalization、WorkLog 与 MessagesTimeline owner，settled 多段 assistant 复用 W3 narration/work collapse，streaming 仍是单一 live row。该关注点不准入 Automation 产品语义、Workspace/UI/Git/PR/model picker、其他 streaming 改造、package/install 或 release/feed；README adopted Synara head 继续等待完整 source slice 闭合后更新。
