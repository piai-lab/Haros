...
没有正在施工的 source adoption 或瘦身 Slice。责任级瘦身已从 `main@a63b283fb0` 闭合三个独立 `RETIRE`：旧 OmniMind free-form model hints、从未获得消费者的发行兼容双轨、Browser Host 旧 pipe identity 及其构建输入；当前 shipped product head 为 pushed `main@822af96fc0e0ba0ecd853beacf9b2a94543157b2`。

安装版 product bytes 准确绑定 pushed product `822af96fc0e0ba0ecd853beacf9b2a94543157b2`；isolated fresh launch、Browser 打开/导航/截图/关闭、同 profile 重开、旧 identity 缺席和最终进程清理已闭合。后续 docs-only main 状态提交不冒充 shipped bytes。混合多个产品结果的 AppSettings 迁移未获逐项裁决，不得从这三刀自动扩张为批量 legacy 清理。
...
下一轮只优先审计 `providerRuntime` 中仅定义、无消费者的兼容类型别名；先证明真实 support graph，再单独裁决 `RETIRE/KEEP`。Automation 的 `content` alias、`everyMinutes` shorthand 与默认 heartbeat 会改变 Agent tool contract，必须另开责任切片；SQLite migration 1–100 在首次公开发行冻结前不压缩，engine catalog warming 在出现性能反证前保留。新的 Synara 更新仍统一按 [`SYNARA-INTAKE.md`](SYNARA-INTAKE.md) 处理。
