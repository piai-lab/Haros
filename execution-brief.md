...
E2 已在 exact product `4b3c038a8…` / evidence `8c5193d60…` 完成 fresh independent completion audit 并 **Exit**：typed begin-login contract只接受 `api_key`，Server在任何 Pi mutation前用同一 task-local ModelRuntime证明目标是 builtin 且真实提供 `apiKey.login`，唯一 `runtime.login` 调用固定为 `api_key`；OAuth-only、models.json/custom与unknown id均 fail closed。judge 独立复核4 files/87 tests与Contracts/Server/Web typecheck，无P0/P1；producer的DeepSeek、packaged App与DMG证据仍保持candidate边界。当前唯一下一施工 slice是指南E5 OAuth typed interaction：只对Pi metadata真实暴露的builtin OAuth capability接通provider-owned browser/device/manual-code login、cancel/timeout/logout与provider-scoped refresh，并保持token不进入renderer/cache/log。E6 custom-provider persistent mutation仍未获授权且hard-gated；不得在E5新增models.json写入API、adapter、store或按品牌复制OAuth capability。E3/E4不因本轮继续重开。
