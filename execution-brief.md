...
E1 当前是已推送、已打包安装的 **candidate**，不是 producer 自证 verified：首轮 independent audit 在 `e0ba0b307…` 复现同一 reader 成功观察 `models.json` 后删除或换 inode 仍被当作合法新配置的 P1；exact product 修正 `9956e16c0…` 已在既有 reader 闭包内锁定首次成功 identity，后续缺失/替换固定 fail closed，并修正旧 archive digest mutation fixture。Server projection/security/Session 3 files/54 tests、typecheck/build、document contract 17/17、MiMo 首轮/continuation、DeepSeek 首轮、243-component legal closure、精确 source provenance，以及 fresh isolated installed-App 关闭/重开已形成更新证据；下一动作是对新的 exact product/evidence SHA 做 fresh independent E1 completion audit。E2 custom-provider mutation、auth interaction 与网络 refresh 继续 gated，在 E1 独立 Exit 前不施工；E3/E4 不因本轮 source adoption 重开。
