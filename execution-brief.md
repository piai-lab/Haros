...
截至当前 pushed SHA `6f7a10ec56e3d6687766012b3e790fb4f26adf3d`，Web 全量 `326` files / `4177` tests、Server 全量 `373` files / `4367` tests（另 3/16 skipped）通过，双方 typecheck 通过；Reasoning projection/ingestion、Skill injection focused tests 通过（ingestion `111/111`，ProviderCommandReactor `162/162`）。从该 SHA 重建的 arm64 DMG 已完成 240 项 legal closure，SHA-256 为 `a72e0a795eacb6ad6bcbcf3a8f767bc78da2f0d51ebb8666be6246c92243849a`；通过任务专用隔离环境的 packaged startup smoke。完整 fresh-profile Timeline 交互 journey（选择 Aihot、回执、Reasoning/command/mixed、关闭重开）尚未在 packaged App 上闭合，因此仍不能宣称安装版已完成全链路验收。
