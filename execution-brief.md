...
E1 当前是已推送、已打包安装的 **candidate**，不是 producer 自证 verified：Pi source tests、Server projection/security/Session tests、Web loading/empty/connected/stale/error 与中英 UI、MiMo 首轮/continuation、DeepSeek 首轮、deterministic archive、legal/SBOM，以及 fresh isolated installed-App 关闭/重开已形成证据；下一动作是对精确 product/evidence SHA 做 fresh independent E1 completion audit。E2 custom-provider mutation、auth interaction 与网络 refresh 继续 gated，在 E1 独立 Exit 前不施工；E3/E4 不因本轮 source adoption 重开。
