...
S1 当前仅准入 R7 Automation backend durability：沿既有 Automation contract、service、repository、scheduler 与 recovery 单一 owner，把 legacy `stop_on_error=true/false` 保持为明确 threshold `1/null`，持久化 consecutive failure、disabled reason/time 与单调 definition revision；run terminal transition 与 failure/success accounting 必须在现有 repository method 内同事务且只允许 active status，`skipped` 等 terminal 不得被覆盖。所有 definition writer 以 revision 为唯一 optimistic authority，跨请求 stale full-form/Agent view 必须冲突，只有初始 revision 已匹配且请求执行期间 touched fields 未变化时才可安全重放；misfire run+schedule advance、restart、max、one-shot 与 completion 均不得留下 ghost run 或覆盖新 definition。Migration 只使用新 ID `093` 增加 schema/default，不猜任何 legacy max-iterations reason/time；`automation_run` source/sidebar、完整 Automation 编辑 UX、其他 Synara 项、package/install、README adopted head 与 release/feed 均不在本关注点。
