...
S1 当前仅准入同一 R5 follow-up：revert/rollback 必须保留 retained messages 的原有 segments 并只删除 pruned rows，SQL、snapshot、in-memory 与 file reopen 同义；assistant boundary 只由同批实际 projected 且可能进入既有 WorkLog/Timeline 的 activity 在 durable dispatch 成功后触发，全局 quiet/hidden row 与另一 Thread 不得切段；streaming、buffered completion 与 spill 的 process-local state 只在 durable command 链成功后提交，before-commit 与 ambiguous-after-commit 均须以同一 command receipt/payload 幂等重试；mixed causal/legacy timeline 以两条各自稳定总序的流合并，不再使用非传递 pair comparator。该关注点仍不准入 Automation 产品语义、Workspace/UI/Git/PR/model picker、其他 streaming 改造、package/install 或 release/feed；README adopted Synara head 继续等待完整 source slice 闭合后更新。
