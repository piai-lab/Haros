...
- exact `cf1a1e580509423a92e5334a438a3e077d376210` 的三文件UI与 `584045a291a91e57ec50ce0e91cee29253334ef1` 的initial Session漏传都只保留历史证据。当前修复在同一 exact upstream `914cf147…` 上产生patch `499b1257…` 与vendored artifact `b57b866d…`，并冻结为exact pushed product SHA `db25a5b91343a4ddbf70fedd98ea3583bd020317`。该SHA已重建并安装：DMG `9a62e238…`、ZIP `43d44689…`、installed app.asar `95937e7f…`，asar内嵌commit一致，240项legal identities闭合。隔离packaged journey覆盖MiMo Chat preconfigured V1→save V2/no reload仍V1→explicit reload后V2、DeepSeek Agent V2、custom rules、no-op hash/mtime、close/reopen、restore factory与remove。F-22仍保持candidate。
...
architecture/brief、source、exact pushed SHA、installed bytes与隔离live/packaged evidence现已互相一致。下一步只剩维护者对F-22证据作独立裁决；实施者不自行升为verified。Node公共fs seam仍只提供进程内writer串行与expected-version optimistic conflict detection，不能原子消除最终极窄的非协作external-writer TOCTOU。一次`--version`探测被Electron当正常默认启动，未进入UI且立即停止；真实`~/.omnimind/agent` 7个Prompt leaf复核仍全部absent，该次探测不计入journey。
