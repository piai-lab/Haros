...
E1 的锁定 Pi `v0.84.1` public-API blocker 已由维护者单独授权的窄 source adoption 解除：既有 product-owned `@omnimind/pi-coding-agent` 增加 caller-owned `models.json` content reader、官方 parser 接受后的 provider-id provenance，以及 reader 模式下显式 `modelsStorePath` 继续使用 Pi `FileModelsStore`。后续同一用户任务为 stock/product Pi-family adapter 已公开的 Extension command/input handled 结算补充一份 typed prompt-outcome seam；stock `@earendil-works/pi-coding-agent` 只应用该 exact-hashed compiled patch，其 identity、configDir、state、catalog 与其他默认行为保持原样。Host 的唯一 reader 负责 physical containment、no-follow、4 MiB hard bound、64 KiB cancellation、same-handle identity、fatal UTF-8 与 observed-present 后状态变化 fail-closed；Settings、discovery、Session create/refresh 共用该 source，Pi 仍是 comments/JSON/schema/composition/error 与 catalog-store authority。唯一 generator 验证 exact revision、product source patch与stock dependency patch的 committed digest，任何 conflict 会在构建或安装前失败，普通产品 build 继续离线消费固定 tgz。
...
E5 OAuth 的 typed browser/device/manual-code interaction、取消/timeout/unmount cleanup、logout、provider-scoped catalog refresh、proxy correction 与 request-scoped callback presentation 均已形成 pushed candidate。真实隔离 packaged journey 已覆盖 OpenAI Codex login→catalog→两次 turn→完整退出/重开→logout；亮色 OmniMind callback 页面已覆盖 OpenAI Codex、Anthropic、OpenRouter、Radius，并只显示安全的“已收到授权”事实。实现结果已经闭合，剩余唯一 E5 动作是 fresh independent completion audit；在 judge 复现 P0/P1 前不得继续扩写 OAuth、修改 stock Pi 或重做 callback server。

E6 的 Pi-owned typed persistent mutation、四种 generic API、test/save/reopen/edit/retest/refresh/delete、retryable credential-first deletion、cold-start loading、正常产品语言与同供应商双实例 packaged journey均已形成 pushed candidate。Exact cumulative product `5c5df7675439d367a02c512c03c9b7d63f02096b` / evidence `f0f29e0d1dae1c8fbb9eac0a69cebc9c1b5610ce` 已证明两个同名 serviceId 分别保存、Settings/Composer 消歧、第二 exact selection 重启恢复、删除第一后第二 config/credential/catalog/selection不变、stock `.pi` 不存在且未创建 Conversation；后续独立audit结论见下段，不以producer证据替代。

E6 已由 fresh completion audit 对 exact product `66e0213c5…` 复核唯一 vendor generator、patch/tgz/lock/document digest、四协议配置保留、secret 裁剪与双实例 owner，结论 `CLEAN`、P0/P1=0。E5 的 OAuth/callback candidate仍欠独立 completion audit，不能因后续累计构建而自动 Exit。
