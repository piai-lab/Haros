...
- exact `cf1a1e580509423a92e5334a438a3e077d376210` 及其本机 ad-hoc 安装只证明旧“三类 Prompt 文件管理”candidate 的 source/live/packaged 事实。后续 exact pushed/installed `584045a291a91e57ec50ce0e91cee29253334ef1` 已实现“两卡片”模型，但真实 Provider journey 证伪了其 service-based initial Session 会漏传 customized default；其 installed app.asar `cfab377f…` 同样只保留为 superseded 历史证据。当前窄 runtime 修复在同一 exact upstream `914cf147…` 上产生 patch `499b1257…` 与 vendored artifact `b57b866d…`，已通过 source gate，但新的 pushed SHA 与 installed journey 尚未产生，F-22 不能据此 verified。
...
architecture/brief 已接纳新的两卡片关注点，F-22 继续只记录 candidate evidence、不授予准入。下一步是冻结并推送已通过 source gate 的 Session 创建修复，从该 exact pushed SHA 重建 macOS arm64 candidate，并在显式隔离 HOME、OmniMind home、Electron userData 与 Provider private home 后重验 Chat/Agent 的 preconfigured first request、save-without-reload、explicit reload、no-op、关闭重开及 MiMo/DeepSeek。Node 公共 fs seam 仍只提供进程内 writer 串行与 expected-version optimistic conflict detection，不能原子消除最终极窄的非协作 external-writer TOCTOU；F-22 是否 verified 仍由证据与维护者裁决，不由实施者拔高。
