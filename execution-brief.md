...
E5 OAuth typed interaction、取消/timeout/logout、proxy correction 与 request-scoped callback presentation 已形成 pushed candidate，fresh independent E5 completion audit仍是其 Exit 门。随后维护者对 Model services IA 的 root review 已在 exact product `043f7c284…` 收口：页面只管理 OmniMind service connection/auth/catalog/model/status-recovery；Git writing退出该页且底层字段不迁移，独立 Engine custom slug则薄迁到既有Agent engines detail并保留新增/删除/reset能力；fresh profile概览不再平铺全部Pi built-in，`添加模型服务`进入可键盘搜索的runtime service列表，detail使用单独页面并可返回保留查询。API地址入口是弱一级但必须真实完成的E6能力；capability未交付的build不显示入口，不能渲染disabled/“尚未开放”假操作，也不能以长期隐藏替代save/reopen/edit/delete。旧`setting-installed-clis` target字节继续指向更名后的`引擎详情`。OmniMind/Pi/Droid历史custom slug字段继续兼容保存但不再暴露无效editor或驱动自动展开；OmniMind/Pi从静态默认表移除，Project/Terminal/Kanban/direct resolver无 exact model时null/fail closed，Server settings拒绝provider-only跨到runtime-catalog Engine以避免hybrid binding。sole owner machine block与document contract固定这组边界，未来Pi intake还必须重证`provider_default`首项被上游明确标为default/recommended，否则回到显式选择。

当前 Model services 的下一项必交付能力是 E6 custom-provider persistent mutation。维护者于2026-08-13明确要求“配置必须保存”并授权在既有product-owned Pi adoption的ModelConfig/ModelRuntime owner内增加窄、typed、可删除的持久mutation seam；这解除的是source-adoption gate，不授权Host自建models.json parser/writer、Registry、store、fetcher或数据库。入口必须完成四种Pi generic API协议的填写/test/save、packaged restart后仍存在、edit/retest/refresh/delete；credential遵循Pi安全存储/reference语义。stock Pi保持原样，上游提供等价API后删除补丁。完成当前正在施工的Pi runtime model-detail projection后即进入该E6 slice；不能把disabled placeholder、永久隐藏或只读projection写成Model services完成，也不得借此重开E3/E4、Extension执行安全或Settings taxonomy。

重新冻结 E6 或进入 Extension/E7 前，还必须在既有 Web presentation owner 内关闭一项已复现的正常路径语言漂移：当前 Model services 的 API Key/OAuth/custom API、模型空态、保存/删除与来源说明仍把 `Pi`、`credential owner`、`models.json`、`runtime projection` 等技术 authority 写成普通用户文案，认证对话框还可让上游 raw prompt/event 取代本地化主状态。修正只按 `architecture/workbench.md` 分离产品摘要与技术详情，保持 Pi parser/credential/runtime authority、typed bridge、stock Pi identity 与秘密边界原样；不得全局改名内部 API、吞掉完成认证所需的 Provider instruction，或为文案另建 auth/translation registry。最窄证据必须覆盖 en/zh-CN 的 API Key、OAuth、custom API、空目录与失败恢复主状态，注入上游英文/内部术语时主表面仍使用 OmniMind 产品语言，并以 stock Pi Engine detail 成对证明真实 identity 未被误删。仅有 catalog parity、`t(...)` 调用或 runtime tests 绿色不能重新冻结该 candidate。

上一轮 exact product `043f7c284…` 的source与packaged证据只证明owner/static-default/Settings职责迁移，不证明E6。exact-SHA source gates通过Web typecheck、83个logic/unit与23个Chromium browser tests；arm64 DMG为244,763,767 bytes、SHA-256 `32a8f0d9d026d95a3c6ca7b29927db3f66ff0a73b26cde0e3807206928b98f80`，installed ASAR为241,360,854 bytes、SHA-256 `6db54485c5e625c6d19c7764b77473c3032ef5448f0d7f13ffa0ea9fe440dd3c`且embedded commit精确为`043f7c2841fbda51bf7b016eea58341784913ea7`。fresh isolated profile已从Main/Helper/Renderer/bundled Server证明task-specific HOME/OMNIMIND_HOME/Provider homes/userData，真实journey覆盖overview→添加→搜索DeepSeek→键盘detail→返回保留查询、旧stable target搜索跳转、Codex custom slug保存/重开/删除与Pi无假editor。测试期间发现/误启动的默认profile实例均在任何设置、凭据或其他产品mutation前终止且该轮证据作废；只观察进程/窗口状态，未读取、迁移或清理默认profile数据。纯Extension provider尚未进入Settings projection：这是V1必交付缺口而非可选增强；被动页面继续zero-execute，用户进入添加流程后必须复用Pi既有ResourceLoader/Session provenance owner完成intent-scoped安全投影，不建第二loader/global runtime。该项可在E6 generic models.json旅程之后独立闭合，但在E7/E8完整Model services candidate前必须完成。ChatView仍以non-null Thread presentation contract为local no-model draft构造synthetic空model，durable/direct consumer fallback已闭合但该presentation debt保持OPEN，不新增逐消费者guard。E5本体仍等待自己的fresh completion audit，不能由本delta自动Exit；当前App仍为ad-hoc且strict codesign失败，F-18不变。
