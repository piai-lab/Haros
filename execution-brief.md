...
- untouched baseline的三个P4反例是`browser_open reuse:true`导航当前Tab、Timeline假重开与token URL进入recent-history；当前source candidate已以Browser owner内部typed dedicated-tab seam、per-call ephemeral handle、exact pending reopen与non-history projection闭合，focused tests证明三个反例不再出现在候选路径。exact `52c8a25e75`安装候选进一步证明显式review会创建独立临时Tab、批准后terminal清理并让同一turn继续；多Thread packaged覆盖尚未完成，不能把这一局部门升级成完整安装产品证据。
