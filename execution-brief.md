...
因此当前唯一下一动作进入 E8：冻结 product `6e3e2ef3d…` 与对应 Campaign evidence SHA，先完成 E5 与 E7 的 fresh independent completion audit，再做 exact-SHA cumulative source/live/packaged 收口。真实 P0/P1 只回到现有 owner最小修正，不重开 E3/E4。此前未提交的 `.github/workflows/ci.yml`、`.github/workflows/release.yml` 与 `scripts/release-smoke.ts` side work 曾被一次越权工作树 reset 丢失，未从聊天摘要重造；在其 owner/维护者裁决恢复前，完整 Run 必须保持 non-candidate，且不得以 E8 顺手重建或把当前 baseline冒充该 side task结果。
