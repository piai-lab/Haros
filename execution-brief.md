...
因此当前唯一下一动作保持在 E8：冻结 product `6e3e2ef3d…` 与对应 Campaign evidence SHA。该 SHA 的 cumulative source、隔离 packaged 打开/关闭/重开及一组 MiMo direct Pi stream/continuation 证伪已经完成；剩余门是 E5 与 E7 的 fresh independent completion audit，以及外部 signing/notary、Windows/Linux 与 full completion audit。真实 P0/P1 只回到现有 owner最小修正，不重开 E3/E4，也不重复已经无新假设的 live/package probe。此前未提交的 `.github/workflows/ci.yml`、`.github/workflows/release.yml` 与 `scripts/release-smoke.ts` side work 曾被一次越权工作树 reset 丢失，未从聊天摘要重造；在其 owner/维护者裁决恢复前，完整 Run 必须保持 non-candidate，且不得以 E8 顺手重建或把当前 baseline冒充该 side task结果。
