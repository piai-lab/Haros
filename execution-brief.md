...
S1 当前仅准入 Provider lifecycle reliability 关注点：stale nonterminal 不进入 canonical journal 或 Product projection；旧 terminal 只可在没有 newer current generation 且 durable binding 仍精确证明同一 provider、event generation 与 active turn/session 时结算，journal、binding update 与 publish 共享同一接纳判定；recovery-capable send 不得快路由进 generation-mismatch live adapter，显式 stop 仍保留 zombie cleanup authority，interrupt 不在缺少 physical-generation proof 时越过既有 guard。该关注点保留现有 `runCurrent`/task drain、dispatch ambiguity、early `session.started` 与 generation lease，不准入 text-segment migration、Automation 产品语义、Workspace/UI/Git/PR/model picker、其他 streaming 改造、package/install 或 release/feed；README adopted Synara head 继续等待完整 source slice 闭合后更新。
