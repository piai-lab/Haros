...
Codex, Claude, Grok, Droid, Kilo, OpenCode and DeepSeek use the exact native package referenced by the
...
Codex、Claude、Grok、Droid、Kilo、OpenCode、DeepSeek 使用官方 npm 元数据指向的原生包，不依赖系统
...
`cursor-agent.cmd`, `agy.exe`, `grok.exe`, `droid.exe`, `kilo.exe`, `opencode.exe`, `dsh.exe` on Windows).
