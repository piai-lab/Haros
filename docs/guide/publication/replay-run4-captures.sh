...
replay_root="${GUIDEBOOK_RUN4_CAPTURE_REPLAY_DIR:-$(mktemp -d "$web_root/node_modules/.guidebook-run4-capture-replay.XXXXXX")}"
state_root="${GUIDEBOOK_RUN4_CAPTURE_STATE_DIR:-$(mktemp -d /tmp/haros-guidebook-run4-capture-state.XXXXXX)}"
...
(
  ...
  HARNESSOS_HOME="$state_root/harnessos-home" \
    ...
    src/components/GuidebookRun4.capture.browser.tsx
)
...
for capture_id in \
  ...
  capture-18-recovery-settings; do
...
test "$capture_count" = "4"
printf 'run4-capture-replay=PASS captures=%s state=%s output=%s\n' \
