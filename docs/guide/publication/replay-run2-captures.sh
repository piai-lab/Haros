...
replay_root="${GUIDEBOOK_RUN2_CAPTURE_REPLAY_DIR:-$(mktemp -d "$web_root/node_modules/.guidebook-run2-capture-replay.XXXXXX")}"
state_root="${GUIDEBOOK_RUN2_CAPTURE_STATE_DIR:-$(mktemp -d /tmp/haros-guidebook-run2-capture-state.XXXXXX)}"
...
(
  ...
  HARNESSOS_HOME="$state_root/harnessos-home" \
    ...
    src/components/GuidebookRun2.capture.browser.tsx
)

capture_count=0
maximum_normalized_rmse="0.002"
for capture_id in capture-04-engine-availability capture-05-exact-model capture-06-permission-decision; do
...
printf 'run2-capture-replay=PASS captures=%s state=%s output=%s\n' "$capture_count" "$state_root" "$replay_root"
