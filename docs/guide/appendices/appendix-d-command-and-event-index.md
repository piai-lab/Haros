...
| Pinned messages                 | `thread.pinned-message-added`<br>`thread.pinned-message-removed`<br>`thread.pinned-message-done-set`<br>`thread.pinned-message-label-set`                                            | Rebuilds bounded Thread annotations without turning UI state into a separate owner. Transcript highlight/underline markers are retired; historical marker events rewrite to `thread.meta-updated`.                                 |
