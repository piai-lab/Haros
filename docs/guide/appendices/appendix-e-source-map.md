...
|       7 | First-Run Setup                                                | `apps/web/src/onboarding/OnboardingDialog.tsx`                        | `apps/web/src/onboarding/logic.test.ts`                                               | A welcome tour may exist, but it never silently chooses an Engine. Settings/Composer plus capability projection still own send readiness. |
...
|      17 | Notes and Pinned Messages                                      | `packages/shared/src/pinnedMessages.ts`                               | `apps/server/src/orchestration/Layers/pinnedMessagesRoundTrip.integration.test.ts`    | The owner defines behavior; the test can disprove the edition claim but does not own it. Transcript markers are retired by migration 005. |
