...
chapter: 17
part: 3
title: Notes and Pinned Messages
language: en
...
source_anchors:
  ...
  - apps/server/src/persistence/Migrations/005_RemoveTranscriptMarkers.ts
---

# Chapter 17 — Notes and Pinned Messages {#chapter-17}

## The question

A Thread can remain useful long after its first Turn. That durability creates a practical problem:
the facts you need are scattered through an expanding transcript. Haros offers two memory aids with
different scopes. Thread notes summarize context for the whole Thread. A pinned Message points back
to one complete Message.

They are not two styles for the same annotation. Scope determines what a reader can safely infer.
A note is authored context and may be revised. A pin preserves the identity of an existing Message.
Choosing the smallest correct scope keeps memory useful without turning it into a second transcript.

Transcript highlight and underline markers are retired. Historical marker events keep their sequence
and rewrite to `thread.meta-updated`; pins and notes remain. Selecting assistant text can still add
the excerpt to Chat. That is a composer attachment, not a durable marker.
...
_Figure 17.1 — Historical edition art still shows a retired third column. Current product memory is notes and pins._

**Accessible equivalent.** Thread Notes apply to the Whole Thread. A Pinned Message points to a Whole message. Text markers are no longer a product aid.
...
If one clause in a long Message matters, pin the Message and quote the clause in the Thread note, or
add the selected text to Chat as a composer attachment. Do not expect a durable highlight to survive
reconnect as a third annotation type.
...
stable nouns and, when important, point readers to a pinned Message.
...
observations and one final disposition, pin the final disposition and quote the two clauses that
need follow-up in the Thread note. If all ten remain equally important, the Thread may need a clearer
note or a separate synthesis Message.
...
1. Read the latest Thread state and decide whether you need orientation or a whole-message anchor.
2. Choose notes or a pin by scope—not by which control is nearest.
3. Give pins short labels that explain why the source matters.
4. For notes, write a compact present-tense summary and link mentally or explicitly to durable
   transcript evidence.
5. Reopen the aid once to ensure it resolves to the intended Thread or Message.
...
contract. Privacy boundary: the original must not be forwarded when normalization fails.” That
orients the whole task and records the exact constraint.

She pins the user's Message because the complete reproduction steps and attachment are important.
Later, the team can navigate at two resolutions: summary and source Message. The precise clause lives
in the note, next to the pin that still owns provenance.

After the fix is verified, Lena revises the note to record the current outcome. The pinned user
Message remains useful as the original report. Six months later, a maintainer can still read the
actual history without confusing a former highlight with permanent transcript content.
...
| one privacy sentence             | Thread note    | quote the clause beside the pin          | source Message around it     |
| obsolete hypothesis              | revise note    | aid no longer helps                      | transcript remains unchanged |
...
Avoid labels that prejudge unresolved facts. “Probable parser race” is safer than “confirmed parser
race” until evidence confirms it. The annotation should not gain more certainty than its source.

## Update and removal boundaries

Editing Thread notes changes authored Thread-level context. Updating a pin label changes the aid's
explanation. Removing an aid removes its navigational relationship. None of these ordinary actions
edits the source Message.

This separation gives cleanup a small blast radius. Teams can prune a cluttered pin list without
...
different lifecycle. A pin attached to the old Message does not automatically become a truthful pin
on the new one. Re-resolve the intended source and create a new aid when necessary.

## Failure and recovery

### The aid points to the wrong place

Stop and inspect its stable source relationship. Similar text is not enough. Confirm Thread ID and
Message ID. Remove the incorrect aid and recreate it against the correct source. Do not alter
transcript text to make a bad annotation appear right.

### A save appears to vanish

The client may have shown an optimistic update before server acceptance. Reconnect or reload the
projection. If the note or pin is absent, verify limits and validation errors, then submit once more.
Avoid repeated clicks, which can create duplicate aids when delayed responses arrive.

### A source Message is unavailable

An unresolved pin is not evidence that the Message was deleted by the annotation action. Check Thread
scope and history state first. If the underlying Message was removed through a separate history
lifecycle, the aid should fail visibly or be cleaned up; it must not silently attach to a different
similar Message.

### The limit is reached

Curate rather than bypass. Merge overlapping Thread notes or remove obsolete pins. Do not create
parallel Threads solely to obtain a fresh quota, because that fragments the very history the aids
...
every note or pin. When exact context matters, inspect the admitted request and capability contract
rather than guessing from visibility.
...
fabricate native Session continuity. Notes and pin relationships may remain visible as Haros facts
while the target begins a new Engine Session. That is useful precisely because product memory and
runtime memory are not the same thing.

## Review hygiene

At the end of a milestone, scan memory aids before creating more. Ask whether the Thread note states
the current situation and whether each pin still saves meaningful search time. Remove aids that
merely repeat the latest Message.
...
into notes or labels. The same security boundary applies to memory views as to ordinary Messages.
...
- You need one sentence in a long review. Pin the Message and quote the sentence in the note, or add
  the selection to Chat.
...
Neither grants authority, changes Project ownership, or substitutes for the Timeline and receipts
that prove work occurred.
...
Take one long Message and identify two possible memories: a whole-Thread implication, and the reason
the complete Message matters. Write a hypothetical note and pin label. If the note merely quotes one
clause, it is too narrow unless that clause is the current constraint. If the pin label states a
conclusion absent from the Message, it is misleading.
...
Useful evidence includes the Thread ID, aid type, stable Message ID, the label, the action attempted,
and the state after canonical reload. Avoid screenshots that expose unrelated private Messages. A
narrow description makes it possible to determine whether the defect is in validation, relationship
persistence, projection, or navigation.

Do not report “memory lost” when only a composer selection failed to attach. State separately whether
the aid record exists and whether the source Message remains. Those are two different preservation
claims.

## Source trail

- `packages/contracts/src/orchestration.ts` defines Thread notes and pinned-Message shapes, limits,
...
- `apps/server/src/persistence/Migrations/005_RemoveTranscriptMarkers.ts` retires historical
  highlight/underline payloads while preserving event identities, pins, and notes.
