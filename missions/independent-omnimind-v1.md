...
| F-05 | `Agent｜Chat` 是侧栏顶部同时可见、Agent 左/Chat 右、一次激活的唯一一级入口；Agent=folder-backed Project Thread，Chat=Home/Studio managed Thread，Groups=Spaces；无第二 durable objects/panel/navigation authority | source audit + minimum-width/dual-locale/a11y create/reopen/switch journeys | open | 历史 `c0c0cc882` 曾实现并验证直接双入口，但不再拥有架构执行权；当前 source 在正确位置恢复为只显示 active surface 的 dropdown，未满足最新 Workbench owner。须基于当前 route/restore/prewarm 做窄修复，并证明显式隐藏 Chat 时不出现单选项假 switcher | `d442d00a1027feeab819c761a4c0cae5ade5569b` |
...
| F-10 | Synara Workbench、Settings、File/Viewer/Diff/Terminal/Git、stream/scroll、a11y/perf 在 surgery 后无 material regression；OmniMind-owned 一级入口和 first-party public exits 不因 source transplant 回退 | e2e/visual/a11y/profile + bounded product-surface falsifier | open | 既有 File/Viewer/Diff/Terminal/Git/stream/perf 证据仍是有效输入，但 fresh review 发现已获接受的 Agent/Chat 直接双入口被 transplant 回退为 dropdown，且 current narrow gates 未能发现；Public Surface resolver 当前正确，但还需由 sole-owner denylist 驱动的窄 production-source leakage falsifier。须在 F-17 focused 闭环后以当前路由原生实现、最小宽度/双语/a11y 与真实 Electron 重新冻结，不恢复旧 retained panels 或 generic identity platform | `d442d00a1027feeab819c761a4c0cae5ade5569b` |
...
| F-17 | 一套 OmniMind message catalog 覆盖正常用户可达产品面；中文/英文、IME、keyboard、screen reader、reduced motion、真实 long thread/burst/large output 达到可用质量；不把 source 零散文案冒充完整 i18n                                | catalog coverage + dual-locale/a11y/profile  | open | fresh-context source audit 证明原 parity/prefix/browser 探针不能发现 Settings 直达子面板的大量硬编码英文；既有 IME/a11y/perf 与 catalog 机制证据仍有效，但“正常用户可达产品面已覆盖”的 candidate 结论失效，须以硬编码 falsifier 和真实双语 Settings/Chat/Workbench/Library/error/updater 路径重新冻结 | `94e2225c371039ae825c0dd3cdc461bc02db0276` |
