...
| F-03 | Synara exact baseline `02c8a6c…` 的 source、lineage、rights、selected paths 与 OmniMind differences 可追踪                                                                                                                        | source/legal audit                           | candidate | sole README adoption record 保留 Synara exact source/rights，并记录 private workspace scope 从 `@synara/*` 无 alias 硬切到 `@omnimind/*`；`02d8c4a…` 的 target-derived identity falsifier、document contract 16/16 与 deterministic legal metadata 243 components 绿；Pi runtime adoption 与 retained licenses 不变 | `02d8c4a850cd7d7bc454b7243374d5f2bed04a9b` |
| F-04 | source alignment 只使用一个 exact Synara baseline，不维护 `stable + HEAD patches` 双轨                                                                                                                                            | source/tree reproducibility audit            | candidate | exact source 单次物理 transplant 保持；`02d8c4a…` 仅硬切 OmniMind-owned workspace namespace，1064 个非 owner/fixture 文件逐字证明无其他业务变更；frozen typecheck、full test 8/8、build 与 arm64 DMG install/reopen 绿 | `02d8c4a850cd7d7bc454b7243374d5f2bed04a9b` |
...
| F-10 | Synara Workbench、Settings、File/Viewer/Diff/Terminal/Git、stream/scroll、a11y/perf 在 surgery 后无 material regression；OmniMind-owned 一级入口和 first-party public exits 不因 source transplant 回退 | e2e/visual/a11y/profile + bounded product-surface falsifier | candidate | 既有 Workbench/quality evidence 保留；从 exact pushed `02d8c4a…` 构建并安装的 arm64 App 以任务专用 product/OS home 完成 Agent→Chat→Agent、关闭与同 profile 重开，Agent｜Chat、Kanban、新建任务/对话、Workbench 与 OmniMind 品牌可达，可见文本及 ASAR 无 Synara，Desktop→bundled Server 且无 service/native-host；真实 `.pi/.omnimind` 根 mtime/ctime/size 前后不变 | `02d8c4a850cd7d7bc454b7243374d5f2bed04a9b` |
...
| F-17 | 一套 OmniMind message catalog 覆盖正常用户可达产品面；当前与未来 OmniMind-owned 功能默认完整交付简中/英文且无逐项 fallback 混杂；产品术语、原始资产/诊断边界、自然双语语气、IME、keyboard、screen reader、reduced motion、真实 long thread/burst/large output 达到可用质量；locale 不改变用户内容或模型回复语言 | catalog/terminology/source coverage + dual-locale/a11y/profile | candidate | `1e3d465…` 的唯一 catalog、AST falsifier、双语 packaged journey、`8e951f6…` 的 diagnostics 分层与 exact pushed `df4b4bd…` 的 Chat 术语/空态证据保留；exact pushed `93f979c…` 以唯一 catalog 与共用 display-title helper 覆盖 generic Terminal title 的全部普通 UI consumer，focused unit 210/210、browser 13/13、React Compiler 13/13、document contract 16/16，且同 SHA 全仓 typecheck/test/build 绿；从该 SHA 安装的 fresh-profile Electron 实证中文主区/侧栏均为“新建终端”，切换英文后均为 `New terminal`，完整退出/重开后 locale 与投影一致保留，普通 Agent/Chat 空态无 placeholder heading 或 Provider icon；独立 completion audit pending | `93f979c4f054d0571945c7e22c0e599204b40c60` |
