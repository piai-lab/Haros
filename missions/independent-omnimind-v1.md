...
| F-05 | `Agent｜Chat` 按 `architecture/workbench.md` 成为侧栏顶部同时可见、Agent 左/Chat 右、一次激活的唯一一级工作模式；Agent 二级入口使用 `新建任务 / New Task`，Kanban 保持 Agent 域二级控制台；Projects 是完整 folder-backed Thread 来源，Groups 在其下默认折叠并以 Space identity + Thread 多选 membership 归类会话；空白 Agent/Chat 主顶栏无占位或 Engine 图标，真实标题只显示标题，Terminal 保留类型图标；无 Project 标签、未分组伪项、第二 durable objects/panel/navigation/title authority | source/data contract + minimum-width/dual-locale/a11y membership/create/delete/header/reopen journeys | candidate | `99618e5…` 的多分组、双入口、Kanban 与重开证据保留；exact pushed `df4b4bd…` 的整页 Chromium 组合路径 7/7 覆盖中英 Agent/Chat 空态、真实标题、重命名与 Terminal 例外，document contract 15/15；同 SHA 安装 App 在经进程参数确认的 fresh `/tmp` profile 中完成 Agent 空态→Chat 空态→本地真实标题→关闭/重开，普通顶栏始终无 Provider/Logo，Agent｜Chat、Kanban 与侧栏入口未回退。全仓 final gate 另有 3 个非本 concern 的既有/stale test failure，独立 completion audit pending | `df4b4bd97c6d3aa2d75a00db57a98e701830b083` |
...
| F-17 | 一套 OmniMind message catalog 覆盖正常用户可达产品面；当前与未来 OmniMind-owned 功能默认完整交付简中/英文且无逐项 fallback 混杂；产品术语、原始资产/诊断边界、自然双语语气、IME、keyboard、screen reader、reduced motion、真实 long thread/burst/large output 达到可用质量；locale 不改变用户内容或模型回复语言 | catalog/terminology/source coverage + dual-locale/a11y/profile | candidate | `1e3d465…` 的唯一 catalog、AST falsifier、双语 packaged journey 与 `8e951f6…` 的错误摘要/原始 diagnostics 分层证据保留；exact pushed `df4b4bd…` 将 Workbench/shortcut 的 Chat 动作统一为“新建对话”，catalog key/placeholder parity 与 terminology focused 绿；整页 Chromium 在中英 Agent/Chat 四种空态证明无硬编码占位 heading、无 phantom `h2`，真实标题/重命名与 720px 顶栏 geometry 保持。全仓 final gate 另有 3 个非本 concern 的既有/stale test failure，独立 completion audit pending | `df4b4bd97c6d3aa2d75a00db57a98e701830b083` |
