...
| F-05 | `Agent｜Chat` 按 `architecture/workbench.md` 成为侧栏顶部同时可见、Agent 左/Chat 右、一次激活的唯一一级工作模式；Agent 二级入口使用 `新建任务 / New Task`，Kanban 保持 Agent 域二级控制台；Projects 是完整 folder-backed Thread 来源，Groups 在其下默认折叠并以 Space identity + Thread 多选 membership 归类会话；无 Project 标签、未分组伪项、第二 durable objects/panel/navigation authority | source/data contract + minimum-width/dual-locale/a11y membership/create/delete/reopen journeys | open | `99618e5…` 的多分组、双入口、Kanban 与重开证据保留为输入；新的 Task/Chat 用户词汇和双语 installed-App 证据待当前变更闭合 | `99618e5a73d2be60c7cc5643a1ab225a642a210d` |
...
| F-17 | 一套 OmniMind message catalog 覆盖正常用户可达产品面；当前与未来 OmniMind-owned 功能默认完整交付简中/英文且无逐项 fallback 混杂；产品术语、原始资产/诊断边界、自然双语语气、IME、keyboard、screen reader、reduced motion、真实 long thread/burst/large output 达到可用质量；locale 不改变用户内容或模型回复语言 | catalog/terminology/source coverage + dual-locale/a11y/profile | open | `1e3d465…` 的唯一 catalog、AST falsifier、双语 packaged journey 与 `8e951f6…` 的错误摘要/原始 diagnostics 分层证据保留为输入；维护者新确认的完整词汇、future-feature gate、全局 copy 审校与 frozen installed-App 证据待当前变更闭合 | `96a37dd31a5580eceb1ceebe91074d553c530e31` |
