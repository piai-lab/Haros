...
| F-05 | `Agent｜Chat` 按 `architecture/workbench.md` 成为侧栏顶部同时可见、Agent 左/Chat 右、一次激活的唯一一级工作模式；Kanban 保持 Agent 域二级控制台；Projects 是完整 folder-backed Thread 来源，Groups 在其下默认折叠并以 Space identity + Thread 多选 membership 归类会话；无 Project 标签、未分组伪项、第二 durable objects/panel/navigation authority | source/data contract + minimum-width/dual-locale/a11y membership/create/delete/reopen journeys | open | 旧 direct-entry/Kanban evidence 仍是输入；维护者已锁定 conversation-level many-to-many Groups，需在新 frozen SHA 重建 source/data/browser/安装 App 证据 | `bb2978b92c23db1d12d6b0383c5690788afe16da` |
