...
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| F-01 | sole owners、旧 decision、source re-entry 与 executable document contract 对 single substrate、OmniMind-only 普通 UI、Agent/Chat、Provider-native ecosystem、Remote V2 无矛盾                                                     | structural contract + fresh document audit   | candidate | document contract 10/10；fresh-context audit pending                                                                                                                   | `ca36dbae24d5b0cfc48e9a0b2b59a207ab824fe3` |
| F-02 | Repository 只有一个 Product Orchestration、Provider Registry 与 Project/Thread/Space product-truth authority；平行 ProductControlPlane/Run/outbox/adapter 被删除                                                                  | source/tree/responsibility audit             | candidate | exact substrate tree；`apps/service`/`apps/native-host` 与旧 product-truth/control-plane 已删除                                                                        | `a54ef5ba49ac496604cd05eeaad0646f37faca8e` |
| F-03 | Synara exact baseline `02c8a6c…` 的 source、lineage、rights、selected paths 与 OmniMind differences 可追踪                                                                                                                        | source/legal audit                           | candidate | sole README adoption record 锁定 `02c8a6c…`、物理路径与有界 OmniMind delta                                                                                             | `a54ef5ba49ac496604cd05eeaad0646f37faca8e` |
...
| F-07 | bounded `omnimind` 与 stock `pi` 均进入 inherited Registry；OmniMind Agent 以 Pi `v0.84.1` 为 lineage，使用 `.omnimind` global/project-local state；stock Pi 显式选择时才使用 `.pi`，identity/version/config/session/state 不混用 | source/API + real-provider journeys          | open      | stock Pi slice：三包与 transitive Pi runtime 锁定 `0.84.1`；默认 discovery/health 不访问 `.pi`，显式选择后 session 跟随 stock `agentDir`；OmniMind Agent slice 仍 open | `010ce86c5e5482feab80015e3797a66ec9c81f49` |
| F-08 | command admission、native acceptance、interrupt、settlement 与 unknown 准确；OmniMind Agent 与 stock Pi 之间无 cursor/state/replay/fallback 混用                                                                                  | recovery/failure matrix                      | open      | stock Pi 无凭据在 `sendTurn` 前同步拒绝且无 fallback；跨双 runtime recovery 仍 open                                                                                    | `010ce86c5e5482feab80015e3797a66ec9c81f49` |
| F-09 | stream、reasoning、tool、structured request、usage/error 进入 existing Timeline 且保留 Provider provenance                                                                                                                        | event replay + UI/profile                    | open      | stock Pi + DeepSeek focused probe 已观察非空流式 delta 与自然 completion；完整 Timeline/reasoning/tool matrix 仍 open                                                  | `010ce86c5e5482feab80015e3797a66ec9c81f49` |
...
| F-19 | shipped inherited Provider 保持 discovery/health 和 source 已支持的 core journeys；ready/warning/error/auth/version/capability truth 准确；stock Pi session runtime 与 optional local CLI version 不混淆                          | provider smoke + source audit                | open      | stock Pi health 纯投影 bundled `0.84.1`，不再以本机 CLI 探测冒充 runtime；App updater 独占 bundled Pi 更新；其余 inherited Provider 仍 open                            | `010ce86c5e5482feab80015e3797a66ec9c81f49` |
