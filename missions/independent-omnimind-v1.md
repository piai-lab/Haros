...
- Probe E provisionally selects source files as authority with deletable in-place manifest/Wiki/FTS projections, deterministic source references before any optional semantic projection, and one product-owned immutable artifact-generation pipeline. Real Agent/model navigation, semantic recall, production artifacts, migrations and cross-platform rollback remain explicit M2/M5 revalidation gates. No temporary corpus, index, artifact, donor code, source adoption or acceptance status entered the repository. M1 remains open for Probe A's real package matrix and is not `candidate`.
