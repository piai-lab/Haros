...
| F-12 | Synara PluginLibrary/Skills/provider discovery 恢复；用户选择 Engine 后按 `architecture/execution.md` 组合完整 native ecosystem、兼容的 OmniMind Library 与 Workbench，保留 provenance/identity、private home 与准确 unavailable；无 shared PackageActivation/current/LKG 或跨 Engine durable state | native + additive ecosystem UI/journeys + home/isolation/conflict fault tests | candidate | 既有隔离 Electron native/additive ecosystem 与 frozen gate evidence 保留为输入；`3777c4c…` 让 Codex usage 继续由 native app-server 返回；exact pushed `96a37dd…` 的安装 App 又证明 stock Pi 只在显式展开后投影其 native MiMo models，受控 discovery 不改 `.pi` auth/models；OpenCode 保持其 Homebrew provenance，Droid 缺失被准确显示 unavailable；没有 shared activation/LKG、private-home migration 或 silent replacement | `96a37dd31a5580eceb1ceebe91074d553c530e31` |
...
| F-19 | shipped inherited Provider 保持 discovery/health 和 source 已支持的 core journeys；ready/warning/error/auth/version/capability truth 准确；stock Pi session runtime 与 optional local CLI version 不混淆                          | provider smoke + source audit                | candidate | `9ce368f…` 的 inherited Provider smoke、`3777c4c…` 的 Codex native auth/usage 与 `507318f…` 的 Pi intent gate 保留为输入；exact pushed `96a37dd…` 的安装 App 显式展开 Pi 后显示本机 MiMo models；OpenCode 准确识别 Homebrew owner，并从 UI 实际执行 `brew upgrade anomalyco/tap/opencode` 由 1.14.40 更新到 1.18.15，未走 npm/EEXIST 路；Droid 在本机缺失时显示 disabled unavailable，无假 update action、无 spawn；provider toast 位于右下角且成功摘要脱敏 | `96a37dd31a5580eceb1ceebe91074d553c530e31` |
