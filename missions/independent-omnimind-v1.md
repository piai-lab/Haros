...
| F-05 | `Agent｜Chat` 按 `architecture/workbench.md` 成为侧栏顶部同时可见、Agent 左/Chat 右、一次激活的唯一一级工作模式；Kanban 保持 Agent 域二级控制台；Projects 是完整 folder-backed Thread 来源，Groups 在其下默认折叠并以 Space identity + Thread 多选 membership 归类会话；无 Project 标签、未分组伪项、第二 durable objects/panel/navigation authority | source/data contract + minimum-width/dual-locale/a11y membership/create/delete/reopen journeys | candidate | exact pushed `99618e5…` 删除旧 Project↔Space 写入口与重复 standalone Chat section，以 Thread `groupIds` 持久化 folder-backed many-to-many membership；contracts 40/40、server 102/102、web unit 457/457、browser 20/20、document contract 14/14。安装版从 SHA 对应 DMG 以隔离 profile 创建 Project/Thread 与两个 Group，同一 Thread 在两组可见；Group/Thread 的 `Shift+F10` 菜单、Project provenance picker、删除 Group 保留 Thread/另一 membership、默认折叠、关闭重开、简中/English、最小侧栏、Agent↔Chat 与 `/kanban` 均真实通过；Desktop→bundled Server 且无 service/native-host | `99618e5a73d2be60c7cc5643a1ab225a642a210d` |
