...
在 `/Users/liuzaoqu/Desktop/Develop/independent/Haros` 完成英文 Haros Guidebook：50 章、8 个附录、Markdown 唯一内容源、140 个规划生成图槽位、160–220 张表格、18–24 张可复现真实界面证据图，并从同一 Markdown 派生网站、独立 HTML、PDF 与 EPUB。C-005 已由 E-051 独立验收并关闭生产；当前 Active Claims 是 C-006 与 C-007：以 C-001–C-005 为不可回归基线完成全书集成与同源四格式出版验收，直到全部 required Claims 在同一最终版本 verified。
...
第一安全动作：在任何生产写入前，原样运行 bootstrap，核对 E-051、当前 HEAD、dirty paths 与 C-006/C-007；一致后验证 Goal/spec，再进入当前 Claim 的 bounded evidence wave。
